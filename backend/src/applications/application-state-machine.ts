 ...
 * 但 VOIDED 於本 Phase 完全不可達——沒有任何轉換路徑寫入該值（AC-58）。
 * 作廢功能屬 PHASE-009；**PHASE-009 將擴充 ALLOWED_TRANSITIONS**
 * 以納入涉及 VOIDED 的轉換（例如 COMPLETED→VOIDED），屆時須連動修改本檔
 * 及本檔測試的矩陣斷言。
 *
...
/**
 * 本 Phase 可達之轉換集合（D2）：僅 DRAFT → COMPLETED。
 * 長度恆為 1——任何放寬都必須先經 Gate 決策批准，並在此加註引用該決策；
 * 測試檔以 `ALLOWED_TRANSITIONS.length === 1` 斷言防止未經批准的悄悄放寬。
 */
...
/**
 * 斷言狀態轉換合法；非法一律拋 AppError。DRAFT→COMPLETED 之外全部拒絕。
 *
 * 兩種拒絕情形，分別對應不同錯誤碼：
 *
 * 1. `from` 非 DRAFT（即對已完成或已作廢者嘗試轉換，含 COMPLETED→DRAFT、
 *    COMPLETED→VOIDED、VOIDED→* 等）→ 403 FORBIDDEN，訊息比照 D13。
 *    語意：來源狀態本身已鎖定，這是「權限/不可變性」層級的拒絕。
 *
 ...
 *    註：本 Phase 內所有已知呼叫端（T8 完成流程）只會以 to="COMPLETED" 呼叫，
 *    此分支目前是防禦性守門，非任何已知使用者可觸發路徑。
 */
