 ...
 * `createdAtToExclusive`（**不含**）。`where` 因而必須是 `gte` ＋ **`lt`**；
 * 寫成 `lte` 會讓 `dateTo` 當日整整多撈一天的列（該日末毫秒與次日首毫秒之間的
 * 分野消失）。
 *
 * **【2026-08-09 Mock Gate MG-2 裁定「以台灣日曆日為準」／`SPEC-REV-010-GATE`
 * 之 AC-02(e)】** 兩界之「日」為**台灣（UTC+8）日曆日**，故兩端點之值為：
 *   · `createdAtFrom`        ＝ `dateFrom` 之台北時 `00:00`
 *                            ＝ **前一日 `16:00:00.000Z`**
 *   · `createdAtToExclusive` ＝ `dateTo` 之台北次日 `00:00`
 *                            ＝ **`dateTo` 當日 `16:00:00.000Z`**
 * 本層之欄位名與 `gte`／`lt` 用法**零變更**（PHASE-010-T-MG1-BE2：
 * `buildAuditLogWhere` 零程式變更，僅兩個 `Date` 之值改變）。整合測試
 * （`test/integration/phase10-audit-query.test.ts` 之 FW-2／B-29）以
 * 「台北當日 `15:59:59.999Z` 必命中 ∧ 台北次日 `16:00:00.000Z` 必不命中」
 * 雙向釘死。
 *
 * ---------------------------------------------------------------------------
