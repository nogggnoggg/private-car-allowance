...
/**
 * AC-12(a) 之四類樣式（`conn-string`／`private-key`／`argon2-hash`／
 * `long-secret`）——PHASE-011-T17／AC-29(c)（D16-2）已收斂至
 * `test/support/secret-patterns.ts`（與 `phase11-backup-restore.test.ts` 曾各
 * 自之逐字複本合併為單一來源；四條 pattern 之 `id`／`regex` 逐字不變，理由
 * 見該檔頭之完整記載）。
 */
