...
describe("VoidApplicationDialog", () => {
  ...
  // ---- AC-33 五態 ----
  //
  // 【留白帳／AC-33 涵蓋範圍聲明（PHASE-009-T14R 即審 SF-1）】
  //
  // 本檔對 AC-33 之涵蓋為**部分**，非全滿——逐子態列明如下，勿據本檔認定
  // AC-33 已閉環：
  //
  //   ・Loading（送出中）           → 本檔**全滿**
  //   ・Error（400／409／500 各一）  → 本檔**全滿**
  //   ・Permission denied（403）     → 本檔**全滿**
  //   ・Empty                        → 本檔僅覆蓋**對話框層**（初始態無錯誤
  //     區塊、無送出中文案、原因欄為空）。AC-33 原文之 Empty 為
  //     **「未作廢時無作廢資訊區塊」**——那是**頁面層**語意（詳情頁未作廢時
  //     不得渲染作廢資訊區塊），本元件不負責該區塊，故本檔之 Empty
  //     **為代理、非等價**。該子態歸 **PHASE-009-T15**（三型詳情頁）。
  //   ・Success                      → 本檔僅覆蓋**對話框層**（200 後以回應之
  //     application 呼叫 onVoided 恰一次）。AC-33 原文之 Success 有兩條：
  //     **「作廢成功」＝ 頁面轉為唯讀檢視並顯示三項作廢資訊**（作廢原因／
  //     操作者／時間）歸 **PHASE-009-T15**；**「修正版建立成功」**歸
  //     **PHASE-009-T16**。兩者本元件皆不負責。
  //
  // 據此，§12 之 AC-33 映射列（:782）不應把 AC-33 單獨記於 T14——Task 欄之
  // 修訂由大總管以 Spec 勘誤批次另辦，本註解為該修訂前之留白帳。
