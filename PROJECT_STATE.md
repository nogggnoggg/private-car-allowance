...
> - **【新追蹤項（R1 實測確認之既有 src 行為）】`mileage-range.ts` tripDate 比較依賴 DB session TimeZone=UTC**：`date >= timestamptz` 比較依 session 時區轉換，非 UTC session 下 dateFrom 當日被靜默排除（off-by-one）。現以 SHOW TimeZone 守門測試顯式化；根治需 src 變更（如 AT TIME ZONE 'UTC' 或改綁 date 字面），建議併 PHASE-011 或基礎設施變動時處理。PHASE-004 AC-61 同性質。
