...
> - **【新追蹤項（T6 移交，另案）】PHASE-004 `GET /applications` 疑同型陣列 query 風險**：重複 query 鍵之非字串值流入 resolveOwnerId/parseApplicationListQuery，T4 即審已實測同輸入 500；T6 修復僅涵蓋統計端點（applications/** 在其 Forbidden）。建議 005 終審後以 Lite/批次回合比照 assertScalarQueryParams 模式修復並回歸。
