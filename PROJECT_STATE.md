...
> - **R1 輕量複審：APPROVE**（0 Must/0 Should/4 AR）。四項 finding 逐一鑑別力實證關閉（M-1② 雙向必紅、AC-07 生產路徑 53.75 必紅、S-1 reviewer 以唯讀探針證明 findMany 同為 1 次 SQL 故 SQL 文字斷言為必要鑑別）。AR-1 D2(a) 掃描缺正對照（一行補強）；**AR-2 TimeZone off-by-one 為真實生產缺陷，reviewer 建議提前至 005 終審前修復——大總管採納，開 R2**；AR-3 AC-13 檔內順序耦合（fail-closed，記錄）；AR-4 toBe(1) 對 Prisma 版本敏感（有意收緊）。**期中 Review 全數清零**（S-2 由 SPEC-SYNC-1 `75e3ea9` 關閉，R1 複審 reviewer 順手抽查 GREEN 測試名逐字存在）。
> - **R2（TimeZone 根治 + AR-1 正對照，src 變更）：派工中**。
> - **【追蹤項（R2 處理中）】`mileage-range.ts` tripDate 比較依賴 DB session TimeZone=UTC**：`date >= timestamptz` 比較依 session 時區轉換，非 UTC session 下 dateFrom 當日被靜默排除（off-by-one）。現以 SHOW TimeZone 守門測試顯式化；根治需 src 變更（如 AT TIME ZONE 'UTC' 或改綁 date 字面），建議併 PHASE-011 或基礎設施變動時處理。PHASE-004 AC-61 同性質。
