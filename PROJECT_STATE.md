...
> - **R4-LITE：派工中**——依 reviewer 撤回指示：移除 SHOW TimeZone 斷言與三處已證偽敘述（會為非需求原因變紅的測試與假測試同屬缺陷）；改以「Prisma @db.Date 之 date-OID 綁參」真性質守門（$transaction+SET LOCAL Etc/GMT+12 重跑四點斷言——Prisma 升版若改變綁參行為會靜默平移邊界並由 006/007 金額繼承）。編碼慣例（「日期區間比較一律走 Prisma 結構化 API，勿以 raw SQL+JS Date 比較 @db.Date」）記入 Spec 修訂紀錄。
