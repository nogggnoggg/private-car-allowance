...
> - **T8 即審＋T5 複審＋T4R 確認：REQUEST_CHANGES（0 Must/2 Should 皆純文件/8 AR；usage ~165k/75 tool uses）→ 文件面處置完畢、SF-1 停等人類**。T8 程式面零缺陷：AC-25 紅燈獨立重現（恰 3 紅零附帶）、13 mutant 全殺（含上限 5→6/5→4 雙側、MAINTENANCE 誤傷偵測）、26 wire 探針（五身分逐格、完成鎖定旁路封閉、併發 8 輪、管理員代掛語意 P4/P8 對偶證成）。T5 六 mutant 全殺（排除自身 7 紅、跨 owner 洩漏側 1 紅、D2(a) 雙側）；AC-08 等效論證成立。T4R M13 獨立重放：修復前全綠存活之缺口現由 4 條冗餘釘住。**SF-2 已處置**：§12 AC-06/07 GREEN、AC-08 PARTIAL（完成面待 T9）、AC-23/26 GREEN、AC-25 GREEN（T9 補真實流程重驗）。**★ SF-1 停等人類裁定 ★**：Spec B-22「去重後計數」與實作不符（reviewer P5b 實證：[b1..b5,b1] 去重後恰 5 應 200，實測 409 零關聯；行為與 006 逐行同型、fail-closed）——(A) 改 Spec 措辭認可 409 現況（推薦，零程式，與 006 一致）／(B) 改程式去重（行為變更，折舊與保養分岔，須 Spec Gate）。AR 記錄：AR-3 分支未校驗 type（三分支對稱缺省，現無寫入路徑可達）、AR-5 UTC 午夜窄窗、AR-7 notice 交易外計算（advisory 正確、T9 不得沿用做完成判定）、AR-8 保序依賴續掛帳（T9/T11 必引）。
