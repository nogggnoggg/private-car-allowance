 ...
 * 缺裝，經 `font-family` 回退鏈）下 Chromium/Skia 對此文件改採逐字元
 * `/Subtype /Type3` 字型內嵌（單頁 70+ 個字型子集），其 `ToUnicode` CMap 對
 * 部分字元（含一次性自訂錨點文字）之覆蓋不完整（物件級 dump 逐一驗證：
 * `SEGORIGIN1` 之 `S` 字元 glyph code 於對應字型之 CMap 內完全無條目），
 * 造成錨點比對不可靠，且無法排除與正式容器（已裝字型）之字型內嵌管線不
 * 同構。
 *
