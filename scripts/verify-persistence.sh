...
#
# 結束碼：0＝三指紋全等且模式斷言成立；1＝任一不等或斷言不成立；2＝前置條件不足（環境／參數）。
# `destroy` 模式**必然**回 1——那正是「本驗證不是恆真」的證據，不是缺陷。
#
# 用法：
#   scripts/verify-persistence.sh [--mode preserve|destroy|rebuild-backend|rebuild-frontend] [--yes-destroy-data]
#
...
MODE=preserve; CONFIRM_DESTROY=0
...
wait_ready() { # 三面就緒：db/backend 之 compose health ＋ 經 nginx 的 /api/health 回 200
  ...
  die "等待逾時（${WAIT_TIMEOUT_SEC}s）：/api/health 未回 200（最後 $code）" 1
}
