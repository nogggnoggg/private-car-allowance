...
- **正式 PDF 沿用附件之同一紀律（PHASE-008）**：PDF **一律經授權端點回傳**，volume **不得由 nginx 靜態直出**——此為上一條既有紀律之延伸，並新增**結構性守門**：以解析 `nginx.conf.template` 之 `location`／`server` 區塊斷言無任何 `root`／`alias` 指向 storage／report volume 路徑（含裸 `root`、帶正則或前綴修飾子之 `location`、`if` 巢狀大括號三種繞過型態之鑑別力自證）。`storageKey` 由系統產生、不含使用者輸入。
