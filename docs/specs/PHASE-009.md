...
> **預授權（沿 T11b／R1b 先例）**：上述**六類**（第六類為 `SPEC-REV-9T3` 補列）連動之修改，於 Spec Gate 通過後**視為同批預授權**——相關 Task 之 Packet 得直接修改該些既有測試檔之**基線值**，無需另行請示；但**任何鑑別性斷言之弱化、刪除或 skip 一律禁止**，且 implementer 須於 Handoff 逐檔列出實際改動處數與 diff 行數。
...
| AC-03 | 純函式（無 DB）＋ integration，**同檔** | `backend/test/integration/phase9-void.test.ts`（**單一落點**——gate table 置於該檔**頂層 `describe`**，15 個 `it` 皆為純函式、不需 DB 亦執行；**不另立** `void-reason.test.ts`，`backend/src/applications/void-reason.ts` 之覆蓋即由此頂層區塊承擔） | `AC-03: 作廢原因 gate table（12 列：缺鍵／null／空字串／半形空白／全形空白／Tab／換行／1 字元／500 字元／501 字元／前後空白 trim／XSS payload）` › `缺鍵（undefined）→ 拒絕`／`null → 拒絕`／`空字串 "" → 拒絕`／`半形空白（"   "）→ 拒絕`／`全形空白（"　　　"）→ 拒絕`／`Tab（"\t\t"）→ 拒絕`／`換行（"\n\n"）→ 拒絕`／`混合空白（全形空白＋Tab＋換行）→ 拒絕`／`恰 1 字元 → 接受`／`` 恰 ${VOID_REASON_MAX_LENGTH} 字元 → 接受 ``／`` ${VOID_REASON_MAX_LENGTH + 1} 字元 → 拒絕 ``／`前後空白（B-03："  誤植金額  "）→ 接受，value 為 trim 後之 "誤植金額"`／`XSS payload（B-04）→ 接受，原樣（未跳脫）保留於 value——跳脫是呈現責任（T11），非驗證責任`／`非字串（number）→ 拒絕`／`fields[] 恰含 { field: "reason" }（AC-03 明文）`；`AC-03: 400 時零寫入（status 不變、四欄仍 NULL、零稽核列）` | T3 | PENDING |
...
| AC-21 | integration | `phase9-void-report.test.ts` | `AC-21(a): 作廢版 PDF 同交易產生並校驗；任一階段失敗 → 整筆回滾、狀態仍 COMPLETED、零殘留、500 + details.stage`；`AC-21(b): 作廢後下載回作廢版且報表編號與檔名不變`；`AC-21(c): 原 Report PDF 位元組雜湊前後不變`；`AC-21(d): 作廢後重複下載位元組全等且渲染器零呼叫`；`AC-21(e): 未產生報表者作廢 → 零渲染零 storage 寫入`；`AC-21(f): reports/ 零 report.update／delete／upsert 之既有結構斷言零改動且全綠`——**`SPEC-REV-9T12` 拆分**（大總管 2026-08-07 依治理 §10「大接線一律預拆」裁定）：**(a)(c)(e)(f) 歸 T12a**（作廢交易內之產生／保存面：渲染 → `put` → 讀回校驗 → `INSERT VoidedReportFile` → 提交／整筆回滾）、**(b)(d) 歸 T12b**（下載語意端點面）。**⚠ AC-21 不得於單側綠時整條標 `GREEN`**——T12a 綠不蘊含 (b)(d) 綠（沿 AC-08 先例） | **T12a**（(a)(c)(e)(f)）＋ **T12b**（(b)(d)） | PENDING（D4=b1 已裁定解鎖） |
...
| AC-23 | integration | `phase9-contract.test.ts` | `AC-23: 授權矩陣 10 格逐格（狀態碼 + 回應鍵集）`；`AC-23: 報表四端點於 VOIDED 之回歸四格`；`AC-23: 403 回應零業務值掃描`——**`SPEC-REV-9T7` 加註（T4 即審 SF-1 銷帳裁定／FW-A）**：報表四端點回歸四格中之「**下載 → 200（依 D4）**」一格，須待作廢版 PDF 落地方可成立，故**歸 T12**（T12 補該格並回頭更新 `phase9-contract.test.ts` 檔頭之留白清單）；T4／T7 交付時該格**未覆蓋屬預期留白，非漏測**——終審覆蓋檢查時不得據此判 T4／T7 有缺。**`SPEC-REV-9T12` 承接**：T12 拆分後該格歸 **T12b**（下載語意面），`phase9-contract.test.ts` 檔頭留白清單之更新一併由 T12b 執行 | T4 ＋ T7（＋「下載 → 200」一格歸 **T12b**） | PENDING |
...
| AC-32 | frontend | 三型頁測試檔 | `AC-32(a): 已完成頁出現「建立修正版」按鈕（取代佔位文案）`；`AC-32(c): 原頁顯示指向修正版之連結、修正版頁顯示指向原申請之連結`；`AC-32(d): 附件區提示含「如需修正附件，請建立修正版」`；**`SPEC-REV-9T16` 補列（提醒文案面，歸 T16R）**：`AC-32(c) 提醒文案: 原申請頁（supersededBy 非 null）之版本關係區塊顯示重複計入提醒 — 差旅／保養／折舊三型各一`；`AC-32(c) 負向: 已完成但未建立修正版時零提醒文案（防恆真）`——**⚠ AC-32 不得於單側綠時整條標 `GREEN`**（沿 AC-08／AC-21 先例）：連結面（T16）綠不蘊含提醒文案面（T16R）綠 | **T16**（(a)／(c) 連結面／(d)）＋ **T16R**（(c) 提醒文案面） | PENDING |
...
| **AC-41**（`SPEC-REV-9GATE` 新增；Mock Gate 定案③） | integration（投影面）＋ frontend（呈現面） | `phase9-void.test.ts`；三型頁測試檔 | **投影面（T15c）**：`AC-41 投影: 三型 VOIDED 詳情之 void.totalAmount ＝ 完成時 Application.totalAmount（作廢前後同值）`；`AC-41 投影 負向: 未作廢 → void 恆 null（既有斷言零弱化，金額無從外露）`；`AC-41(d) 結構: VOIDED 之 snapshot 仍恆為 null — 三型（定案③「不恢復整個快照區塊」；三型 service 檔零 diff）`。**呈現面（T15d）**：`AC-41(a): 已作廢詳情頁「作廢當時金額」逐字標籤 ＋ 整數原樣值 — 差旅／保養／折舊三型各一`；`AC-41(b) 負向: 未作廢之已完成頁零「作廢當時金額」字樣（防恆真）`；`AC-41(c): void.totalAmount 為 null 時該列顯示「—」`；`AC-41(d) 呈現: 已作廢唯讀頁零單價／比例／計算明細` | **T15c**（投影面）＋ **T15d**（呈現面） | PENDING |

> **⚠ AC-41 不得於單側綠時整條標 `GREEN`**（沿 AC-08／AC-21／AC-32 先例）：投影面（T15c）綠**不蘊含**呈現面（T15d）綠。

> **本表統計**：**41 條 AC**（`SPEC-REV-9GATE` 由 40 增為 41——新增 **AC-41**，係 Mock Gate 定案③之人類裁定落地）；映射表 **48 列**（`SPEC-REV-9GATE` 由 47 增為 48：新增 AC-41 一列）；原 47 列之構成如下——（`SPEC-REV-9T7` 由 45 列增為 47：AC-01 拆為 (a)(c)(d)(h)／(b)／(e)／(f)／(g) **五列**、**AC-08 拆為 (a)(b) 兩列**（`SPEC-REV-9T3`）、**新增 `AC-27／B-16` 與 `AC-32／§7.2 isRevision` 兩列**（皆為既有條文之覆蓋缺口補列，**非新增 AC**），其餘各一列）。其中 **8 列標記 `PENDING（Spec Gate 2026-08-07 裁定後解鎖）`**——**6 條完整 AC**（AC-14／AC-18／AC-21／AC-22／AC-38／AC-40）＋ **AC-01 之兩個子項**（(b) 依 D4、(e) 依 D10）。**狀態欄現況**：**1 列 `GREEN`**（AC-08(b)，T4b 即審 APPROVE），其餘 **47 列** `PENDING`（`SPEC-REV-9GATE` 後由 46 增為 47；T1~T7 已交付部分之實名回填與狀態轉綠仍併結案 DOC-SYNC 批次辦理）。
