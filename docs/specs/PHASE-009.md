...
| AC-37 | 全 Task ＋ 終審 | —（**無單一測試載體**；以三套件實跑數為證） | 每個 Task 之 Done When 含「既有測試零弱化」；終審以三套件**自行實跑**對照基準線 **後端 3230／前端 346／E2E 50**（起點 2952／254／44）。**目前證據**：後端 3230 ＝ T18 結案後大總管獨立驗收實跑；前端 346 ＝ T15d 結案實跑；E2E 50 ＝ T17 實跑 ＋ 大總管親跑複驗（含冪等重跑）——**惟三者為分次量測、非同一時點之全套齊跑**，故**本列不轉綠**，待終審一次實跑三套件後由終審轉 `GREEN` | 全 Task ＋ 終審 | `PENDING`（**待終審三套件實跑核對**——見表後未轉綠清單） |
