...
| 4 | **機械連動預授權**（`ReportData` 擴欄 → AC-27 鍵集封閉 ＋ `PHASE_008_SRC_FILES` 九檔清單必紅；Task 拆分時預授權同批修改，沿 T11b／R1b 先例） | **明文寫入 Task 列**：§15 T10／**T12a**（`SPEC-REV-9T12` 拆分後；原記 T12）之「機械連動預授權」欄逐檔列出（`phase8-report-data.test.ts` 鍵集列、`phase8-contract.test.ts` 之 `PHASE_008_SRC_FILES`）；另**新增本 Phase 專屬之三類連動**（Application 欄位集合、`AuditAction` enum 基線、業務表數）並同樣預授權 | §15 各 Task「機械連動」欄、§8.5 |
...
**AC-03 作廢原因之驗證（必填）**——`reason` 經 `trim` 後長度須落於 **1..500**；缺鍵／`null`／空字串／**僅空白字元（含全形空白 `　`、Tab、換行）** 一律 **400 `VALIDATION_ERROR`** 且 `fields[]` 恰含 `{ field: "reason" }`；超過 500 字元亦 400。**拒絕時零寫入**（`status` 不變、**三個作廢欄**（`voidReason`／`voidedAt`／`voidedById`）仍為 `NULL`、零稽核列）。持久化值為 **`trim` 後**之字串（前後空白不入庫）。

> **措辭精確化（`SPEC-REV-9DOCA`；沿 `SPEC-REV-9T7` ④ 對 AC-09 之同型處置，驗收語意零變更）**：原文作「四個作廢欄仍為 `NULL`」——第四個新欄 `supersedesId` **不屬作廢欄**，且於「本身即為修正版」之申請上**恆非 `NULL`**，故該處語境（作廢**被拒絕**時之零寫入）正確表述為三欄。**測試零修改義務**：`phase9-void.test.ts` 之既有測試名沿用「四個作廢欄仍為 `NULL`」，係其 fixture 為非修正版之已完成申請（四個新欄本即皆 `NULL`），斷言強度不變且更強（多鎖一欄）；本次僅統一 Spec 用語。
...
**AC-32 修正版入口與版本關係（FE-US-25①③、FE-US-21④、AD-US-08③）**——(a) 已完成申請詳情頁出現「建立修正版」按鈕（取代現況佔位文案「功能將於後續版本提供」，spec-writer 實查 `TravelApplicationPage.tsx` :510）；(b) 點擊成功後導向新草稿頁；(c) 原申請詳情頁顯示「已建立修正版」與**指向修正版之連結**；修正版頁顯示「本申請為 {原編號或原日期} 之修正版」與**指向原申請之連結**；**且原申請詳情頁之版本關係區塊（`supersededBy` 側）必含「重複計入」提醒文案**【`SPEC-REV-9T16` 補入——依 §16 **D15** 之人類 Spec Gate 批准前提「**不自動作廢 ＋ 前端強提示**」（§18 之 Spec Gate 裁定列「D15=(a) 不自動作廢＋前端強提示」；D15 推薦段「**但強制搭配前端提示**（AC-32(c) 之版本關係區塊加註提醒文案）」；§17.1 #1 之緩解清單「前端提示 ＋ 版本關係顯示 ＋ Gate 目視」）。**原 AC 條文漏載該提示，屬 Spec 覆蓋缺口**：依字面實作則已批准之緩解措施全案零落地】：**語意要求（三點不可省）**——①原申請**仍為已完成狀態**（未因建立修正版而自動作廢）；②**若不作廢，原申請與修正版兩筆都會計入里程與金額統計**；③**作廢入口即在本頁**。**逐字字串**（可直接實作並逐字斷言）：`本申請仍為已完成狀態。若未作廢，本申請與修正版將同時計入里程與金額統計；如需避免重複計入，請於本頁作廢本申請。`——**`SPEC-REV-9DOCA` 去「暫定」二字**：該逐字文案已於 **Mock Gate（人類 leonchih 2026-08-08）定案①「DUPLICATE_COUNT_WARNING 照現值定案」**經人類目視確認為最終值；日後若人類要求調整，仍走 Gate 反饋流程（Spec 修訂 → implementer → 複審），**上開三點語意不因文案微調而縮減**。**測試義務**：三型詳情頁**各一正向**（`supersededBy` 非 `null` 時提醒文案在場）＋ **一條負向**（`supersededBy` 為 `null` 之已完成頁零提醒文案，防恆真；§11.0 #2）。**修正版側（`supersedes`）不加此提醒**——作廢動作之落點在原申請頁。(d) 附件區之提示文案含「如需修正附件，請建立修正版」（FE-US-21④）；(e) 管理員代操作路徑之入口與文案一致（AD-US-08③）。
...
**AC-37 既有測試零弱化與基準線**——每個 Task 之 Done When 含「既有測試零弱化」；終審以三套件實跑數對照基準線（**後端 3230／前端 346／E2E 50**）核對；本 Phase 之機械連動（AC-01(f)）**僅更新基線值**，不得刪除或 skip 任何既有測試。

> **基準線更新（`SPEC-REV-9DOCA`；機械連動之基線重錨定，非弱化）**：起點基準線為 **後端 2952／前端 254／E2E 44**（PHASE-008 終審實測）；本 Phase 全部 Task 交付後之終值為 **後端 3230／前端 346／E2E 50**（後端 3230＝T18 結案後大總管獨立驗收實跑；前端 346＝T15d 結案實跑；E2E 50＝T17 實跑 ＋ 大總管親跑複驗，含既有 44 之全 Phase 變更後首次實跑全綠）。**三數皆為淨增（+278／+92／+6），零刪除、零 skip**。終審須以三套件**自行實跑**對照本三數，不得採信本 Spec 之轉載值。
...
### L. 【Mock Gate 定案③】已作廢詳情頁之作廢當時金額

**AC-41 已作廢詳情頁顯示「作廢當時金額」（Mock Gate 定案③＝(b)；`SPEC-REV-9GATE`）**——(a) **三型逐一正向**：`status === "VOIDED"` 之詳情頁「作廢資訊」區塊**末列**新增一列，標籤**逐字** `作廢當時金額`（**`SPEC-REV-9DOCA` 去「暫定」二字**：Mock Gate 定案③之人類原話即「作廢當時金額」逐字，T15c 結案列已記載「標籤視為已定案，不再另問」，T15d 已依此逐字落地並經即審 APPROVE），值＝該申請**完成時保存之最終整筆金額**（`Application.totalAmount`；新臺幣整數——AC-04 保證作廢**不清空**該欄），以**整數原樣**呈現（沿既有 `<dd>{snapshot.totalAmount}</dd>` 之慣例：零千分位、零幣別前綴、零單位字尾），與列印版／PDF 之 `最終金額（新臺幣整數）`（`report-html.ts` :212／:256／:325，同樣 `String(totalAmount)`）為**同一整數、同一長相**。(b) **負向（防恆真，§11.0 #2）**：**未作廢**之已完成詳情頁**零** `作廢當時金額` 字樣——結構性保證為 `void === null` 時整個作廢資訊區塊不渲染（AC-33 Empty），至少一型逐字負向。(c) **邊界**：`void.totalAmount` 為 `null` 時該列顯示 `—`（沿 `ApplicationListSection.tsx` :346 之既有 `totalAmount === null → "—"` 慣例），**不得**整區消失、**不得**拋錯；經正常流程（完成 → 作廢）之列**恆非 `null`**（完成流程寫入 ＋ AC-04 保證不清空）。(d) **範圍守門（定案③「單一欄位、不恢復整個快照區塊」之機械落點）**：`VOIDED` 詳情 DTO 之 `snapshot` **仍恆為 `null`**（三型；`toTravelApplicationDto`／兩個 `buildSnapshotDto` 之 `status === "COMPLETED"` 守門**本次零改動**之結構性斷言），已作廢唯讀頁**零**單價／比例／計算明細之呈現——**本 AC 只放行單一金額欄，不得順帶放行任何其他快照欄**。

> **實作註記（後端投影面實查結論；`SPEC-REV-9GATE` 據實落檔，實查基準 `5436fd8`）**——結論：**AC-41 非純前端，須含後端 DTO 最小調整**。
>
> 1. **現況＝金額被快照守門完全擋住**：三型詳情 DTO **均無** top-level `totalAmount` 鍵；該值**僅**存在於 `snapshot` 內（`TravelSnapshotDto`／`MaintenanceSnapshotDto`／`DepreciationSnapshotDto` 之 `totalAmount: number`），而三處 `snapshot` 皆由 `status === "COMPLETED"` 閘住（`travel-service.ts` :1800、`maintenance-service.ts` :838、`depreciation-service.ts` :902）→ **`VOIDED` 下 `snapshot` 恆 `null`，金額完全不在回應中**，前端無從顯示。
> 2. **落地形狀（Spec 決策；內部 DTO 結構，記錄型）**：於既有 `VoidInfoDto`（`backend/src/applications/application-void.ts` :66）**新增一鍵** `totalAmount: number | null`，由同檔 `resolveVoidInfo`（:86）一併投影。理由三點——①`void` 於未作廢時恆 `null`，故**未作廢之回應逐鍵不變**，AC-41(b) 之負向由**結構**保證而非僅靠 UI 條件；②`resolveVoidInfo` 為三型共用之單一函式，**三個 service 檔零改動**（呼叫端傳入之 `application` 列本即帶 `totalAmount`，參數型別以結構化子型別相容）；③折舊詳情 DTO 之**封閉鍵集斷言**（`phase7-depreciation-draft.test.ts` :920，機械連動**第六類**，§8.5）**完全不受影響**（top-level 鍵集不變）。**已考慮並否決之替代形狀**：於三型 DTO 各加 top-level `totalAmount` 鍵——須改三個 service 檔 ＋ 觸發折舊封閉鍵集連動 ＋ 使 `DRAFT`／`COMPLETED` 回應一併變形，範圍顯著大於定案③且無對應收益。
> 3. **零新增資料揭露（權限面零變更）**：同一整數**早已**由列表 DTO 以 top-level 鍵**無條件**投影（`application-query.ts` :422 型別／:494 `totalAmount: app.totalAmount`，**不受 `status` 閘門**）並由列表頁對已作廢列直接呈現（`ApplicationListSection.tsx` :346）。故本 AC **不是新的揭露決策**，而是**消除「列表看得到、詳情看不到」之不一致**；授權路徑與可見範圍逐字不變。
> 4. **機械連動（既有封閉斷言之受控擴充，非弱化）**：`phase9-void.test.ts` 三型 `void` 區塊之閉集斷言（:1037-1041 保養、:1060-1064 折舊之 `toEqual`；差旅為逐鍵 `expect`，:1018）須**同批**補入新鍵，否則必紅；前端 `VoidInfoDto` 副本（`frontend/src/api/applications.ts` :124）與三型頁測試檔之 `voidInfoFixture`（`TravelApplicationPage.test.tsx` :62、`MaintenanceApplicationPage.test.tsx` :116、`DepreciationApplicationPage.test.tsx` :145）須**同批**對齊，否則 `frontend` typecheck 必紅。上述連動即 §15 **T15c** 之檔案清單依據。
...
export interface VoidInfoDto {
  ...
  totalAmount: number | null;   // AC-41：作廢當時金額（＝完成時之 Application.totalAmount）
}
...
/*
 * `VoidInfoDto.totalAmount`（SPEC-REV-9DOCA 補記；宣告落後之勘誤）——
 * `SPEC-REV-9GATE` 已於 §2 §L 之 AC-41 實作註記 #2 明定「於既有 VoidInfoDto
 * 新增一鍵 totalAmount: number | null，由同檔 resolveVoidInfo 一併投影」，惟
 * 當次 Packet 之硬約束為「零其他條文修改」，故上方宣告區塊未同步（該次已列
 * 為殘留項回報大總管）。本次據 T15c 已交付之實作（`application-void.ts` 之
 * `VoidInfoDto`／`resolveVoidInfo`；`totalAmount` 為**必填非 optional**，以型別
 * 攔截投影靜默 undefined）補齊記載。**三鍵→四鍵為記載補齊，非新增決策**；
 * 未作廢時 `void` 恆為 `null`，故未作廢之回應仍逐鍵不變（AC-41(b) 之結構保證）。
 */
...
**鍵集紀律**【**已修訂（`SPEC-REV-9T3`；原敘述對折舊型證偽**）】：初版敘述為「三型詳情 DTO 之既有實作**無**封閉 `toEqual` 鍵集斷言」——**該敘述僅對差旅／保養成立，對折舊為誤**：`backend/test/integration/phase7-depreciation-draft.test.ts` :915／:920 之 `DepreciationApplicationDto 之鍵集全等於 §20.6 宣告（含新增之 annualTotalKm）`（describe `§20.6 DTO 鍵集封閉：三型 DTO 皆不外露車價／年限／預估年里程／版本 id`）**確為封閉 `toEqual`**，PHASE-007 §20.6 已固化。故：**折舊 DTO 每新增一鍵，該處清單必須同批同步，否則必紅**（機械連動**第六類**，見 §8.5；`void` 一鍵已由 **T3b-LITE** 落地同步，`supersedes`／`supersededBy` 兩鍵之同步屬 **T7**）。差旅／保養兩型 DTO 經實查確無同型斷言（全 repo `Object.keys(application).sort()` 僅此一處命中）。新增鍵一律仍須有**逐鍵正向 ＋ 負向（未作廢／無修正版時為 `null`）**測試。列表 DTO 之既有 **14** 鍵逐字不變（**`SPEC-REV-9DOCA` 鍵數勘誤**：原作「17 鍵」為記載失準——T16pre **W-1** 實查後端 `ApplicationListItemDto` 與前端獨立副本皆為 14 鍵，加 `isRevision` 後為 15；§8.5 第七類與 §12 之對應列同批更正）。
...
model VoidedReportFile {
  ...
  storageKey  String   @unique                    // "rpt/<uuid>/void"（系統產生；SPEC-REV-9DOCA 勘誤，見下）
...
> **`storageKey` 形狀勘誤（`SPEC-REV-9DOCA`；T12a **W-2** 裁定）**：初版註解作 `"rpt/<uuid>/void.pdf"`——**實際落地為 `rpt/<uuid>/void`（無 `.pdf` 副檔名）**。原因非風格選擇：既有 storage key 白名單**拒絕 `.` 字元**，帶副檔名之 key 無法通過。T12a 已依此落地並經即審 APPROVE；`fileName`（對外檔名，含 `.pdf`）與 `storageKey`（內部物件鍵）為**兩個不同欄位**，AC-21(b)「`Content-Disposition` 之報表編號與檔名不變」取的是 `fileName`，故本勘誤**零行為影響**。**同源殘留（不在本 Task 之 Allowed 檔案）**：`backend/prisma/schema.prisma` 之同欄註解仍作 `void.pdf`（T12b 即審 FW 已登記），須由大總管另派 Lite 更正。
>
...
| **（第六類，`SPEC-REV-9T3` 補列）折舊詳情 DTO 鍵集封閉** | `phase7-depreciation-draft.test.ts` :915／:920（`DepreciationApplicationDto 之鍵集全等於 §20.6 宣告（含新增之 annualTotalKm）`）—— **一處** | **折舊 DTO 每新增一鍵即須同批加入該清單**（否則 `toEqual` 必紅）：`void` 一鍵已由 **T3b-LITE**（`d94cc62`，該檔 `+1/-0`）落地同步；`supersedes`／`supersededBy` 兩鍵之同步歸 **T7**。**差旅／保養兩型無同型斷言**（實查：全 repo `Object.keys(application).sort()` 僅此一處） |
| **（第七類，`SPEC-REV-9DOCA` 補列）列表 DTO 鍵集基線** | `phase9-contract.test.ts` **§M**（`PHASE-009-T16pre — 列表 DTO \`isRevision\`（§7.2）＋ 列表鍵集基線` › `§7.2 鍵集基線: 列表項鍵集全等於 15 鍵宣告（多一鍵／少一鍵／改名必紅）`）—— **一處** | **列表 DTO 每新增一鍵即須同批加入 `APPLICATION_LIST_ITEM_KEYS`**（否則 `toEqual` 必紅）。**本類為 T16pre 首次開列**（T16pre **W-2** 實查：本 Task 之前全 repo **無**任何列表鍵集封閉斷言，故屬「首次開列」而非「基線重錨定」）；`isRevision` 一鍵已由 **T16pre**（`d568380`）落地。**鍵數勘誤**：既有 **14** 鍵 ＋ `isRevision` ＝ **15**（`SPEC-REV-9DOCA` 更正——§7.2 之「既有 17 鍵」與 §12 原記之「17→18」皆為記載失準，實查 14→15；前端獨立副本 `ApplicationListItemDto` 交叉印證同為 14 鍵）。**R-1（T16pre 登記）**：基線既已開列，後續任何列表增鍵之 Task 須於 Packet 列本連動點，否則撞無授權紅燈 |

> **預授權（沿 T11b／R1b 先例）**：上述**七類**（第六類為 `SPEC-REV-9T3` 補列、第七類為 `SPEC-REV-9DOCA` 補列）連動之修改，於 Spec Gate 通過後**視為同批預授權**——相關 Task 之 Packet 得直接修改該些既有測試檔之**基線值**，無需另行請示；但**任何鑑別性斷言之弱化、刪除或 skip 一律禁止**，且 implementer 須於 Handoff 逐檔列出實際改動處數與 diff 行數。
...
| 作廢原因驗證純函式 | **`backend/test/integration/phase9-void.test.ts` 之頂層 `describe`**（`SPEC-REV-9T3` 更正——不另立 `backend/test/unit/void-reason.test.ts`；該區塊 15 個 `it` 皆為純函式、無 DB 依賴，於無 `DATABASE_URL` 時亦執行） | AC-03：gate table（≥12 列，含全形空白、Tab、換行、500／501 邊界、trim） |
...
| 作廢流程 | `phase9-void.test.ts`（新） | AC-03~AC-08、AC-24／AC-26（作廢側）；B-08 併發雙作廢；快照逐欄不變；**AC-41 投影面**（`void.totalAmount` 三型正向 ＋ 未作廢負向 ＋ (d) `snapshot` 恆 `null` 之結構守門——`SPEC-REV-9DOCA` 補列，落點為 §12 之權威記載） |
...
| 三型詳情頁 | 既有三檔（只增不改） | AC-30／AC-31／AC-32：唯讀分支、作廢資訊、版本關係、文案更新；**AC-41 呈現面**（「作廢當時金額」逐字標籤 ＋ 整數原樣值 ＋ `null`→`—` ＋ `0` 邊界 ＋ (d) 呈現側負向——`SPEC-REV-9DOCA` 補列）；**AC-33 之 Empty／Success 兩子態亦落於本三檔**（頁面層語意，非對話框層——T14 即審 **SF-1** 裁定） |
| 列表 | `frontend/test/ApplicationListSection.test.tsx`（既有，只增不改） | AC-30(a)(b)：已作廢狀態欄與狀態篩選；**修正版徽章**（`isRevision` 正負向 ＋ 混合列表逐列綁定——`SPEC-REV-9DOCA` 補記，落點見 §12「AC-32／§7.2 列表 DTO `isRevision`」列） |
...
> **格式**：`AC` ｜ `層級` ｜ `測試檔（實際路徑）` ｜ `測試名（實名）` ｜ `Task` ｜ `狀態`
> 狀態值：`PENDING`（尚無測試證據或待終審實跑）／`RED`（測試已寫且失敗）／`GREEN`（實作完成通過且該 Task 鏈 Review 清零）。
> **Phase 完成前之覆蓋檢查與終審核對以本表為準**（治理 2026-08-02.1）。
> **`GREEN` 列之測試名一律為實際存在之測試名（逐字可 `grep`）**；巢狀者以 `describe 標題 › it 標題` 表示。**`SPEC-REV-9DOCA` 全表回填**：本次以 `rg`／逐檔實開對 48 列逐列覈實，將預定名替換為實名並轉綠；**凡無測試證據之列一律不轉綠**（見表後未轉綠清單）。表列僅載**鑑別性代表測試**，非該檔全部 `it` 之窮舉——覆蓋檢查以「該 AC 之每一子項至少有一條實名在場」為準。

| AC | 層級 | 測試檔 | 測試名（實名） | Task | 狀態 |
|---|---|---|---|---|---|
| AC-01(a)(c)(d)(h) | integration | `backend/test/integration/phase9-migration-safety.test.ts` | `PHASE-009-T1 migration safety net — existing DB` › `AC-01(a): Application 之四個新欄 — 型別／可空性／無 default 逐欄符合 §8.1（14 欄封閉集合）`／`AC-01(a): Application.supersedesId — 唯一約束 ＋ FK(onDelete: Restrict) 指向 Application(id)`／`AC-01(a)/(b): 三個新唯一約束各自獨立拒絕碰撞（Application.supersedesId／VoidedReportFile.reportId／VoidedReportFile.storageKey）`／`AC-01(a): FK onDelete Restrict — deleting an Application referenced by another's supersedesId is rejected`／`AC-01(c) 九表零改寫: every pre-existing column, across nine representative tables this migration does NOT touch, is byte-for-byte unchanged`／`AC-01(c) 突變自證: the nine-table zero-rewrite comparator is discriminating — a rewritten value, and an ADDed column, each make it red`／`AC-01(c): Application 既有列 — 原 10 欄逐欄零改寫，新增 4 欄一律 NULL（零回填；多餘 ADD COLUMN 必紅）`／`AC-01(c) 突變自證: ALTER 型比較器具鑑別力 — 多一個 ADD COLUMN／新欄被回填／既有值被改寫／ADD COLUMN 遺漏，各自必紅`／`AC-01(b)/(c): migration.sql（table）除 Application／VoidedReportFile 外零 ALTER，零 DROP／INSERT／UPDATE／額外 ALTER TYPE`／`AC-01(d): prisma migrate deploy succeeds incrementally against an existing (pre-Phase) DB; before snapshots complete`／`AC-01(d): re-running migrate deploy against the already-upgraded existing DB is idempotent (no-op, still succeeds)`／`AC-01(h): supersedes/supersededBy and voidedFile 為零 DDL 之虛擬反向關聯 — 讀取端可 include，寫入端於 Application/Report 皆無對應實體欄`；`PHASE-009-T1 migration safety net — clean DB` › `AC-01(d): prisma migrate deploy succeeds end-to-end against a brand-new empty DB, and is idempotent on re-run` | T1 | **GREEN** |
| AC-01(b) | integration | `phase9-migration-safety.test.ts` | `AC-01(b): the new VoidedReportFile table exists and is empty (零回填) after migrating an existing DB`；`AC-01(b): VoidedReportFile 之欄位集合／型別／可空性逐欄符合 §8.2（8 欄，全數 NOT NULL）`；`AC-01(b): VoidedReportFile 之 PK、兩個唯一約束（reportId／storageKey）、FK(onDelete: Restrict) → Report(id)`；`AC-01(b): FK onDelete Restrict — deleting a Report that has a VoidedReportFile is rejected` | T1 | **GREEN** |
| AC-01(e) | integration | `phase9-migration-safety.test.ts` | `AC-01(e): AuditAction 於既有 DB 升級後含 APPLICATION_VOIDED（且僅新增此一值）`；`AC-01(e): AuditAction 恰新增 APPLICATION_VOIDED — 九個既有值 ＋ 恰一個新值，聯集全等（多一／少一必紅）`；`AC-01(e): ALTER TYPE 新值於獨立交易後方可使用 — 同一交易內使用新值必失敗；獨立交易後可用（raw SQL 自證，探針值與 APPLICATION_VOIDED 無關）` | T1 | **GREEN** |
| AC-01(f) | integration | `phase9-migration-safety.test.ts`；連動基線檔 `infra001-isolation-self-check.test.ts`／`phase5a／6／7／8-migration-safety.test.ts`（僅基線值更新，**測試名零改動**故無新實名） | `AC-01(f) 表數 16→17 — VoidedReportFile is the 17th business table`；`AC-01(f) 機械連動實查: 本 Phase 更新之基線處數與 Handoff 回報值相符`——**實查處數（T1 據實回報，與 §8.5 預估有一處差異）**：表數 16→17 **6 處**（與預估相符）、`AuditAction` enum 基線 **4 處**（預估 5——`phase5a-migration-safety.test.ts` :559 實為**存在檢查非閉集**，不需更新；T1 據實記載）、`Application` 欄位集合 **1 處**（十欄→十四欄） | T1 | **GREEN** |
| AC-01(g) | integration | `infra002-schema-drift.test.ts`（既有，本 Phase 零改動） | `INFRA-002 — schema drift guard (AC-01(h))` › `schema.prisma 與 prisma/migrations 逐一套用後之狀態完全一致（prisma migrate diff --exit-code = 0）；影子 schema 用畢即清`——**註**：該 `describe` 標題內之 `(AC-01(h))` 為 **PHASE-008** 之 AC 編號（drift guard 常設化之出處），**非**本 Phase AC-01(h)；同名不同 Phase，屬既有記載，本次零改動 | T1 | **GREEN** |
| AC-02 | unit（轉換集合）＋ integration（D8(a) 結構面） | `backend/test/unit/application-state-machine.test.ts`；`backend/test/integration/phase9-migration-safety.test.ts`（D8(a) 結構面，T1 交付） | `ALLOWED_TRANSITIONS — 本 Phase 可達轉換集合（D2, PHASE-009-T2 依 AC-02 受控擴充）` › `AC-02: ALLOWED_TRANSITIONS 恰兩條（DRAFT→COMPLETED、COMPLETED→VOIDED）`／`兩元素逐字為 [DRAFT, COMPLETED] 與 [COMPLETED, VOIDED]（全等比對，防偷加第三條或改動內容）`；`AC-02 逐字驗收 — 四種非法轉換逐一（錯誤碼分流不變）` › `AC-02: VOIDED→COMPLETED／VOIDED→DRAFT／DRAFT→VOIDED／COMPLETED→DRAFT 逐一被拒且錯誤碼分流不變`；`assertTransition — 狀態轉換矩陣（資料驅動，取自 enum 全部值）(AC-55~58)` › `DRAFT→COMPLETED 與 COMPLETED→VOIDED 是僅有的兩個不拋格子（矩陣鑑別力總結，AC-02）`；**D8(a)「零布林旗標」之結構面守門 ＝ T1 之 14 欄封閉集合斷言**（`AC-01(a): Application 之四個新欄 — 型別／可空性／無 default 逐欄符合 §8.1（14 欄封閉集合）`）——**不另造第二個較弱守門**（T1 即審 **FW-3** 裁定：欄位集合封閉即涵蓋「零 `isVoided`／`voided`」，寫入路徑列舉≠布林守門重造） | T2（轉換集合）＋ **T1**（D8(a) 結構面） | **GREEN** |
| AC-03 | 純函式（無 DB）＋ integration，**同檔** | `backend/test/integration/phase9-void.test.ts`（**單一落點**——gate table 置於該檔**頂層 `describe`**，15 個 `it` 皆為純函式、不需 DB 亦執行；`backend/src/applications/void-reason.ts` 之覆蓋即由此頂層區塊承擔，**不另立** `void-reason.test.ts`） | `AC-03: 作廢原因 gate table（12 列：缺鍵／null／空字串／半形空白／全形空白／Tab／換行／1 字元／500 字元／501 字元／前後空白 trim／XSS payload）` › `缺鍵（undefined）→ 拒絕`／`null → 拒絕`／`空字串 "" → 拒絕`／`半形空白（"   "）→ 拒絕`／`全形空白（"　　　"）→ 拒絕`／`Tab（"\t\t"）→ 拒絕`／`換行（"\n\n"）→ 拒絕`／`混合空白（全形空白＋Tab＋換行）→ 拒絕`／`恰 1 字元 → 接受`／`` 恰 ${VOID_REASON_MAX_LENGTH} 字元 → 接受 ``／`` ${VOID_REASON_MAX_LENGTH + 1} 字元 → 拒絕 ``／`前後空白（B-03："  誤植金額  "）→ 接受，value 為 trim 後之 "誤植金額"`／`XSS payload（B-04）→ 接受，原樣（未跳脫）保留於 value——跳脫是呈現責任（T11），非驗證責任`／`非字串（number）→ 拒絕`／`fields[] 恰含 { field: "reason" }（AC-03 明文）`（**共 15 個 `it`**，多於標題所稱 12 列）；`PHASE-009-T3 — voidApplication 作廢 service` › `AC-03: 400 時零寫入（status 不變、四欄仍 NULL、零稽核列）` › `空白原因 → 400 VALIDATION_ERROR，status 仍為 COMPLETED，四個作廢欄仍為 NULL`／`501 字元原因 → 400 VALIDATION_ERROR，零寫入` | T3 | **GREEN** |
| AC-04 | integration | `phase9-void.test.ts` | `AC-04: 作廢成功同交易寫入四欄；totalAmount／completedAt／三型快照欄逐欄逐位元組不變` › `差旅：成功作廢 → status=VOIDED、四欄正確寫入；totalAmount/completedAt/travel 快照 4 欄/segment 快照 4 欄逐欄不變`／`保養：成功作廢 → 五個快照欄（intervalKm/officialKm/ratio/rawAmount/calculatedAt）逐欄不變`／`折舊：成功作廢 → 九個快照欄逐欄不變`／`代操作者作廢（actorId ≠ ownerId）：voidedById 為實際操作者，非擁有人（本函式不判斷授權，僅忠實記錄呼叫端傳入之 actorId）`——**允許變動集合之封閉**由 T3R 之 `fingerprintRow` 全欄指紋法承擔（T3 即審 **SF-2**；T5pre 修復 `Date` 毫秒截斷後鑑別力再上升） | T3 | **GREEN** |
| AC-05 | **unit**（(a)）＋ integration（(b)） | **(a)**：`backend/test/unit/application-state-machine.test.ts`（T2 交付）／**(b)**：`backend/test/integration/phase9-contract.test.ts`（T4 交付） | **(a)**：`AC-05(a) 結構性守門 — backend/src 全域零 VOIDED→COMPLETED／DRAFT 寫入路徑` › `至少掃到一些 src 檔案（確保這個測試本身沒有因為路徑算錯而空跑）`／`status: COMPLETED／DRAFT 之程式碼出現位置逐一比對白名單，白名單外必紅`／`鑑別力自證（SF-3, T2R-LITE：真實 fixture 目錄實跑，非恆真式）：listStatusLiteralOccurrences 對 scratchpad fixture 目錄實際掃描，涵蓋字串字面值／enum 成員／跨行三種寫入形式，且正確排除行註解與 JSDoc`／`三筆 COMPLETED 寫入（travel／maintenance／depreciation）皆含同函式的 assertTransition(existing.status, "COMPLETED") 呼叫（VOIDED 來源必先 403，不可能執行到該 update）`；**(b)**：`AC-05(b): 已作廢申請之四類端點守門（完成 403 逐字／作廢 409／PUT 403／DELETE 403，皆零寫入）` › `完成端點 POST /applications/:id/complete → 403 FORBIDDEN，message 逐字為 ALREADY_FINAL_MESSAGE，零寫入`／`作廢端點 → 409 CONFLICT（非 403、非冪等 200）；三型 PUT → 403 ALREADY_FINAL_MESSAGE；DELETE → 403 ALREADY_FINAL_MESSAGE；皆零寫入`／`FW-9: VOIDED 相關之使用者可見 403 文案聯集恰為兩支（申請面 ALREADY_FINAL_MESSAGE ＋ 附件面 ATTACHMENT_LOCKED_MESSAGE），兩者於既有 src 皆在場且互異` | T2（(a)）＋ T4（(b)） | **GREEN** |
| AC-06 | integration | `phase9-void.test.ts`（service 層）；`phase9-contract.test.ts`（端點層） | `AC-06: 已作廢再作廢 → 409 CONFLICT + details.status="VOIDED"，零寫入零稽核（非冪等成功）` › `第二次作廢 → 409 CONFLICT，details.status=VOIDED，原 voidReason 不被覆寫`；端點層 › `409 CONFLICT + details.status="DRAFT"（AC-07 端點層）與 details.status="VOIDED"（AC-06 端點層）；訊息逐字釘樁（T3 即審 AR-4）` | T3（service）＋ T4（端點） | **GREEN** |
| AC-07 | integration | `phase9-void.test.ts`；`phase9-contract.test.ts`（側信道） | `AC-07: 草稿作廢 → 409 + details.status="DRAFT"，零寫入` › `對草稿呼叫 voidApplication → 409 CONFLICT，details.status=DRAFT`／`狀態守門優先於原因驗證：草稿 + 空白原因 → 仍為 409（狀態），非 400（原因）`；`AC-23 側信道: 他人之「草稿」「已完成」「已作廢」三種狀態回應逐字相同（授權判定早於狀態守門）` › `三種狀態之 403 回應（狀態碼＋body，requestId 除外）逐字全等，且皆零寫入`；`AC-27: 作廢端點錯誤表逐格（401／403×2／404／409+details.status／400+fields[]；existingRevisionId 與修正版各格屬 T7）` › `判定順序: 他人 × 草稿 × 空原因（三重違規）→ 403（授權），非 409（狀態）亦非 400（原因）` | T3（service）＋ T4（端點與側信道） | **GREEN** |
| **AC-08(a)**（三型 `PUT`／`DELETE` 面） | integration | `phase9-void.test.ts` | `AC-08: 已作廢之三型 PUT／DELETE 端點皆 403 且零寫入（assertApplicationMutable 之真實回歸）` › `差旅 PUT（含 purpose 與 segments[].attachmentIds 夾帶）→ 403，purpose 零改寫`／`保養 PUT → 403，actualCost 零改寫`／`折舊 PUT → 403，applicationYear 零改寫`／`FW-1 保養 PUT 夾帶 attachmentIds:[TEMP 附件] → 403，該附件仍為 TEMP（零關聯寫入）`／`FW-1 折舊 PUT 夾帶 attachmentIds:[TEMP 附件] → 403，該附件仍為 TEMP（零關聯寫入）`／`DELETE /applications/:id → 403，列不被刪除` | T3（＋ T4 之 FW-1 兩變體） | **GREEN** |
| **AC-08(b)**（附件**刪除**端點面） | integration | `backend/test/integration/phase4-attachment-ard.test.ts`（`SPEC-REV-9T7` ① 更正之落點；T4b 即審裁定該落點正確） | `AC-08(b) — VOIDED 申請之附件鎖定（三個 refType 分支）` › `TRIP_SEGMENT：已作廢申請之附件 DELETE → 403，附件仍 LINKED（零寫入）`／`MAINTENANCE：已作廢申請之附件 DELETE → 403，附件仍 LINKED（零寫入）`／`DEPRECIATION：已作廢申請之附件 DELETE → 403，附件仍 LINKED（零寫入）`。**修復前紅燈實證**：三分支各一 `expected 200 to be 403`（`deriveContainerState` 原以 `application.status === "COMPLETED" ? "completed" : "draft"` 判定，`VOIDED` 被誤判為 `draft`；最小修為 `!== "DRAFT"`，`ContainerState` 型別與呼叫端契約零變更）。**「新增」面不在本列**：`PUT …/attachmentIds` **不經** `deriveContainerState`，由 `assertApplicationMutable` 於解析 body 前守門，故歸 **AC-08(a)** | **T4b** | **GREEN** |
| AC-09 | integration | `phase9-revision.test.ts` | `PHASE-009-T5 修正版複製 service（AC-09／AC-10／AC-11）` › `AC-09: 建立修正版 → 201；新列 status=DRAFT、ownerId=原擁有人、createdById=操作者、totalAmount／completedAt／作廢三欄皆 NULL` › `差旅（管理員代建）：父列全欄封閉比對——僅六個允許欄位有值，其餘（totalAmount／completedAt／三個作廢欄）恆 NULL`／`保養（本人操作）：createdById ＝操作者本人；primaryDate 由複製後之 currentMaintenanceDate 推導`／`折舊（管理員代建）：primaryDate ＝複製後 applicationYear 之 12-31（deriveDepreciationPrimaryDate 既有規則）` | T5 | **GREEN** |
| AC-10 | unit ＋ integration | `backend/test/unit/revision-copy.test.ts`；`phase9-revision.test.ts` | **unit**：`AC-10: 三型「複製鍵集」與「禁複製鍵集」toEqual 封閉（多複製一欄必紅）— 差旅` › `Application 層鍵集恰為六鍵 ＋ travel 子鍵；五個禁複製鍵恆不出現`／`TravelApplication 子層鍵集恰為三鍵；十二個禁複製鍵恆不出現`／`TripSegment 鍵集恰為五鍵；段快照四欄恆不出現；逐段保序且值逐欄相同`／`純函式：同輸入兩次呼叫結果 toEqual（零時鐘、零隨機）`；`…— 保養` › `MaintenanceApplication 子層鍵集恰為五鍵；五個快照禁複製鍵恆不出現`；`AC-10: 折舊複製 applicationYear 與 annualTotalKm，九個快照欄與參數版本引用欄恆 NULL` › `DepreciationApplication 子層鍵集恰為兩鍵；十一個禁複製鍵（含九個 snapshot*、calculatedAt、depreciationParameterVersionId）恆不出現`；**integration（DB 列層封閉）**：`AC-10: 三型「複製鍵集」與「禁複製鍵集」toEqual 封閉（多複製一欄必紅）` › `差旅子列：tripDate／purpose 複製，十二個快照／參數版本引用欄恆 NULL（DB 列層封閉比對）`／`保養子列：五個業務欄位複製，五個快照欄恆 NULL（DB 列層封閉比對，Decimal scale 以 toFixed 保真）`／`AC-10: 差旅逐段複製且 sortOrder 保序，段快照四欄恆 NULL`／`AC-10: 折舊複製 applicationYear 與 annualTotalKm，九個快照欄與參數版本引用欄恆 NULL` | T5 | **GREEN** |
| AC-11 | integration | `phase9-revision.test.ts` | `AC-11: 原申請零改動（逐欄 ＋ 產物雙層）` › `AC-11(a): 原 Application／三型子列／TripSegment 逐欄逐位元組不變（含 updatedAt 未被 touch）`／`AC-11(b): 原附件列五欄（id／storageKey／refId／status／linkedAt）逐欄不變`／`AC-11(c): 原 Report 七欄不變且其 PDF 位元組 SHA-256 前後相同（逐位元組實證）`——**(c) 之位元組為合成非真實渲染**（T5 揭露④；真實 Chromium 之逐位元組實證由 **T9** 之 AC-15 一列承擔） | T5 | **GREEN** |
| AC-12 | integration | `phase9-revision.test.ts` | `PHASE-009-T7 修正版端點 POST /applications/:id/revision（AC-12／AC-13／AC-26）` › `AC-12: 版本關聯之保存與雙向可見（單一 supersedesId 之雙向投影）` › `AC-12(a): 端點 201 且新列 supersedesId ＝原申請 id；回應之 supersedes 非 null（§7.3）`／`AC-12(b) 原申請側: 建立前 supersededBy 為 null；建立後為 { id, status, primaryDate } 三鍵封閉`／`AC-12(b) 修正版側: supersedes 為 { id, status, primaryDate, reportNumber } 四鍵封閉——原申請有報表時為其編號，無報表時為 null`／`AC-12(c) 單一事實來源: Application 無任何反向關聯欄位（information_schema 封閉掃描），且刪除修正版後原申請之 supersededBy 自動回 null`／`AC-12: 保養與折舊兩型之端點成功與雙向投影（型別分派逐型別覆蓋）`；**(b) 之端點覆蓋面（T7b W-3）**：`PHASE-009-T7b 合約一致性（B-16 停用擁有人守門／W-3 兩鍵投影）` › `W-3: admin 三個代建端點之詳情回應含 supersedes／supersededBy 兩鍵（AC-12(b) 投影套用於每一個回傳詳情 DTO 之端點）` › `` W-3 正向（${kind}）: POST /admin/users/:userId/applications/${kind} → 201 且兩鍵**在場**、值皆 null（新草稿無版本關聯） ``（三型參數化）／`W-3 負向（鑑別性）: 兩鍵為**真投影**而非硬編 null——admin 代建之草稿掛上 supersedesId 後，詳情端點之 supersedes 非 null 且雙向皆到位` | T7（＋ **T7b** 之 W-3 端點覆蓋面） | **GREEN** |
| AC-13 | integration | `phase9-revision.test.ts` | **端點層（T7，權威）**：`AC-13: 修正版之來源守門（三種 409 逐格 ＋ (d) 零寫入、零附件複製、零 storage 寫入）` › `AC-13(a): 來源為草稿 → 409 CONFLICT ＋ details.status='DRAFT'，零寫入零 storage 寫入`／`AC-13(b): 來源為已作廢 → 409 CONFLICT ＋ details.status='VOIDED'，零寫入零 storage 寫入`／`AC-13(c): 原申請已有修正版 → 409 CONFLICT ＋ details.existingRevisionId（D7：0..1），零寫入零 storage 寫入`／`FW-6 併發雙建（T1 FW 落地）: 兩個併發 POST 恰一個 201、一個 409，且 DB 恰一筆修正版`；**service 層（T5，縱深不得取代端點層——T5 即審 FW-4）**：`T5 service 守門：非 COMPLETED 來源與既有修正版（AC-13 之端點層斷言歸 T7）` › `草稿來源 → CONFLICT ＋ details.status=DRAFT，且零新增 Application`／`已作廢來源 → CONFLICT ＋ details.status=VOIDED，且零新增 Application`／`已有修正版 → CONFLICT ＋ details.existingRevisionId，且零新增 Application（D7：0..1）`／`SF-1 結構性守門：FOR UPDATE 早於讀取與寫入、位於單一 $transaction 回呼內（行鎖移除／交易拆分之 mutant 必紅）` | T7（端點）＋ T5（service 縱深） | **GREEN** |
| AC-14 | integration | `phase9-revision-attachment.test.ts` | `PHASE-009-T6 修正版之證明附件複製（AC-14）` › `AC-14(a)(b) 新列六欄 ＋ 位元組 SHA-256 相同 ＋ key 互異 ＋ 縮圖同步複製` › `差旅：段附件複製為新列與新 storageKey（含縮圖），位元組逐位元組相同`／`保養：容器附件複製為新列與新 storageKey，位元組逐位元組相同`／`折舊：容器附件複製為新列與新 storageKey，位元組逐位元組相同`／`來源附件之列與位元組零改動（複製流程不得觸原 storageKey）`；`AC-14(c) 差旅段—圖對位複製（依 sortOrder，不跨段錯置）` › `三段（插入順序刻意與 sortOrder 升序不同、中間段零附件）逐段對位，零跨段錯置`；`AC-14(d) 附件上限於複製後仍成立` › `` 差旅：原段滿 ${SEGMENT_ATTACHMENT_LIMIT} 張（既有常數上限），複製後新段恰 ${SEGMENT_ATTACHMENT_LIMIT} 張且未超限 ``／`` 保養：原容器滿 ${MAINTENANCE_ATTACHMENT_LIMIT} 張，複製後恰 ${MAINTENANCE_ATTACHMENT_LIMIT} 張且未超限 ``（**測試一律 `import` 既有常數，不寫死字面值**）；`無附件之來源：複製零動作仍成功（零 Attachment 新增、零 storage 寫入）`（B-22）；`AC-14(e) 複製失敗 → 整筆交易回滾、零殘留、500、日誌零 storage key` › `storage put 注入失敗：DB 零新增列（Application／TripSegment／Attachment 皆零）＋ 已寫新 key 補償刪除 ＋ 500`／`失敗路徑之日誌零 storage key（掃描 ＋ 反向探針證明掃描器有鑑別力）`／`複製成功之後才失敗（onCreated 拋錯，對應 T7 稽核寫入失敗／commit 失敗之路徑）：整筆回滾、已寫新 key 補償刪除、零殘留` | T6（＋ T6R 之 onCreated seam） | **GREEN** |
| AC-15 | integration | `phase9-revision.test.ts` | `PHASE-009-T9 AC-15 修正版完成 → 新報表編號與新 PDF` › `AC-15(d) 結構: report-number.ts 本 Phase 零 diff` › `backend/src/reports/report-number.ts 之原始碼 SHA-256 等於 PHASE-009 基準 commit ac56f20 之值（改動任一位元組即紅——AC-15 不得以修改編號邏輯達成）`；`AC-15(a)(b)(c): 修正版完成後產生報表 → 新編號／新 PDF，且原報表逐位元組不變` › `(a) 201 ＋ 新 reportNumber 與原申請不同且序號為同月同型下一號；(b) storageKey 互異且 PDF 位元組 SHA-256 不同；(c) 原 Report 七欄與其 PDF 位元組 SHA-256 前後不變`——**⚠ (a)(b)(c) 三子項由「單一 `it`」承載**（真實 Chromium ×2 之全鏈實證；T9 即審 **AR-1** 明令：回填不得照抄原三條分列預定名，該三名於 repo 不存在） | T9 | **GREEN** |
| AC-16 | integration | `phase9-void-statistics.test.ts` | `AC-16 結構: backend/src/mileage 全目錄本 Phase 零 diff` › `目錄之實際 .ts 檔案清單恰為三檔（於 src/mileage/ 新增或刪除任何檔案即紅）`／`三檔之原始碼 SHA-256 逐檔等於 PHASE-009 基準 commit ac56f20 之值（改動任一位元組即紅）`／`AC-16 逐字: buildOfficialMileageWhere 之 status 為字面 "COMPLETED"，原始碼中不存在 { not: "DRAFT" } 之否定式過濾`；`PHASE-009-T9 — 統計排除之真實作廢流程回歸（AC-16／AC-17）` › `AC-16: 經真實作廢端點作廢後，sumOfficialMileage 恰等於未含該筆之值` › `三筆已完成差旅 → 作廢中間一筆（真實端點）→ 總和恰等於另兩筆之和、筆數恰減 1；且 { not: "DRAFT" } mutant 條件於同一組資料下必得不同值（鑑別力正向對照）` | T9 | **GREEN** |
| AC-17 | integration | `phase9-void-statistics.test.ts` | `AC-17(a): 保養草稿之 computed.officialKm 於區間內某差旅被真實作廢後下降對應值` › `期間內兩筆已完成差旅（100.00 ＋ 60.00）→ 草稿 officialKm 160.00；作廢其中 60.00 那筆 → 恰降為 100.00`；`AC-17(b): 折舊草稿之年度公務里程於年度內某差旅被真實作廢後下降對應值` › `年度內兩筆已完成差旅（300.00 ＋ 200.00）→ 草稿 officialKm 500.00；作廢其中 200.00 那筆 → 恰降為 300.00`；`AC-17(c): 已完成之保養／折舊快照於作廢後不變（快照即歷史）` › `同一次作廢事件下：已完成側之 snapshot.officialKm 與整列指紋逐欄不變，草稿側（重算路徑）同步下降 —— 已完成側若改為重算則必紅` | T9 | **GREEN** |
| AC-18 | integration | `phase9-reference-guard.test.ts` | `PHASE-009-T8 — 參數引用保護之作廢語意（AC-18 / D2(a)）` › `AC-18(a): 五型別逐一——引用來源被作廢後仍判定為已被引用（COMPLETED-only mutant 必紅）`／`AC-18(b): 僅被草稿引用者仍判定為未被引用`／`AC-18(c): 既有 COMPLETED 正向判定逐型別零弱化`——**結案限定（T8 即審 AR-4）**：函式語意已結案，**使用者可達之參數覆寫端點尚不存在**，D2(a) 之實際保存效果待 PHASE-010／011 之覆寫端點落地時兌現（該端點必須呼叫本守門，已移交） | T8 | **GREEN** |
| AC-19 | integration | `phase9-void-report.test.ts` | `PHASE-009-T10 §A — report-data.ts 結構性掃描（AC-19(d)／AC-39(a)）` › `AC-19(d): report-data.ts 原始碼零 .times(/.div(/.plus(/.minus(（PHASE-008 AC-17(c) 之既有掃描於本 Phase 擴欄後維持全綠）`；`PHASE-009-T10 §C — buildReportData 之作廢欄位與排序（AC-19／AC-39）` › `AC-19(a): ReportCommon 鍵集恰八鍵（toEqual 全等，多一鍵必紅）` › `三型 × 作廢／未作廢共六種組合之 common 鍵集皆恰等於八鍵清單`／`void 物件本身亦為封閉三鍵（reason／at／byDisplayName；多一鍵如 voidedById 必紅）`／`未作廢申請之 void 恆為 null（負向；三型各一）`／`已作廢申請之 statusLabel 為「已作廢」且 void 三欄逐值正確（真實端點產生）`；`AC-19(b): 三型 body 鍵集逐字不變（作廢資訊只落於 common）` › `三型之作廢／未作廢兩態，body 鍵集皆逐字等於 PHASE-008 之既有清單`／`已作廢三型之 body 序列化零作廢字面（reason／void 鍵／作廢時間皆不出現於 body）`；`AC-19(c): byDisplayName 由 voidedById 解析；使用者不存在時為「—」且不洩漏 id` › `正常態：byDisplayName 為作廢操作者之顯示名稱`／`孤兒態：voidedById 指向不存在之使用者 → byDisplayName 為「—」，不拋錯`／`兩態之完整輸出皆零 voidedById（鍵名與值字面雙向負向）` | T10 | **GREEN** |
| AC-20 | unit | `backend/test/unit/report-html.test.ts` | `AC-20(a) — 作廢標示為專屬區塊且位於文件首屏（renderCommonHeader 之前）` › `` `.void-banner` 專屬區塊在場且掛 avoid-break ``／`` 逐字 `已作廢` 出現於 `.void-banner` 區塊之內（非僅出現在別處） ``／`` `.void-banner` 位於 `renderCommonHeader` 輸出（<header class="report-header">）之前 ``／`三型皆於 header 之前呈現作廢區塊`；`AC-20(b) — 作廢原因／操作者／時間三項逐字在場（時間為台北時區中文格式）` › `作廢原因逐字（標籤 ＋ 值）`／`作廢操作者逐字（標籤 ＋ 值）`／`作廢時間為 formatTaipeiDateTime 之輸出（2026-08-06T09:05:00.000Z → 2026/8/6 下午5:05），原始 ISO 字面不在場`；`AC-20(c) — 未作廢申請之列印版零 .void-banner、零「已作廢」字樣（三型抽驗）` › `差旅／保養／折舊（void: null）皆零作廢痕跡`／`正向對照：同一 fixture 加上 void 後兩者皆在場（避免上述負向斷言恆真）`；`AC-20(d) — 作廢原因經 escapeHtml 跳脫（3 XSS payload 逐一）` › `` payload=${JSON.stringify(payload)} — 作廢原因欄跳脫後在場（字面期望值）、原始 payload 不在場 ``／`` payload=${JSON.stringify(payload)} — 作廢原因欄之 value 區段零裸 " ' < >（屬性／標籤逃逸負向） ``／`` payload=${JSON.stringify(payload)} — 作廢操作者欄同一防線（字面期望值） ``（**共 6 格**——T11 W-3 將 `byDisplayName` 一併納入；**期望值為字面硬編碼**，T11R 修正原「以 `escapeHtml` 自算」之自我抵銷缺陷）；`AC-20(e) — 已作廢輸入之 renderReportHtml 仍為純函式` › `三型已作廢 fixture 同輸入兩次呼叫 toEqual`／`兩個各自獨立建構、內容相同之已作廢輸入，結果亦 toEqual`；`AC-20(f) — .void-banner 之分頁與列印媒體可見性` › `` `.void-banner` 掛 avoid-break class（沿既有 break-inside: avoid 規則） ``／`` `@media print` 區塊內零 .void-banner 之 display: none（列印媒體與 PDF 皆可見） ``／`` 全文任何 `.void-banner` 樣式規則皆不含 display: none `` | T11（＋ T11R 之字面期望值加固） | **GREEN** |
| AC-21 | integration | `phase9-void-report.test.ts` | **(f) ＋ 兩下游約束（T12a §D-0）**：`PHASE-009-T12a §D-0 — 結構性斷言（AC-21(f) ＋ SPEC-REV-9T12 兩下游約束）` › `AC-21(f): report-service.ts／reports/routes.ts 原始碼零 .report.update(／.delete(／.upsert(／.deleteMany(／.updateMany(（PHASE-008 既有斷言於作廢版落地後零改動且全綠）`／`SPEC-REV-9T12 下游約束 #1：作廢版渲染碼整段位於既有 §9.1 產生流程之後——renderReportHtml( 之首現位置仍落在產生交易內（phase8-report-generate :225-247 之三段序不變）`／`SPEC-REV-9T12 下游約束 #2：本 Task 零新增 backend/src/reports/*.ts（PHASE_008_SRC_FILES 九檔清單零觸）`；**(a)(c)(e)（T12a §D）**：`PHASE-009-T12a §D — 作廢版 PDF 之產生與保存（AC-21(a)(c)(e)）` › `AC-21(a) 正向: 作廢已產生報表之申請 → 200 ＋ 恰一列 VoidedReportFile（逐欄齊）＋ 讀回位元組之長度與 SHA-256 與該列相符`／`AC-21(a) 作廢標示可證面: 餵給 renderPdf 之 HTML（作廢版 PDF 之產生輸入）逐字含「已作廢」＋作廢原因，且 ReportData 之 common 帶正確作廢資訊與原編號`／`AC-21(c): 原 Report 列逐欄不變、其 storageKey 之位元組 SHA-256 前後不變（作廢成功路徑）`／`AC-21(e): 未產生報表之申請作廢 → 200、零渲染（renderReportHtml／renderPdf 零呼叫）、零 storage 寫入、零 VoidedReportFile`／`AC-21(e) 結構性證人: 未產生報表時 prepareVoidedReport 回 null，且**兩個 storage 皆為呼叫即拋錯之替身**仍不拋錯（零 storage 觸碰非靠條件式運氣）`／`AC-21(a) 失敗側：四階段失敗注入各自整筆回滾、零殘留、stage 逐字` › `① RENDER（renderReportHtml 拋錯）→ ReportGenerationError(stage=RENDER) ＋ 整筆回滾 ＋ 零殘留`／`① RENDER（renderPdf 拋錯／逾時）→ stage=RENDER ＋ 整筆回滾 ＋ 零殘留`／`② STORE（put 拋錯）→ stage=STORE ＋ 整筆回滾 ＋ 零殘留`／`③ VERIFY（讀回位元組被竄改：同長度、單一位元組不同）→ stage=VERIFY ＋ 已 put 之 key 補償刪除 ＋ 整筆回滾（校驗略過與補償停用兩個 mutant 之共同自證點）`／`③ VERIFY（讀回位元組長度不符）→ stage=VERIFY ＋ 零殘留（位元組數比對之獨立自證）`／`④ PERSIST（reportId 唯一約束衝突）→ stage=PERSIST ＋ 整筆回滾 ＋ 零殘留`；`AC-21(a) wire 層: 作廢版 PDF 產生失敗 → 500 REPORT_GENERATION_FAILED ＋ details.stage 逐字 ＋ 整筆回滾（狀態仍 COMPLETED、零稽核列、storage 零殘留）`／`PDF 步驟成功、其後稽核寫入於 DB 層失敗（**經真實端點** ＋ 觸發器注入）→ 500 ＋ 整筆回滾：零 VoidedReportFile（INSERT 移出交易之 mutant 必紅）＋ **端點之外層補償**使 storage 零殘留（刪除 routes.ts 之 plan.compensate() 之 mutant 必紅）`（T12aR **MF-1** 修復後之形狀）／`交易 timeout 裕度: 交易內渲染耗時 6000ms（逼近並超過 Prisma 套件預設之 5000ms 交易 timeout）→ 仍 200 ＋ 恰一列 VoidedReportFile（移除 application-void.ts 之顯式 timeout／maxWait 之 mutant 必紅）`（T12aR **SF-2**）／`B-08 併發回歸: 同一筆已產生報表之申請雙作廢 → 恰一成功（200）一衝突（409），恰一列 VoidedReportFile，storage 恰增一個 key`／`§真實 Chromium 端到端（renderPdf 未覆寫）` › `AC-21(a) 端到端: 真實 Chromium 產出之作廢版 PDF 通過 %PDF-／%%EOF／長度校驗，讀回 SHA-256 與 VoidedReportFile 相符，且與原 PDF 位元組互異（T11 FW③ 之預期差異）`；**(b)(d)（T12b §E）**：`PHASE-009-T12b §E — 作廢後之下載語意（AC-21(b)(d)）` › `AC-21(b): 作廢後下載 → 200 ＋ 回作廢版位元組（SHA-256 ＝ VoidedReportFile.contentHash，且**不等於**原檔）`／`AC-21(b): Content-Disposition 之報表編號與檔名作廢前後逐字不變（storageKey 之 void 後綴不得外洩為檔名）`／`AC-21(d): 作廢後重複下載位元組全等，且下載期間 renderPdf／renderReportHtml 零呼叫（PHASE-008 AC-08 紀律於作廢後仍成立）`／`fallback（AR-1／FW-3）: VOIDED 但無 VoidedReportFile（直造資料）→ 200 回**原檔**位元組，非 404／500`／`回歸: 未作廢（COMPLETED）申請之下載行為零變更——200 回原檔、重複下載全等、渲染器零呼叫；未產生報表仍 404（B-27）` | **T12a**（(a)(c)(e)(f)）＋ **T12b**（(b)(d)） | **GREEN**（兩側皆綠且各自 Review 清零，故整條轉綠——符合「不得單側綠時整條標 GREEN」之原意） |
| AC-22 | integration | `phase8-report-print.test.ts`（既有檔，T13 只增不改） | `§K AC-22（D5）：列印端點對已作廢申請放行 ＋ VOIDED 之授權矩陣重驗` › `§K1-a 已作廢（且已產生報表）→ 200 text/html，.void-banner 區塊內含 已作廢／原因／操作者／時間`／`§K1-b 已作廢但**未產生報表**（AC-21(e) 純 DB 作廢）→ 仍 200，含作廢標示且報表編號呈「尚未產生」`／`§K1-c 負向對照：同型**未作廢**（COMPLETED）申請之列印版零 .void-banner、零「已作廢」字樣`／`§K1-d 草稿仍 409 CONFLICT + details.status='DRAFT'（放行 VOIDED 後之複核；§D 既有斷言零弱化）`；`§K2 授權五格於 VOIDED 逐格重驗` › `格1 未登入 → 401 UNAUTHORIZED，鍵集恰 {code,message,requestId}，零洩漏（含作廢原因），非 text/html`／`格2 需強制改密 → 403 PASSWORD_CHANGE_REQUIRED，鍵集，零洩漏（含作廢原因）`／`格3 本人（擁有人）→ 200 text/html，含報表編號、擁有人姓名與作廢標示`／`格4 他人（一般使用者）→ 403 FORBIDDEN，鍵集，零洩漏，且與**同一身分對 COMPLETED** 之回應指紋逐字相同（不因作廢而洩漏狀態）`／`格5 管理員 → 200 text/html，含作廢標示（可見面不擴張，與擁有人視角同一份列印版）`；`§K3 XSS wire 探針（HTTP body 層）＋ Content-Type charset` › `` ${testCase.label}：作廢原因原樣入庫，列印回應之 body 為跳脫後字面，原文不在場 ``（三 payload 參數化）；`§K4 routes.ts 列印端點：狀態守門為 COMPLETED ∪ VOIDED 白名單字面，且零 DRAFT 黑名單字面`；併 `§J 結構性守門：授權判定早於狀態判定（產生／列印兩端點；順序對調 mutant 必紅）`（既有字面連動零弱化） | T13 | **GREEN** |
| AC-23 | integration | `phase9-contract.test.ts`；`phase8-report-print.test.ts`（列印格） | **作廢端點 5 格（T4）**：`AC-23: 授權矩陣 5 格逐格（POST /applications/:id/void × 5 身分；修正版端點 5 格屬 T7）` › `§6.1 #1 未登入 → 401 UNAUTHORIZED（判定先於存在性——以不存在之 id 呼叫亦為 401）`／`§6.1 #2 已登入但 mustChangePassword → 403 PASSWORD_CHANGE_REQUIRED（先於存在性判定）`／`§6.1 #3 擁有人本人 → 200，回應為型別分派之詳情 DTO 且 void 非 null`／`§6.1 #4 他人一般使用者 → 403 FORBIDDEN，回應零業務值（狀態／型別／金額／姓名／原因皆不外洩）`／`§6.1 #5 管理員（非擁有人）→ 200（AD-US-10 明文授權）`；**修正版端點 5 格（T7）**：`AC-23: 授權矩陣 5 格逐格（POST /applications/:id/revision × 5 身分）` › `§6.1 #6 未登入 → 401 UNAUTHORIZED（判定先於存在性——以不存在之 id 呼叫亦為 401）`／`§6.1 #7 已登入但 mustChangePassword → 403 PASSWORD_CHANGE_REQUIRED（先於存在性判定）`／`§6.1 #8 擁有人本人 → 201，回應為新草稿之型別分派 DTO 且 supersedes 非 null`／`§6.1 #9 他人一般使用者 → 403 FORBIDDEN，回應零業務值（狀態／型別／金額／姓名／行程目的皆不外洩），零寫入`／`§6.1 #10 管理員（非擁有人）→ 201，新草稿 ownerId ＝原擁有人（AD-US-09①）`；**報表四端點於 VOIDED 之回歸四格（四格全滿）**：`AC-23 回歸四格: 報表四端點於 VOIDED 之授權判定與 COMPLETED 逐字相同` › `未登入／強制改密／他人 三種身分 × 四端點：VOIDED 與 COMPLETED 之回應指紋逐格全等（12 組比對）`／`擁有人視角: 產生 → 409 + details.status='VOIDED'（B-27／D13）；查詢 → 200 report:null；下載 → 404（尚未產生，B-28 語意不變）`／`擁有人／管理員視角: 已作廢申請列印 → 200 text/html ＋ .void-banner 作廢標示（依 §16 D5；T4 開列之 T13 留白補齊）`／**`擁有人／管理員視角: 已作廢**且有報表**之申請下載 → 200（依 §16 D4(b1)；T4 即審 FW-A 之留白補齊）`**（**FW-A 正式閉環**）／`他人視角 × VOIDED【且有報表】: 四端點之回應指紋與同型 COMPLETED【且有報表】逐格全等（T12b 即審 AR-2 之回歸網固化）`；併側信道 `AC-23 側信道: 他人之「草稿」「已完成」「已作廢」三種狀態回應逐字相同（授權判定早於狀態守門）`（作廢／修正版兩端點各一組） | T4（作廢 5 格）＋ T7（修正版 5 格）＋ **T12b**（下載格）＋ T13（列印格） | **GREEN** |
| AC-24 | integration | `phase9-void.test.ts`；`phase9-contract.test.ts`（端點層同交易自證） | `AC-24／AC-26: 作廢稽核（同交易）` › `AC-24: 本人經端點作廢 → 恰一筆 AuditLog(APPLICATION_VOIDED)，action／actorId／targetId／targetLabel／summary 逐鍵`／`AC-24 同交易自證: 稽核 hook 拋錯 → 四欄寫入一併回滾（零孤兒稽核列、狀態未變）`／`AC-24: 本人作廢亦寫稽核（正向斷言——與 PHASE-004 AC-86「本人自建草稿不寫稽核」刻意不同）`；**端點層之真實同交易自證（§G DB 觸發器注入；T4 即審裁定手法可接受）**：`§7.5 500 INTERNAL_ERROR ＋ AC-24 同交易之端點層自證（稽核寫入失敗 → 整筆回滾）` › `稽核 INSERT 於 DB 層失敗 → 500 INTERNAL_ERROR，狀態仍 COMPLETED、四欄仍 NULL、零稽核列（稽核移出交易之 mutant 必紅）` | T3（service hook）＋ T4（稽核與端點層自證） | **GREEN** |
| AC-25 | integration | `phase9-contract.test.ts` | `PHASE-009-T4 — AC-25 稽核與日誌零敏感資料（作廢原因零入日誌 ＋ 反向探針）` › `AC-25: 作廢原因入稽核（正向）——AuditLog.summary.reason 逐字為使用者輸入之原因`／`AC-25: AuditLog.summary 零敏感資料（密碼／cookie 值／att-rpt storage key 前綴／volume 絕對路徑／折舊車價／折舊年限）`／`AC-25: 作廢原因零出現於 request.log 任何一行（200／409／403／400 四條路徑皆已走過）`／`AC-25: 日誌流零其餘六類敏感值（密碼／session cookie 值／att-rpt 前綴／兩個 volume 絕對路徑／折舊車價／折舊年限）`／`AC-25 反向探針: 刻意將同一組敏感字串寫入同一 logStream，上述掃描全部能偵測（證明掃描機制非恆真）` | T4（＋ T18 之掃描窗擴充，見 AC-28） | **GREEN** |
| AC-26 | integration | `phase9-void.test.ts`（(a)(d)）；`phase9-revision.test.ts`（(b)(c)） | `AC-26(a): 管理員代作廢 → actorId=管理員、targetId=擁有人（actorId ≠ targetId）`；`AC-26(d): 管理員作廢自己的申請 → 仍寫 APPLICATION_VOIDED（作廢不受 AC-86 本人豁免影響）`；`AC-26: 代操作稽核（(b) 管理員代建沿用既有 enum；(c) 本人零稽核列）` › `AC-26(b): 管理員代建 → 恰一筆 APPLICATION_CREATED_ON_BEHALF，actorId＝管理員、targetId＝擁有人、summary 三鍵封閉 { applicationId, type, revisionOf }`／`AC-26(b) 不新增第二個 enum 值: AuditAction 之十值集合逐字不變（修正版稽核沿用既有 APPLICATION_CREATED_ON_BEHALF）`／`AC-26(c): 本人建立修正版 → 零稽核列（沿 PHASE-004 AC-86 之 userId !== actorId 判定；hook 對本人也寫之 mutant 必紅）`／`AC-26(c) C3 邊界: 管理員對**自己擁有**之申請建立修正版 → 201 且零稽核列（判定基準為 actorId !== ownerId，非 role === 'ADMIN'——角色判定之 mutant 必紅）`（T7R 補；封閉性三向推演）；併 `§7.5 500 INTERNAL_ERROR ＋ AC-26(b) 同交易之端點層自證（代操作稽核寫入失敗 → 整筆回滾）` › `稽核 INSERT 於 DB 層失敗 → 500 INTERNAL_ERROR，零修正版列、零稽核列、零附件複製（稽核移出交易之 mutant 必紅）` | T4（(a)(d)）＋ T7／T7R（(b)(c)） | **GREEN** |
| AC-27 | integration | `phase9-contract.test.ts` | `AC-27 結構性 — ErrorCode 聯集全等（本 Phase 零新增碼）＋ applications/ 僅用基線字面值` › `AC-27: errors.ts 之 ErrorCode 聯集與 PHASE-008 結案基線逐字相同（多一碼／少一碼／BOGUS mutant 必紅）`／`AC-27: 本 Phase 掃描面五檔（作廢三檔 ＋ 修正版兩檔）僅使用基線 ErrorCode 字面值`／`AC-27 清單自證: PHASE_009_VOID_SRC_FILES 之五檔皆為實際成員，且作廢面／修正版面之檔名族群無遺漏（新增本 Phase src 檔而未同步清單即紅）`（T4 即審 **FW-D** 之清單腐化缺口由 T7 擴列關閉）；**兩端點錯誤表逐格**：`AC-27: 作廢端點錯誤表逐格（401／403×2／404／409+details.status／400+fields[]；existingRevisionId 與修正版各格屬 T7）` › `404 NOT_FOUND: 擁有人對不存在之 id 作廢（不帶 details，不洩漏型別；B-13）`／`409 CONFLICT + details.status="DRAFT"（AC-07 端點層）與 details.status="VOIDED"（AC-06 端點層）；訊息逐字釘樁（T3 即審 AR-4）`／`400 VALIDATION_ERROR + fields[] 恰含 { field: "reason" }（缺鍵／空字串／僅空白三例），零寫入`／`判定順序: 他人 × 草稿 × 空原因（三重違規）→ 403（授權），非 409（狀態）亦非 400（原因）`；`AC-27: 修正版端點錯誤表逐格（404／409+details.status×2／409+details.existingRevisionId／400 擁有人已停用；body 驗證之 400 不適用）` › `404 NOT_FOUND: 擁有人對不存在之 id 建修正版（不帶 details，不洩漏型別；B-13）`／`409 CONFLICT + details.status="DRAFT"／"VOIDED"（AC-13(a)(b) 端點層），訊息逐字釘樁`／`409 CONFLICT + details.existingRevisionId（§7.5「已有修正版」列；T4 留白清單第 2 點）`／`判定順序: 他人 × 草稿（雙重違規）→ 403（授權），非 409（狀態）`；併 `§7.3: 修正版端點之 body 一律忽略（夾帶任意鍵仍 201，且不影響新列欄位）——故 §7.5 之 **body 驗證** 400 一格對本端點不適用` | T4 ＋ T7 | **GREEN** |
| **AC-27／B-16**（修正版端點之停用擁有人守門；`SPEC-REV-9T7` ⑧ 補列） | integration | `phase9-revision.test.ts` | `PHASE-009-T7b 合約一致性（B-16 停用擁有人守門／W-3 兩鍵投影）` › `B-16: 管理員對已停用使用者建修正版 → 400 VALIDATION_ERROR「指定的使用者已停用」，零寫入` › `B-16: 400 ＋ code／message／fields[] 逐字（沿 B-26 先例）＋ 零寫入（Application 計數不變、零修正版列、零稽核列）`／`B-16 正向對照: 同一管理員對**仍啟用**擁有人之相同形狀來源 → 201（證明 400 非恆真、非管理員身分本身被擋）`／`B-16 重新啟用對照: 同一來源於擁有人重新啟用後 → 201（守門讀的是當下 isActive，非建立時狀態）`／`B-16 判定順序: 停用守門**晚於** 404（不存在之 id）與 403（他人），**早於** 409（狀態）`；`B-15 對照: 管理員對已停用使用者之已完成申請作廢 → 200（作廢為資料修正，不受帳號狀態限制）` › `B-15: 200 ＋ 狀態轉 VOIDED ＋ 三個作廢欄落地（與 B-16 同一擁有人、同一管理員，僅端點不同）`；併 `phase9-contract.test.ts` › `400 VALIDATION_ERROR + fields[] 恰含 { field: "userId" }（B-16 擁有人已停用；error 鍵集封閉、零 details、零洩漏）`（**`fields[].field` 為 `userId` 而非 `ownerId`**——沿代建立草稿之 B-26 逐字先例，T7b **W2** 裁定維持；改名屬使用者可見行為變更，列 Gate 告知候選） | **T7b** | **GREEN** |
| AC-28 | integration | `phase9-contract.test.ts` | `AC-28 掃描窗涵蓋性: 作廢四路徑 ＋ 修正版／附件複製 ＋ 列印(VOIDED) ＋ 上傳補償三條路徑皆已進入同一 logStream（防掃描窗萎縮成恆真）`（T18 擴充後之七條路徑）；`AC-28: 每條 error 級（level 50）日誌行皆含 requestId 可追查`；併反向探針 `AC-25 反向探針: 刻意將同一組敏感字串寫入同一 logStream，上述掃描全部能偵測（證明掃描機制非恆真）` | T18（掃描窗擴充；基礎掃描由 T4 交付） | **GREEN** |
| AC-29 | frontend | `frontend/test/VoidApplicationDialog.test.tsx`（(b)~(f)）；三型頁測試檔（(a) 入口） | `VoidApplicationDialog` › `AC-29(b): 渲染 .dialog-overlay 二次確認對話框，含作廢原因輸入欄與確認／取消兩鈕，且掛載時零請求`／`AC-29(c): 原因為空或僅空白 → 確認鈕停用且零 POST（前端第一道）` › `` ${label} → 確認鈕停用，點擊後零 POST ``（六格參數化，含全形空白）／`輸入非空白原因後確認鈕啟用；再清空 → 重新停用`／`AC-29(f): 取消時呼叫 onCancel，且零請求（即使已輸入原因）`／`AC-33: 作廢側五態` › `Loading（AC-29(d)）：送出中確認鈕顯示「作廢中…」且停用、取消鈕亦停用；重複點擊僅一次 POST`／`` Error 400：逐字顯示 error.message 與 fields[] 之 reason（field: "reason"），零 onVoided，且可修正後重送 ``／`Error 409：顯示狀態衝突訊息並提供「重新載入」，點擊呼叫 onReload 且不再送出 POST`；**(a) 入口（T15，三型各一）**：`作廢（PHASE-009-T15）` › `AC-29(a)：COMPLETED 詳情頁有作廢入口，點擊開啟確認對話框；取消零請求`（`TravelApplicationPage.test.tsx`／`MaintenanceApplicationPage.test.tsx`／`DepreciationApplicationPage.test.tsx`）——**`SPEC-REV-9DOCA` owner 補正**：原記 Task 僅 T14，惟 (a) 為**頁面層**入口，實由 **T15** 交付（同 AC-33 之 T14＋T15 切分） | T14（對話框）＋ **T15**（(a) 入口） | **GREEN** |
| AC-30 | frontend | 三型頁測試檔（(c)）；`frontend/test/ApplicationListSection.test.tsx`（(a)(b)） | `作廢（PHASE-009-T15）` › `AC-30(c)：VOIDED 詳情頁逐字顯示作廢原因／作廢操作者／作廢時間`（三型各一）；`已作廢（PHASE-009-T15b）` › `AC-30(a)：列表該筆狀態欄逐字顯示「已作廢」`／`AC-30(b)：狀態篩選選「已作廢」→ 請求帶 status=VOIDED 且可查得該筆`——**(b) 為 T15b 補之真缺件**（篩選下拉原無「已作廢」選項；T15 即審 **M-1**；`STATUS_LABELS` 退化 mutant 2 紅證綁列設計有效） | T15 ＋ **T15b**（(b) 缺件補做） | **GREEN** |
| AC-31 | frontend | 三型頁測試檔（(a)(b)(c)）；`frontend/test/ReportSection.test.tsx`（(d)） | `作廢（PHASE-009-T15）` › `AC-31(a)：VOIDED 走唯讀分支，不再渲染草稿編輯表單`／`AC-31(b)(c)：VOIDED 唯讀頁零五類按鈕、零可恢復為已完成之控制項`（兩者三型各一；**(a) 為現況缺陷之修復**——紅燈直接展示 VOIDED 渲染草稿表單）；`ReportSection` › `AC-31(d): VOIDED 放行（PHASE-009-T14）` › `VOIDED 且已產生報表：顯示報表編號與「檢視列印版」「下載」兩入口，且**零**「產生正式報表」按鈕`／`VOIDED 且未產生報表：顯示「未產生正式報表」說明文字，且零產生按鈕、零入口`／`COMPLETED 之下載文案不得帶作廢版字樣（零弱化對照：兩態文案不互相污染）` | T14（(d)）＋ T15（(a)(b)(c)） | **GREEN** |
| AC-32 | frontend | 三型頁測試檔 | `修正版（PHASE-009-T16）` › `AC-32(a)：COMPLETED 詳情頁出現「建立修正版」按鈕，且佔位文案逐字不在場`／`AC-32(a) 守門：VOIDED 唯讀頁零「建立修正版」按鈕（T15 負向相容）`／`AC-32(b)／AC-33 Success：點擊建立修正版 → POST /revision（零 body）→ 導向新草稿頁`／`AC-32(b)：修正版端點不得宣告 Content-Type（R8 防線：無 body 不宣告 body 型別）`／`AC-32(c) 原申請側：已有修正版時顯示「已建立修正版」與指向修正版之連結`／`AC-32(c) 負向：無任何版本關聯時零版本關係區塊`／`AC-32(c)：原申請未產生報表（reportNumber 為 null）時以原日期呈現`／`AC-32(d)：COMPLETED 附件區提示逐字含「如需修正附件，請建立修正版」；VOIDED 零該提示`（保養／折舊為「保養證明區」／「折舊證明區」用語）／`AC-32(e)：管理員代操作之申請（onBehalf）入口與文案一致`；**提醒文案面（T16R／T16R2；D15 之人類批准前提）**：`AC-32(c)／D15：原申請已有修正版時，版本關係區塊含「重複計入」提醒文案（逐字）`（三型各一正向）／`AC-32(c)／D15 負向：已完成但未建立修正版時零提醒文案（防恆真）`／`AC-32(c)／D15 負向：修正版側（supersedes）不加此提醒——作廢落點在原申請頁`／`AC-32(c)／D15 負向（T16R2 S-2）：VOIDED＋已有修正版 → 零提醒文案，但版本關係區塊其餘內容仍在場`（T16R 自身引入之 S-2 缺陷之閉環） | **T16**（(a)(b)／(c) 連結面／(d)(e)）＋ **T16R**（(c) 提醒文案面）＋ **T16R2**（S-2 之 `!voided` 守門） | **GREEN** |
| **AC-32／§7.2 列表 DTO `isRevision`**（`SPEC-REV-9T7` ⑨ 補列） | integration（後端鍵）＋ frontend（徽章） | `phase9-contract.test.ts` **§M**；`frontend/test/ApplicationListSection.test.tsx` | `` PHASE-009-T16pre — 列表 DTO `isRevision`（§7.2）＋ 列表鍵集基線 `` › `§7.2 正向: 修正版列之 isRevision 為 true（supersedesId 非 null；投影改恆 false 之 mutant 必紅）`／`§7.2 負向: 一般申請（未經修正版端點建立）之 isRevision 為 false，且鍵存在（非 undefined）`／`§7.2 負向（狀態非權威）: DRAFT 但非修正版之申請仍為 false（誤用 status 判定之 mutant 必紅）`／`§7.2 鍵集基線: 列表項鍵集全等於 15 鍵宣告（多一鍵／少一鍵／改名必紅）`；`修正版徽章（PHASE-009-T16）` › `isRevision 為 true 之列顯示「修正版」徽章`／`isRevision 為 false 之列零「修正版」徽章（徽章恆顯示之 mutant 必紅）`／`混合列表：僅修正版該列有徽章（逐列綁定，非全表掃描）`——**鍵數勘誤（`SPEC-REV-9DOCA`）**：本列原記「列表 DTO 鍵集 17→18」為記載失準，**實查 14→15**（T16pre **W-1**；前端獨立副本交叉印證）；**本基線為 T16pre 首次開列**（W-2：此前全 repo 無任何列表鍵集封閉斷言），已補為 §8.5 **第七類**機械連動；徽章**不觸** `supersedesId`（遵 T16pre FW-2，避免擴大列表揭露面） | **T16pre**（後端投影 ＋ 鍵集基線）＋ **T16**（前端徽章）——**`SPEC-REV-9DOCA` owner 補正**：原記單一 T16，實際依 T15 即審裁定另拆 `T16pre` 小型 BE Task 承接後端面 | **GREEN** |
| AC-33 | frontend | `VoidApplicationDialog.test.tsx`（對話框層）；三型頁測試檔（**頁面層 Empty／Success**） | `AC-33: 作廢側五態` › `Empty：初始態零錯誤區塊、零送出中文案、原因欄為空`／`Loading（AC-29(d)）：送出中確認鈕顯示「作廢中…」且停用、取消鈕亦停用；重複點擊僅一次 POST`／`Success：200 → 以回應之 application 呼叫 onVoided 恰一次，POST 送出 { reason }`／`` Error 400：逐字顯示 error.message 與 fields[] 之 reason（field: "reason"），零 onVoided，且可修正後重送 ``／`Error 409：顯示狀態衝突訊息並提供「重新載入」，點擊呼叫 onReload 且不再送出 POST`／`Error 500：顯示錯誤訊息，零 onVoided，確認鈕回復可用（可重試）`／`Permission denied：403 FORBIDDEN → 顯示無權限訊息，且零後續請求（原因欄與確認鈕皆不在場）`；**頁面層兩子態（T15；對話框層代理非等價——T14 即審 SF-1）**：`AC-33 Empty：COMPLETED（未作廢）詳情頁零作廢資訊區塊`／`AC-33 Success：作廢成功後頁面轉唯讀、顯示三項作廢資訊，且報表區狀態變 VOIDED`（三型各一；FW-1「`status` prop 必須真變 `VOIDED`」已落地並經 mutant ③ 釘樁）；**修正版側 Success／Error（T16）**：`AC-32(b)／AC-33 Success：點擊建立修正版 → POST /revision（零 body）→ 導向新草稿頁`／`AC-33 Error：409 已有修正版 → 顯示訊息並提供指向既有修正版之連結，且零導向`／`AC-33 Error：400 逐字顯示 error.message（T7b AR-2：fields[].field=userId 對不到輸入欄，不得靜默吞錯）` | T14（對話框五態）＋ **T15**（頁面層 Empty／Success）＋ **T16**（修正版側 Success／Error）——**`SPEC-REV-9DOCA` owner 補正**：原記單一 T14（T14 即審 **SF-1** 已裁定為終審假閉環風險） | **GREEN** |
| AC-34 | e2e | `e2e/void-application.spec.ts`；`e2e/revision.spec.ts`（**兩檔兩測試**——T17 **W-2** 四面跨檔拆分裁定） | `作廢端到端 Gate — PHASE-009-T17（AC-35／AC-34）` › `AC-34: 375px——作廢確認對話框／作廢資訊區塊／已作廢列印版無水平溢位`；`修正版端到端 Gate — PHASE-009-T17（AC-36／AC-34）` › `AC-34: 375px——版本關係區塊（原申請側／修正版側）無水平溢位`——**四個受測面依語境分落兩檔**（作廢面三項 ＋ 版本關係面一項），非單一測試 | T17 | **GREEN** |
| AC-35 | e2e | `e2e/void-application.spec.ts` | `作廢端到端 Gate — PHASE-009-T17（AC-35／AC-34）` › `AC-35: 差旅端到端（完成→產生報表→作廢→列表已作廢→詳情三項→列印版標示→下載作廢版→區間里程統計排除）`／`AC-35: 保養端到端（完成→產生報表→作廢→列表已作廢→詳情三項→列印版標示→下載作廢版）`／`AC-35: 草稿無作廢入口（負向）`——**實為三條 `test`**（差旅／保養／草稿負向）；下載斷言為**三段位元組級**（T12b FW-6 落地，防 PHASE-008 型恆真）；統計排除為**作廢前後對照全等式**（T17R 修復 S-2 之近恆真 `includes` 斷言） | T17（＋ T17R 之 S-1／S-2 修復） | **GREEN** |
| AC-36 | e2e | `e2e/revision.spec.ts` | `修正版端到端 Gate — PHASE-009-T17（AC-36／AC-34）` › `AC-36: 修正版端到端（已完成差旅→建立修正版→新草稿含複製欄位與附件→修改後完成→產生報表得新編號→原申請版本關係且原編號未變）`——**`SPEC-REV-9DOCA` 名稱逐字校正**（原預定名 `AC-36: 修正版端到端（建立→複製欄位與附件→完成→新編號→版本關係雙向→原報表編號未變）` 於 repo 不存在） | T17 | **GREEN** |
| AC-37 | 全 Task ＋ 終審 | —（**無單一測試載體**；以三套件實跑數為證） | 每個 Task 之 Done When 含「既有測試零弱化」；終審以三套件**自行實跑**對照基準線 **後端 3230／前端 346／E2E 50**（起點 2952／254／44）。**轉綠證據（終審 2026-08-09）**：終審 reviewer 於同一 HEAD（`fb30d2a`，工作樹乾淨）、同一時段（02:20:47／02:20:51／02:21:52）親自實跑三套件：後端 **123 檔 3230 passed／0 failed／0 skipped**、前端 **20 檔 346 passed／0 failed／0 skipped**、E2E **50 passed**（serial, workers=1, retries=0），三值與基準線逐一全等；另同時段 backend/frontend `tsc --noEmit` 皆 0、biome 282 檔 0 診斷；零 `.only`／零新增 skip／`--diff-filter=D` 為空。狀態欄由大總管依終審裁定回填（白名單） | 全 Task ＋ 終審 | **GREEN** |
| AC-38 | integration | `phase9-contract.test.ts` | `AC-38: 上傳之補償刪檔與非預期錯誤兩條路徑之日誌零 storage key、零 volume 絕對路徑（AR-7 修復點；修復前必紅）`——**修復前紅燈物證**：`EPERM`／`ENOSPC` 兩行實際洩漏 storage key 與絕對路徑（T18 Handoff 記載）；修法沿 **AR-4** 既有形狀 | T18 | **GREEN** |
| AC-39 | integration | `phase9-void-report.test.ts` | `PHASE-009-T10 §A — report-data.ts 結構性掃描（AC-19(d)／AC-39(a)）` › `AC-39(a) 結構: report-data.ts 之 orderBy 含 { id: "asc" } 恰兩處（移除任一處必紅）`；`AC-39(b): 保養／折舊路徑之同 linkedAt 三附件恆依 id 遞增序輸出` › `保養：寫入序與 id 遞增序刻意相反，輸出仍為 id 遞增序`／`折舊：寫入序與 id 遞增序刻意相反，輸出仍為 id 遞增序（同一 fetchApplicationAttachments 路徑之第二消費端）`——**(a)(b) 皆由 T10 全額交付**（T10 **W-3** 提前交付屬加法；**T18 僅覈實未重複實作**，`SPEC-REV-9DOCA` 據此更正 Task 欄，原記 `T10 ＋ T18`）。**PHASE-008 終審 X8b 守門缺口正式關閉** | **T10**（全額；T18 覈實） | **GREEN** |
| AC-40 | e2e（(a) 記載型）＋ integration（(b)） | `e2e/report-print-layout.spec.ts`（**檔頭註解**，無斷言）；`backend/test/integration/phase8-contract.test.ts`（(b)） | **(a) 記載型**：`e2e/report-print-layout.spec.ts` 檔頭已由 T18 更正為實測結果（**子集字型 ＋ 內容流之 `Tj`／`TJ` ＋ `ToUnicode` CMap，非 `Type3` 向量化**；PHASE-008 T8R2 **SF-7** 實測）——**本子項無測試載體**，證據為 T18 commit `5dfd8ed` 之檔頭 diff ＋ T18 即審 **APPROVE**；**殘留（不在 T18 Allowed 內，已登記）**：同檔 :709 與 :1058-1059 兩處**行內註解**仍作 `Type3`，與 `docs/specs/PHASE-008.md` §11.4 併為同批移交（§17.1 #8、§19 #6）。**(b) 有斷言**：`AC-40(b) 結構: reports/routes.ts 之 REPORT_GENERATION_FAILED 經 AppError 承載，零 buildErrorBody 繞道`；`AC-40(b): 500 REPORT_GENERATION_FAILED 之 wire body 逐鍵全等（頂層恰一鍵 error；error 恰四鍵 code/message/requestId/details；details 恰一鍵 stage）`（**硬編碼字面雙實作驗證含鍵序**，T18 即審獨立覈實） | T18 | **GREEN**（(b) 有測試證據；(a) 為記載型無斷言，證據為 diff ＋ 即審 APPROVE——已於名稱欄逐字標示，終審可據此判斷） |
| **AC-41**（`SPEC-REV-9GATE` 新增；Mock Gate 定案③） | integration（投影面）＋ frontend（呈現面） | `phase9-void.test.ts`（投影面）；三型頁測試檔（呈現面） | **投影面（T15c）**：`AC-41 投影：已作廢詳情之作廢當時金額` › `AC-41 投影: 三型 VOIDED 詳情之 void.totalAmount ＝ 完成時 Application.totalAmount（作廢前後同值）`／`AC-41 投影 負向: 未作廢 → void 恆 null（既有斷言零弱化，金額無從外露）`／`AC-41(d) 結構: VOIDED 之 snapshot 仍恆為 null — 三型（定案③「不恢復整個快照區塊」；三型 service 檔零 diff）`；**呈現面（T15d；三型各一）**：`作廢（PHASE-009-T15）` › `AC-41(a)：已作廢詳情頁「作廢當時金額」逐字標籤 ＋ 整數原樣值 — 差旅`／`— 保養`／`— 折舊`；`AC-41(c)：void.totalAmount 為 null 時該列顯示「—」（整區不得消失、不得拋錯）`（三型）；`AC-41(a) 邊界：totalAmount 為 0 → 顯示「0」而非「—」（禁 falsy 回退）`（三型；**T15c 即審 FW「falsy 回退地雷」之落地**——以邏輯或短路取預設值會把 `0` 顯示成 `—`，正解為 `=== null` 三元 ＋ 加一格 `0` 鑑別）；`AC-41(d) 呈現：已作廢唯讀頁零單價／比例／計算明細（只放行單一金額欄）`（三型；附**真陰性論證**——Travel 避「總里程」假紅、M-D 同名標籤屬草稿分支）；**(b) 負向之實際落點**：斷言內嵌於 `AC-33 Empty：COMPLETED（未作廢）詳情頁零作廢資訊區塊`（三型各一，末行 `queryByText("作廢當時金額")` 不在場）——**`SPEC-REV-9DOCA` 依 T15d 即審 AR-1 據實回填：repo 中不存在名為「AC-41(b) 負向」之測試，不得照抄預定名** | **T15c**（投影面）＋ **T15d**（呈現面） | **GREEN**（兩側皆綠且各自即審 APPROVE——T15d 即審已明示「兩側皆綠可於 DOC-SYNC 整條轉 GREEN」） |

> **⚠ 拆列 AC 之轉綠紀律（沿 AC-08／AC-21／AC-32／AC-41 先例）**：凡由兩個以上 Task 分段交付之 AC，**任一側未綠即不得整條標 `GREEN`**。本次（`SPEC-REV-9DOCA`）之轉綠皆已逐側覈實兩（或三）側**同時**具實名測試證據且各自 Task 鏈 Review 清零。

> **本表統計（`SPEC-REV-9DOCA` 更新）**：**41 條 AC**；映射表 **48 列**（列數與構成本次零變動——`SPEC-REV-9GATE` 由 47 增為 48；47 列之構成見 `SPEC-REV-9T7`：AC-01 拆為 (a)(c)(d)(h)／(b)／(e)／(f)／(g) **五列**、AC-08 拆為 (a)(b) **兩列**、`AC-27／B-16` 與 `AC-32／§7.2 isRevision` **兩列**為既有條文之覆蓋缺口補列（**非新增 AC**），其餘各一列）。**狀態欄現況（終審 2026-08-09 後）：`GREEN` 48 列／`PENDING` 0 列——全表轉綠**。AC-37 依終審同時點三套件實跑（3230／346／50 全等）轉綠，證據記於該列。原「8 列 `PENDING（Spec Gate 2026-08-07 裁定後解鎖）`」**已於 Spec Gate 全數解鎖並全部轉綠**（AC-14／AC-18／AC-21／AC-22／AC-38／AC-40 ＋ AC-01(b)／AC-01(e)）。
...
| **T4** | 作廢端點 ＋ 授權矩陣 ＋ 稽核（同交易）＋ 錯誤合約 ＋ **`VOIDED` 之附件鎖定（AC-08(b)）** | BE | AC-05(b), AC-08(b), AC-23（作廢列）, AC-24, AC-25, AC-27 | `backend/src/applications/routes.ts`、`backend/src/applications/application-void.ts`、**`backend/src/attachment/lifecycle-service.ts`**（`SPEC-REV-9T3` 擴列）、`backend/test/integration/phase9-void.test.ts`、`backend/test/integration/phase9-contract.test.ts`、**`backend/test/integration/phase4-attachment-ard.test.ts`**（`deriveContainerState` 之既有守門檔；同型連動測試落點，實查亦見 `phase6-maintenance-attachment.test.ts`／`phase7-depreciation-attachment.test.ts`——**該二檔屬只讀回歸，非本 Task 之修改對象**） | **High**（授權 ＋ 稽核可追溯性 ＋ **附件權限**） | T3 | 接線端點（授權矩陣型）＋ 附件鎖定修復／**~230-330k**（含 AC-08(b) 之增量） | — | 授權五格逐格綠；判定順序側信道測試綠；稽核同交易回滾自證綠；`ErrorCode` 聯集全等（BOGUS mutant 必紅）；本人作廢寫稽核之正向斷言綠；**`VOIDED` 之附件鎖定與 `COMPLETED` 同等——`deriveContainerState` 三個 `refType` 分支（`TRIP_SEGMENT`／`MAINTENANCE`／`DEPRECIATION`）逐一涵蓋，已作廢申請之附件刪除一律 403（非 200）**；**修復前之紅燈實證記入 Handoff**（T3 已據實回報該缺口且依 Stop Conditions 未修）；既有 `COMPLETED`／`DRAFT` 判定行為零改變（三檔既有附件測試零弱化） |
...
| **T12a** | **作廢版 PDF 之產生與保存（service 段）**：作廢交易內渲染 → `put` → 讀回校驗 → `INSERT VoidedReportFile` → 提交；任一階段失敗整筆回滾 ＋ 補償刪檔。**含渲染鏈之依賴注入補齊**（T12a 第一輪 BLOCKED 實查揭露之結構缺口，方案 A） | BE | AC-21(a)(c)(e)(f) | **五檔**：`backend/src/server.ts`（`applicationsPlugin` 補注入 `reportStorage`／`imageMaxPx`／`pdfTimeoutMs`）、`backend/src/applications/routes.ts`（deps 傳遞 ＋ `ReportGenerationError` → 500 轉譯）、`backend/src/applications/application-void.ts`、`backend/src/reports/report-service.ts`、`backend/test/integration/phase9-void-report.test.ts` | **High**（不可逆產物 ＋ 交易 ＋ 補償） | T11, **D4 批准** | 大接線之 service 段（T12 對半估）／**~140-200k** | ✅（**僅在**真的新增／刪除 `src/reports/*.ts` 時）`PHASE_008_SRC_FILES` **九檔**清單——**本 Task 禁新建該目錄檔案（見下二約束），故預期零觸** | 四階段失敗注入各自零殘留且狀態仍 `COMPLETED`；未產生報表者作廢 → 零渲染零 storage 寫入；原 `Report.storageKey` 位元組 SHA-256 前後不變；`reports/` 零 `.report.update(`／`.delete(`／`.upsert(` 之既有結構斷言零改動且全綠；**`server.ts` 沿用同一 `reportStorage` 實例（不新建第二個 `LocalVolumeStorage`）**；`ReportGenerationError` 於作廢端點亦轉譯為 500 `REPORT_GENERATION_FAILED` ＋ `details.stage`（沿 `reports/routes.ts` :173-186 既有形狀）；**下二約束逐項自證（見下）** |
| **T12b** | **作廢後之下載語意（端點面）**：`GET .../report/pdf` 於 `VOIDED` 且有 `VoidedReportFile` 時回作廢版位元組（否則回原檔，皆零渲染）＋ **AC-23「下載 → 200（依 D4）」一格補齊**（FW-A 銷帳） | BE | AC-21(b)(d)、AC-23（下載格） | **三檔**：`backend/src/reports/routes.ts`、`backend/test/integration/phase9-void-report.test.ts`、`backend/test/integration/phase9-contract.test.ts`（下載格 ＋ **檔頭留白清單更新**） | **High**（公開 API contract ＋ 不可逆產物之對外語意） | **T12a** | 接線端點下緣（T12 對半估）／**~140-200k** | — | 作廢後下載回作廢版位元組、`Content-Disposition` 之報表編號與檔名**不變**；重複下載位元組全等且**渲染器零呼叫**（PHASE-008 AC-08 紀律於作廢後仍成立）；未作廢／無 `VoidedReportFile` 者仍回原檔（負向對照）；AC-23 報表四端點於 `VOIDED` 之回歸**四格全滿**且 `phase9-contract.test.ts` 檔頭留白清單同步移除該格；既有下載端點測試零弱化 |
...
| **T15c** | **【Mock Gate 定案③】「作廢當時金額」之 DTO 投影 ＋ 前端型別鏡像**（`SPEC-REV-9GATE`）：`VoidInfoDto` 新增 `totalAmount: number ｜ null` 並由 `resolveVoidInfo` 一併投影；`phase9-void.test.ts` 三型 `void` 區塊之既有閉集斷言同批補鍵；前端型別副本與三型頁 `voidInfoFixture` 對齊（**僅補鍵，零呈現斷言**） | BE ＋ FE（型別鏡像；**非**跨三層） | AC-41（投影面：(a) 之資料前提 ＋ (d) 之結構守門） | **六檔**：`backend/src/applications/application-void.ts`、`backend/test/integration/phase9-void.test.ts`、`frontend/src/api/applications.ts`、`frontend/test/TravelApplicationPage.test.tsx`、`frontend/test/MaintenanceApplicationPage.test.tsx`、`frontend/test/DepreciationApplicationPage.test.tsx`（**後三檔之改動限於 `voidInfoFixture` 補一鍵**，各檔 ≤2 行；**禁止**寫入任何 AC-41 呈現斷言——歸 T15d） | Medium | T15（**排程**：三型頁測試檔與 T16／T16R 同檔，須排在其後，不得併行） | 小型接線／型別鏡像型（單鍵）／**~90-130k** | ✅ `phase9-void.test.ts` 三型 `void` 區塊閉集三鍵→**四鍵**（AC-41 明文要求之受控擴充，**非弱化**）；三型頁測試檔 `voidInfoFixture` 補一鍵 | 三型 `VOIDED` 詳情之 `void.totalAmount` ＝ 完成時 `Application.totalAmount`（作廢前後同值）逐型綠；未作廢 `void` 恆 `null` 之既有負向零弱化；**`VOIDED` 之 `snapshot` 仍恆 `null`** 之結構斷言綠（**三型 service 檔本 Task 零 diff**，非 Files Allowed）；`backend` 既有測試零弱化；`frontend` typecheck 綠且**三型頁零呈現變更**（既有三型頁測試全綠、零新增呈現斷言） |
| **T15d** | **【Mock Gate 定案③】已作廢詳情頁「作廢當時金額」之呈現**（`SPEC-REV-9GATE`）：三型「作廢資訊」區塊**末列**新增逐字標籤 `作廢當時金額` ＋ 整數原樣值（`null` → `—`） | FE | AC-41（呈現面：(a)(b)(c) ＋ (d) 之呈現側負向） | 三型頁 ＋ 三型頁測試檔（**合計 6 檔**；沿 T15／T16／T16R 之既有形狀） | Medium | **T15c** | FE 頁面型下緣（單欄位）／**~100-140k** | — | 三型各一正向（標籤逐字、值取自 DTO 之 `void.totalAmount`、整數原樣**零格式化**）；**一條負向**（未作廢之已完成頁零「作廢當時金額」字樣，防恆真）；`null` → `—` 一格；**唯讀頁仍零單價／比例／計算明細**（定案③「不恢復整個快照區塊」之呈現面負向）；AC-30(c) 既有三項作廢資訊之逐字斷言零弱化；既有三型頁測試零弱化 |
...
| **T18** | chore：AR-7 洩漏修復 ＋ tiebreaker 行為測試 ＋ 檔頭更正 ＋ `buildErrorBody` 回歸 | BE ＋ E2E（檔頭） | AC-28, AC-38, AC-39(b), AC-40 | `backend/src/attachment/upload-service.ts`、`backend/src/reports/routes.ts`、`backend/test/integration/phase9-contract.test.ts`、`backend/test/integration/phase8-contract.test.ts`、`e2e/report-print-layout.spec.ts`（僅檔頭註解） | Medium | **T12b**（`SPEC-REV-9T12`：原記 `T12`，拆分後前提為觸及 `reports/routes.ts` 之 T12b）, **D14 批准** | 多項合併修復（≥4 項移交）→ **逐項累加，勿套單項帶**／**~150-250k** | — | AR-7 修復前必紅之紅燈證據；日誌掃描擴上傳路徑 ＋ 反向探針；保養／折舊 tiebreaker 行為測試綠；`buildErrorBody` 改道後 wire 格式逐字不變；檔頭敘述與實測相符 |

> **AC-37（零弱化與基準線）**：不設獨立 Task——寫入**每個 Task 之 Done When**，並由終審以三套件實跑數對照基準線核對。**基準線（`SPEC-REV-9DOCA` 更新）**：起點 後端 **2952**／前端 **254**／E2E **44**（PHASE-008 終審實測）→ 本 Phase 終值 後端 **3230**／前端 **346**／E2E **50**。

> **T12a／T12b 之兩項下游約束（`SPEC-REV-9T12` 固化；大總管 2026-08-07 親自實查定案，Packet 須逐字轉載）**
>
> 1. **作廢版渲染程式碼必須整段附加於 `report-service.ts` 既有 §9.1 產生流程之後**——`phase8-report-generate.test.ts` :225-247 之結構斷言以 **`src.indexOf("renderReportHtml(")` 之首現位置**判定「渲染呼叫巢狀於 `$transaction(...)` 回呼內部、且配號早於渲染」（`txCallIdx < renderCallIdx < readCommittedIdx`）。**若把作廢版渲染碼插入既有流程之前，首現位置即改指新程式碼，該既有斷言會在零行為缺陷下轉紅**。此為位置約束，非風格偏好；T12a 須於 Handoff 附該檔實跑全綠之證據。
> 2. **不得新建 `backend/src/reports/*.ts`**——`phase8-contract.test.ts` :304-327 之 `PHASE_008_SRC_FILES` 為 **`toEqual` 目錄實列**（`readdirSync` 對照 ＋ `expect(actualFiles.length).toBe(9)`）。**該常數為九檔，含 `report-labels.ts`**（:306-316，大總管親查定案；T10 即審 **AR-5** 判定正確）——**同一檔頭註解之「八檔」失準已誤導兩個 agent**（T12a 第一輪為第二次），檔頭修正歸 chore（優先級已上調）。作廢版渲染邏輯一律落於既有 `report-service.ts`／`application-void.ts`，新增檔案將使該九檔斷言必紅。
>
> **併記（`server.ts` 之注入順序，T12a 實查前提）**：現行 `server.ts` 於 :136 註冊 `applicationsPlugin`、於 :147-166 才解析 `reportStorageRoot` 並建構 `reportStorage`（:166），:167-173 注入 `reportsPlugin`。T12a 補注入時**須將 `reportStorage` 之解析與建構前移至 :136 之前**，`applicationsPlugin` 與 `reportsPlugin` **共用同一實例**。**不得於 `applications/routes.ts` 內自建**——storage root 解析（測試注入 > 環境變數 > production fail-fast > 非 production 暫存目錄）之單一事實來源在 `server.ts`，另建實例等於複製該段邏輯並繞過 `buildServer` 之測試注入（同紀律已逐字記於 `applications/routes.ts` :117-127 之 `attachmentStorage` 註解，本次同型適用）。T12a 第一輪 agent 拒絕自建而回報 BLOCKED **屬正確判讀**。
...
>
> **T15c／T15d 之位置（`SPEC-REV-9GATE` 補記）**：`T15 ──▶ T15c ──▶ T15d`——T15d 之呈現以 T15c 落地之 `void.totalAmount` 為資料前提，**不可併行**。**排程約束**：兩者皆觸及三型頁測試檔（T15c 僅 fixture 補鍵、T15d 為呈現斷言），與**已交付**之 T16／T16R 同檔，故須**排在 T16／T16R 之後**，且彼此不得併行。**與 T17 之關係**：AC-41 **不含** E2E 義務（本 AC 之驗收全在 integration ＋ frontend 兩層），故 T17 不依賴 T15c／T15d；惟 T17 若已交付，T15d 之頁面改動屬**只增一列 `<dt>／<dd>`**，不改變任何既有 DOM 語意，E2E 零預期影響（實跑仍須零弱化）。
>
> **T12a／T12b 之位置（`SPEC-REV-9T12` 補記）**：`T11 ──▶ T12a ──▶ T12b`——T12b 之下載語意以 T12a 落地之 `VoidedReportFile` 為資料前提，**不可併行**（圖中 `T12a` 節點承接原 T12 之全部上游與下游連線）。**T18 之依賴由 `T12` 改為 `T12b`**（機械一致性）：T18 觸及 `backend/src/reports/routes.ts`，須待 T12b 對該檔之修改完成後方可疊加，否則兩 Task 對同檔並行。**T13**（列印端點，同觸 `reports/routes.ts`）之依賴維持 `T11, D5 批准` 不變，惟大總管排程時**不得使 T12b 與 T13 併行**（同檔衝突）。

**TDD 順序建議**：T1（安全網先建立）→ T2（狀態機單一事實來源）→ T3／T5（兩條主線之 service，可先後不可併跑 vitest）→ T4／T7（端點與授權）→ **T7b（合約一致性批次，可與 T8／T9 併行）**→ T6（附件複製）→ T8（引用保護）→ T9（統計回歸）→ T10／T11（報表資料與版型）→ **T12a（PDF 產生保存）→ T12b（下載語意）**／T13（列印端點；**與 T12b 同觸 `reports/routes.ts`，不得併行**）→ T14/T15/T16（前端）→ T17（E2E Gate）→ T18（chore 收尾）。
...
| T2 | 2 | 2 | BE only（純函式） | ✅ |
| T3 | 5 | 6 | BE only | ✅ |
| T4 | **6 條目／5 條 AC**（AC-05(b)、AC-08(b)、AC-23 皆為**子項切片**，其母 AC 之其餘子項屬 T2／T3／T7） | **6** | BE only | ⚠ **貼上限**（`SPEC-REV-9T3` 擴列後：檔數恰 6、AC 條目 6）——**若 T4 實作時逾規模，正解為拆出 `T4b`（附件鎖定面：`lifecycle-service.ts` ＋ `phase4-attachment-ard.test.ts` ＋ AC-08(b)），不得以壓縮測試換取合規**；拆分與否由大總管於派工時裁量 |
| T5 | 3 | 6 | BE only | ✅ |
| T6 | 1 | 3 | BE only | ✅ |
| T7 | 4 | 4 | BE only | ✅ |
...
| T10 | 2 | 3 | BE only | ✅ |
| T11 | 1 | 2 | BE only（純函式） | ✅ |
| **T12a** | 1（AC-21 之四個子項 (a)(c)(e)(f)） | **5** | BE only | ✅（`SPEC-REV-9T12` 拆分後；檔數由 4 增為 5 係方案 A 擴列 `server.ts`／`applications/routes.ts` 並移出 `reports/routes.ts`，仍在 ≤6 內） |
| **T12b** | 2（皆為既有 AC 之子項切片：AC-21(b)(d)、AC-23 之「下載 → 200」一格） | 3 | BE only | ✅（`SPEC-REV-9T12` 補列） |
| T13 | 1 | 2 | BE only | ✅ |
| T14 | 3 | 6 | FE only | ✅ |
| T15 | 2 | 6 | FE only | ✅ |
| **T15c** | 1（AC-41 之**投影面**切片：(a) 之資料前提 ＋ (d) 之結構守門） | **6** | **BE ＋ FE**（型別鏡像；**非** FE＋BE＋DB 三層） | ✅（`SPEC-REV-9GATE` 補列；兩層合規——沿 T18「BE ＋ E2E 檔頭」之既有兩層先例。**⚠ 貼上限**：檔數恰 6，其中三檔為機械補鍵；若實作時逾規模，**正解為拆出 `T15c2`（前端型別鏡像面：`frontend/src/api/applications.ts` ＋ 三型頁測試檔 fixture）而非壓縮測試**） |
| **T15d** | 1（AC-41 之**呈現面**切片：(a)(b)(c) ＋ (d) 呈現側） | 6 | FE only | ✅（`SPEC-REV-9GATE` 補列；形狀同 T15／T16／T16R） |
| T16 | 2 | 6 | FE only | ✅ |
| T17 | 3 | 2 | E2E only | ✅ |
...
| T5 作廢報表／PDF 標示保留（**High**） | **T10 ＋ T11 ＋ T12a ＋ T12b ＋ T13** | 拆分為五（資料／版型／PDF 產生保存／下載語意／列印端點；末二者原為單一 T12，依治理 §10 預拆——`SPEC-REV-9T12`） |
...
**High 風險 Task 清單（須於 Spec Gate 一併取得事前批准）**：**T1、T2、T3、T4、T5、T6、T7、T8、T9、T10、T11、T12a、T12b、T13**——**Spec Gate 批准之單位為 13 項**（清單原文為 T1~T13）；`SPEC-REV-9T12` 將 T12 預拆為 **T12a／T12b** 後 §15 之 High 列數增為 14，**兩者皆繼承原 T12 之事前批准，非新增批准面**（沿 `T4b` 繼承 T4 之先例——拆分屬治理 §10 之規模控制，不改變風險判定亦不擴大 Scope）。**併記（`SPEC-REV-9T7` 遺漏之機械一致性）**：`T7b`（High，合約一致性批次）亦為 T7 之衍生拆列，同樣**繼承 T7 之事前批准**；本行原文之「13 項」係指 Spec Gate 當日之批准單位，衍生列（T4b／T7b／T12a／T12b）**一律隨母 Task 之批准**，不另請示。
...
| **Mock Gate（作廢／修正版流程與作廢標示）** | T11／T15／T16 完成後（**PDF 與 E2E 可先不完備**） | 人類目視：作廢二次確認與原因欄之互動、已作廢唯讀頁三項資訊、版本關係雙向連結、**列印版作廢標示之版面**（首屏、不被分頁切開、原因長文字換行）、375px 可讀；**＋（`SPEC-REV-9DOCA` 補列——兩項已實際發生，補齊記載）**：①**版本關係區塊之「重複計入」提醒文案目視與逐字定案**（AC-32(c)；`SPEC-REV-9T16` 殘留 (ii) 之銷帳——實際走查以 `WALKTHROUGH-PHASE-009-MOCK.md` 之 14 列自擬文案表承接，**定案①「DUPLICATE_COUNT_WARNING 照現值定案」**）②**已作廢詳情頁之「作廢當時金額」目視**（`SPEC-REV-9GATE` 殘留 (iii) 之銷帳——實際走查以 D 段之金額落差必問項承接，**定案③＝(b) 補單一欄位**，並於 T15d 後補實機截圖予使用者） | `docs/verification/WALKTHROUGH-PHASE-009-MOCK.md`（step-by-step，含每步預期畫面與判定準則）；**走查結果欄已回填**（`90f1ea5` 產出、`8b579ef` 定案後同步；Mock Gate 2026-08-08 通過——A~F 全過 ＋ 五項定案） |
...
> **凡未裁定者，相關 AC 與實作一律不得開工**（§12 曾以 `PENDING（Spec Gate 2026-08-07 裁定後解鎖）` 標記受影響之 8 列——**D1~D16 已於 2026-08-07 Spec Gate 全數裁定，該 8 列已全部解鎖並於 `SPEC-REV-9DOCA` 轉 `GREEN`**；本行保留為歷史紀律記載）。
...
**推薦**：**(b1)**。理由：唯一能同時滿足 BE-US-22④ 與 BE-US-27⑤ 字面、又**不弱化任何 PHASE-008 已批准守門**的方案；失敗語意沿用 008 已驗證之「同生共死 ＋ 補償刪檔」形狀，實作風險最低。**若人類優先考量「作廢動作必須永遠成功、不受渲染器影響」，(a) 是唯一零風險選項——請於 Gate 明示，因為它等於接受 BE-US-22④ 字面不落地**（該情形下 AC-21 全列刪除、**T12a／T12b** 取消——`SPEC-REV-9DOCA` 機械更名，原作 `T12`，拆分見 `SPEC-REV-9T12`；本句描述**未被採納**之選項 (a)，屬歷史假設語境，不構成對現行 Task 結構之指示——表數基線連動一併免除）。
...
- (c)：語意最貼近「複製原申請資料」；與既有生命週期完全相容（每個 `Attachment` 列擁有自己的物件，刪除語意不變）；代價＝storage 佔用（≤5 × 10 MB／次）與一次位元組搬移之延時（無影像處理，僅 I/O）。
  > **成本加註（`SPEC-REV-9DOCA`；原句一字未動——本句為人類 Spec Gate 批准 D6=(c) 之依據，不得改寫）**：上句「≤5 × 10 MB／次」之上界**僅對保養與折舊成立**（兩型各恰一個附件容器，上限皆 5 張）。**差旅無總量上界**——PHASE-004 **B-14** 未對 `TripSegment` 之段數設上限，實際複製量為「**段數 × 每段 3 張 × ≤10 MiB**」，在資料層無封頂（見 §17.1 #5 之改寫與 §10 NFR 表）。**人類係在低估成本之前提下批准 (c)**：風險面為**極端差旅之 storage 增長與持鎖時長**（非正確性——AC-14(e) 之整筆回滾與零殘留不受影響），本 Phase **不新增段數上限**（設限屬使用者可見行為之擴充，須人類批准）。**須於下次 Human Gate 向使用者告知本落差**（T6 即審 **SF-3** 之擴散實查、`SPEC-REV-9T7` ⑥ 之改寫皆已在案）。
...
8. **PHASE-008 §11.4 之 Type0/CID 敘述更正**不在本 Task 之 Allowed 檔案內（唯一可寫為 `docs/specs/PHASE-009.md`），僅 `e2e` 檔頭部分於 T18 更正——**PHASE-008 本體之更正仍待大總管另派**（§19 #6）。**併記（`SPEC-REV-9DOCA` 實查）**：`e2e/report-print-layout.spec.ts` 之**檔頭**已由 T18 更正為實測結果（子集字型 ＋ `Tj`／`ToUnicode`），惟**同檔尚有兩處行內註解殘留 `Type3` 字樣**（:709、:1058-1059）——T18 之 Allowed 限於檔頭，agent 未擅動屬正確（T18 **W-3** 已據實登記）；該二處與 `docs/specs/PHASE-008.md` §11.4 併為同一批「記載型更正」移交，**不影響任何斷言**。
9. **作廢交易之逾時裕度為硬編常數，與 `REPORT_PDF_TIMEOUT_MS` 無程式耦合——構成一條運維約束**（`SPEC-REV-9DOCA` 補記；T12a 即審 **AR-3**）：`application-void.ts` 之 `VOID_TX_TIMEOUT_MS = 60_000`／`VOID_TX_MAX_WAIT_MS = 10_000` 為**恆定常數**，刻意**不由** `REPORT_PDF_TIMEOUT_MS` 推導（本檔對 `reports/` 域零依賴，為取裕度而建跨域依賴不划算——裁量已記於該檔註解）。後果：**`REPORT_PDF_TIMEOUT_MS` 之運維設定值須顯著小於 60 s**（預設 30000 ms，須保留 `put`／讀回校驗／`INSERT` 之餘裕）；若被調高至接近或超過 60 s，作廢交易會在渲染仍進行中被 Prisma 自身之交易逾時（P2028）搶先中止，使一筆「其實只是慢」的正常作廢變成 500（整筆回滾、零殘留仍成立，可重試）。**變更該 env 時須同批檢視 `VOID_TX_TIMEOUT_MS`**；本約束列入 PHASE-011 之 RUNBOOK 待辦（§17.2）。
...
| **PHASE-011** | ①備份還原須涵蓋 **`VoidedReportFile` 之 `rpt/` 物件**（若 D4(b1)）與**修正版複製之附件物件**；②孤兒清理排程須認得此二類新物件；③效能實測須涵蓋「作廢含 PDF 產生」之 10 s 目標與「修正版含附件複製」之 5 s 目標；④**RUNBOOK 須載明 `REPORT_PDF_TIMEOUT_MS` 之 ≤60 s 運維約束**（§17.1 #9；與 `VOID_TX_TIMEOUT_MS` 之硬編 60 s 無程式耦合，調高即撞 P2028） |
...
| 2026-08-07 | `SPEC-REV-9T12` | **T12 拆分固化 ＋ 檔案清單擴列 ＋ 兩下游約束入 Spec（零 AC 語意變更；AC-21 條文一字未動）**：**①§15 T12 列拆為 T12a／T12b 兩列**（依治理 §10「大接線一律預拆」，大總管 2026-08-07 裁定）——**T12a**＝作廢版 PDF 之產生與保存（service 段：交易內渲染 → `put` → 讀回校驗 → `INSERT VoidedReportFile` → 提交／整筆回滾補償），AC-21**(a)(c)(e)(f)**；**T12b**＝下載語意端點面（`VOIDED` 且有 `VoidedReportFile` → 回作廢版；否則回原檔，皆零渲染），AC-21**(b)(d)** ＋ **AC-23「下載 → 200（依 D4）」一格補齊**（FW-A 銷帳）＋ `phase9-contract.test.ts` 檔頭留白清單更新。預算由原 T12 之 `~200-300k` 對半估為兩段各 `~140-200k`。**②T12a Files 擴列為五檔（方案 A）**——T12a 第一輪 **BLOCKED**（第二十一次攔截，動工前停止零改動）實查揭露兩根因：(i) `applicationsPlugin` 僅注入 `prisma`＋`attachmentStorage`，渲染鏈**無** `reportStorage`／`imageMaxPx`／`pdfTimeoutMs` 可用（`server.ts` :136 註冊早於 :166 之 `reportStorage` 建構）；於 `applications/routes.ts` 自建實例違反 :117-127 逐字固化之單一事實來源紀律且繞過 `buildServer` 測試注入，**agent 拒絕自建屬正確判讀**；(ii) `ReportGenerationError` 全 repo **僅 `reports/routes.ts` :173 一處轉譯**，作廢端點四階段失敗會回 `INTERNAL_ERROR` 而非 AC-21(a) 逐字之 `REPORT_GENERATION_FAILED` ＋ `details.stage`。故 Files Allowed 加 **`backend/src/server.ts`**（`reportStorage` 之解析與建構前移至 `applicationsPlugin` 註冊之前，**沿用同一實例不新建**）與 **`backend/src/applications/routes.ts`**（deps 傳遞 ＋ 錯誤轉譯，沿 `reports/routes.ts` :173-186 既有形狀），合計五檔：上二 ＋ `application-void.ts` ＋ `report-service.ts` ＋ `phase9-void-report.test.ts`；**`reports/routes.ts` 移出 T12a、歸 T12b**。**③兩項下游約束固化於 §15 表後之專屬 blockquote（Packet 須逐字轉載）**：(1) **作廢版渲染碼必須整段附加於 `report-service.ts` 既有 §9.1 產生流程之後**——`phase8-report-generate.test.ts` :225-247 以 `src.indexOf("renderReportHtml(")` **首現位置**斷言 `txCallIdx < renderCallIdx < readCommittedIdx`，前插即在零行為缺陷下轉紅；(2) **不得新建 `backend/src/reports/*.ts`**——`phase8-contract.test.ts` :304-327 之 `PHASE_008_SRC_FILES` 為 `toEqual` 目錄實列 ＋ `length === 9`，**大總管親自實查定案為九檔且含 `report-labels.ts`（:306-316，T10 即審 AR-5 判定正確）**；同檔頭註解之「八檔」失準**已誤導兩個 agent**（T12a 第一輪為第二次），檔頭修正歸 chore（優先級上調）。另**併記 `server.ts` 注入順序**之實查事實（:136／:147-166／:167-173）。**連帶（機械一致性，非新決策）**：§12 **AC-21 列**之 Task 欄改 `T12a((a)(c)(e)(f)) ＋ T12b((b)(d))` 並加註「**AC-21 不得於單側綠時整條標 `GREEN`**」（沿 AC-08 先例）；§12 **AC-23 列**之「下載 → 200」歸屬由 T12 改 **T12b**；§15.1 依賴圖節點 `T12`→`T12a`、`T12 ──▶ T18`→`T12b ──▶ T18`，補位置註記（`T11 ──▶ T12a ──▶ T12b` 不可併行；**T12b 與 T13 同觸 `reports/routes.ts`，排程不得併行**）；§15 **T18 依賴**欄 `T12`→`T12b`；§15.1 TDD 順序建議同步；§15.2 自查表 T12 列拆為 T12a（1 AC／**5** 檔）與 T12b（2 條目／3 檔），皆 ≤5 AC、≤6 檔、BE only；§15.3 對照表「拆分為四」→「**拆分為五**」；§15.3 High 清單補記 **T12a／T12b 繼承原 T12 之 Spec Gate 事前批准**（沿 T4b 繼承 T4 之先例），併補 `SPEC-REV-9T7` 遺漏之 **T7b 繼承 T7 批准**之同型記載；§0.3 #4 之「§15 T10／T12」→「T10／**T12a**」。**一律不變**：**AC-21 之條文逐字**（§2 :209 零改動）、AC-23 條文、全部 40 條 AC 之驗收語意與守門強度、§7／§8／§9 全部合約與流程、§16 全部決策（D1~D16）、§12 之**列數（47）與全部狀態欄**（AC-21／AC-23 維持 `PENDING`）、其他章節。**性質**：Task 拆分與 Files Allowed 記載（記錄型 ＋ 內部技術細節），**未改變任何 AC 驗收語意、未縮減任何守門強度、未擴大 Scope、未改變任何使用者可見行為、未改變任何已批准 US 原意**；**未修改** `userstory.md`／`docs/PRD.md`／任何程式或測試。Spec 狀態維持 `ACTIVE`。**§11 交付前自查（治理 2026-08-07.1）結果**：修訂後掃描 T12a／T12b／AC-21／AC-23／§12／§15.1／§15.2／§15.3／§0.3 九處交叉引用，**全部一致、零矛盾、零殘留 `T12` 裸引用**（唯一例外見下）。**殘留一項據實回報大總管（本次未改，避免逾範圍）**：§16 **D4 推薦段**（:1004）之反事實句「該情形下 AC-21 全列刪除、**T12 取消**」仍寫作 `T12`——該句描述**未被採納**之選項 (a)（D4 已裁定 (b1)），屬歷史假設語境，**不構成對現行 Task 結構之指示**；因 §16 為決策點章節、本次 Packet 範圍限於 §15 Task 結構與檔案清單，故未動，建議併結案 DOC-SYNC 一併機械改為 `T12a／T12b`。 | 大總管 2026-08-07 依治理 §10「大接線一律預拆」之 T12 拆分裁定（`PROJECT_STATE.md` `4e3a122` **T11 全鏈 APPROVE＋T12 預拆裁定**列）；**T12a 第一輪 BLOCKED ＋ 方案 A 裁定 ＋ 九檔親查定案**（`PROJECT_STATE.md` `a8e292a` 列，含「依 T4b 先例先派 spec-writer 做 §15 T12 列擴列＋T12a/T12b 拆分固化（`SPEC-REV-9T12`）」之明文）；T4 即審 SF-1／FW-A（AC-23 下載格）；T10 即審 **AR-5**（九檔判定）；大總管 2026-08-07 `PHASE-009-SPEC-REV-9T12` Packet |
| 2026-08-08 | `SPEC-REV-9T16` | **單項覆蓋缺口補正：D15 之「前端強提示」載入 AC-32(c)（零新決策，引用既有人類批准落地）**：**①§2 AC-32(c) 加註**——原申請詳情頁之版本關係區塊（`supersededBy` 側）**必含「重複計入」提醒文案**，語意三點不可省（①原申請仍為已完成狀態、②不作廢則兩筆都會計入里程與金額統計、③作廢入口即在本頁），並給**暫定逐字字串**（`本申請仍為已完成狀態。若未作廢，本申請與修正版將同時計入里程與金額統計；如需避免重複計入，請於本頁作廢本申請。`）供實作與逐字斷言，逐字文案於 **Mock Gate** 由人類目視定案（要求調整時走 Gate 反饋流程：Spec 修訂 → implementer → 複審，三點語意不因文案微調而縮減）；**測試義務**＝三型詳情頁各一正向（`supersededBy` 非 `null` 時提醒在場）＋ 一條負向（`supersededBy` 為 `null` 之已完成頁零提醒，防恆真，§11.0 #2）；**修正版側（`supersedes`）不加此提醒**（作廢動作之落點在原申請頁）。**缺口性質**：D15=(a) 之人類批准**逐字前提**為「不自動作廢 **＋ 前端強提示**」（Spec Gate 裁定列；D15 推薦段「**但強制搭配前端提示**（AC-32(c) 之版本關係區塊加註提醒文案）」；§17.1 #1 緩解清單「前端提示 ＋ 版本關係顯示 ＋ Gate 目視」），惟 **AC-32(c) 條文未同步載入該提示**——T16 依 AC 字面實作即屬正確判讀，結果卻是**已批准之緩解措施全案零落地**（T16 即審 **M-1**）。故本次為**Spec 補正而非實作缺失**；提示之落地歸新 Task **T16R**。**②§12 AC-32 列同步**——補兩條預定測試名（三型各一正向 ＋ 一條負向）、Task 欄改 `T16（(a)／(c) 連結面／(d)）＋ T16R（(c) 提醒文案面）`、加註 **⚠「AC-32 不得於單側綠時整條標 `GREEN`」**（沿 AC-08／AC-21 先例）。**③本列**。**一律不變**：AC-32 之 (a)(b)(d)(e) 逐字、其餘 39 條 AC 之字面與驗收語意、§16 全部決策（**D15 之裁定結果 (a) 一字未動——本次不改變任何使用者可見行為之決策，僅使已批准之緩解措施在 AC 層有落點**）、§17.1 #1、§12 之**列數（47，本次為既有列內補測試名，非新增列）**與全部狀態欄（AC-32 維持 `PENDING`）、§7／§8／§9 全部合約、其他章節。**性質**：Spec 覆蓋缺口補正（使 AC 與既有人類批准前提一致），**未擴大 Scope**（提示文案本即在人類批准之 D15 前提內）、**未改變任何已批准 US 原意**、**未新增任何決策**；新增之使用者可見文案係**執行既有批准**，非新增使用者可見行為決策。**未修改** `userstory.md`／`docs/PRD.md`／任何程式或測試。Spec 狀態維持 `ACTIVE`。**§11 交付前自查（治理 2026-08-07.1）結果**：交叉掃描 **AC-32(c) ⇔ §16 D15 推薦段 ⇔ §18 Spec Gate 裁定列 ⇔ §17.1 #1 ⇔ §12 AC-32 列** 五處，修訂後**全部一致、零矛盾**（§17.1 #1 之「緩解＝前端提示 ＋ 版本關係顯示 ＋ Gate 目視」自始即預設本提示存在，本次補正使其成立而非改寫）；方法論 spike **不適用**（無新量測手段，前端文案在場／不在場之斷言為既有三型頁測試檔之既有手法）。**殘留兩項據實回報大總管（本次未改，避免逾 Packet 範圍）**：(i) **§15 無 `T16R` 列**——新 Task 之 Files Allowed（預期＝三型頁 ＋ 三型頁測試檔六檔，同 T16）與依賴（`T16 ──▶ T16R ──▶ T17`）之建立屬派工裁量，建議與 §15.1 依賴圖／§15.2 自查表一併補；(ii) **§15.4 Mock Gate 內容欄**未列「版本關係區塊之提醒文案目視與逐字定案」，建議於 `WALKTHROUGH-PHASE-009-MOCK.md` 與該欄補入，否則 AC-32(c) 所稱「由 Mock Gate 定案」在走查腳本無對應步驟。 | **既有人類批准**：leonchih 2026-08-07 Spec Gate 之 **D15=(a)「不自動作廢＋前端強提示」**（§18 Spec Gate 通過列；`PROJECT_STATE.md` Spec Gate 列；`WALKTHROUGH-PHASE-009-SPEC-GATE.md`）——**本次為該批准之逐字落地，非新決策**；觸發＝T16 即審 **M-1**（reviewer `PHASE-009-T16-REVIEW`）；大總管 2026-08-08 `PHASE-009-SPEC-REV-9T16` Packet |
| 2026-08-08 | `SPEC-REV-9GATE` | **Mock Gate 定案③之落地：新增 AC-41「已作廢詳情頁之作廢當時金額」＋ 拆 T15c／T15d 兩 Task（引用人類裁定，零自創決策）**：**①§2 新增 §L 節與 AC-41**——`VOIDED` 詳情頁之「作廢資訊」區塊**末列**顯示標籤逐字 `作廢當時金額`、值＝該申請**完成時快照之最終整筆金額**（`Application.totalAmount`，新臺幣整數，與列印版／PDF 之 `最終金額（新臺幣整數）` 同一整數同一長相，整數原樣零格式化）；**三型逐一正向** ＋ **一條負向**（未作廢頁零該字樣，防恆真，§11.0 #2）＋ **`null` → `—` 一格**（沿 `ApplicationListSection.tsx` :346 慣例）＋ **範圍守門 (d)**（`VOIDED` 之 `snapshot` **仍恆 `null`**、唯讀頁零單價／比例／計算明細——即定案③「**單一欄位、不恢復整個快照區塊**」之機械落點）。**②後端投影面實查（Packet 明令據實記載，基準 `5436fd8`）→ 結論：非純前端**——三型詳情 DTO **均無** top-level `totalAmount`，該值僅在 `snapshot` 內，而三處 `snapshot` 皆由 `status === "COMPLETED"` 閘住（`travel-service.ts` :1800／`maintenance-service.ts` :838／`depreciation-service.ts` :902），**`VOIDED` 下金額完全不在回應中**。**落地形狀（Spec 決策，內部 DTO 結構）＝於 `VoidInfoDto`（`application-void.ts` :66）新增一鍵 `totalAmount: number ｜ null`，由 `resolveVoidInfo`（:86）投影**——理由：`void` 未作廢恆 `null` 故**未作廢回應逐鍵不變**（AC-41(b) 由結構保證）、`resolveVoidInfo` 為三型共用單一函式故**三個 service 檔零改動**、折舊 top-level 封閉鍵集（`phase7-depreciation-draft.test.ts` :920，機械連動第六類）**不受影響**；**已否決之替代形狀**＝三型 DTO 各加 top-level 鍵（須改三檔 ＋ 觸發折舊閉集連動 ＋ 使 `DRAFT`／`COMPLETED` 回應一併變形，範圍大於定案③）。**③零新增資料揭露之實查佐證**——同一整數**早已**由列表 DTO 無條件投影（`application-query.ts` :422／:494，**不受 `status` 閘門**）並對已作廢列直接呈現（`ApplicationListSection.tsx` :346），故本 AC 係**消除「列表看得到、詳情看不到」之不一致**，非新揭露決策，授權路徑零變更。**④§12 補 AC-41 一列**（投影面／呈現面之預定測試名分列；Task ＝ `T15c ＋ T15d`；`PENDING`）＋ **⚠「不得單側綠時整條標 `GREEN`」**（沿 AC-08／AC-21／AC-32 先例）；統計連動：**40 條 AC → 41 條**、映射表 **47 列 → 48 列**、`PENDING` **46 列 → 47 列**（`GREEN` 仍 1 列）。**⑤§15 補兩列（Packet 原擬單一 `T15c`，因實查後檔數為 9 而拆二——治理 §10 規模上限 ≤6 檔）**：**T15c**＝投影面（`application-void.ts`、`phase9-void.test.ts`、`frontend/src/api/applications.ts`、三型頁測試檔之 `voidInfoFixture` 補鍵，**六檔**；BE＋FE 兩層，非三層；`~90-130k`）；**T15d**＝呈現面（三型頁 ＋ 三型頁測試檔，**六檔**；FE only；`~100-140k`，即 Packet 指定帶）。§15.1 補位置註記（`T15 ──▶ T15c ──▶ T15d`，不可併行；**須排在已交付之 T16／T16R 之後**——三型頁測試檔同檔；AC-41 **不含 E2E 義務**故 T17 不依賴之）；§15.2 補兩列自查（皆 ≤5 AC、≤6 檔；T15c 標 ⚠ 貼上限並附明文正解「逾規模時拆 `T15c2` 前端型別鏡像面，不得壓縮測試換合規」）。**⑥本列**。**一律不變**：其餘 40 條 AC 之字面與驗收語意（**AC-04／AC-30(c)／AC-31／AC-33 一字未動**）、§16 全部決策（D1~D16）、§7／§8／§9 全部合約（**`VoidInfoDto` 之 §7.2 宣告本次未改，屬 §12／§15 未列之連動，見殘留項**）、其餘 §12 狀態欄、§11 各表、其他章節。**性質**：**人類 Gate 裁定之落地**（新增使用者可見欄位係執行 Mock Gate 定案③，非 spec-writer 自創決策）＋ Task 拆分記載；**未改變任何已批准 US 原意**、**未縮減任何守門強度**（(d) 反而新增範圍守門）、**未擴大 Scope**（僅定案③逐字範圍）；**未修改** `userstory.md`／`docs/PRD.md`／任何程式或測試。Spec 狀態維持 `ACTIVE`。**§11 交付前自查（治理 2026-08-07.1）結果**：跨 AC 一致性掃描 **AC-41 ⇔ AC-04（不清空 `totalAmount`）⇔ AC-30(c)（三項作廢資訊，AC-41 為新增第四列，零刪減）⇔ AC-31(b)(c)（唯讀頁零控制項——**純顯示 `<dd>` 非控制項，零衝突**）⇔ AC-33 Empty（未作廢零區塊 ⇔ AC-41(b) 負向，互為佐證）⇔ 定案③「不恢復整個快照區塊」（⇔ AC-41(d) 之 `snapshot` 恆 `null` 結構守門）** 六組，**修訂後全部一致、零矛盾、零需人類裁定之衝突項**；**方法論 spike 不適用**——無新量測手段，投影面為既有 `toEqual` 閉集手法、呈現面為既有三型頁測試檔之 `dtDdText()` 既有手法（`TravelApplicationPage.test.tsx` :1148 等已在用）。**殘留三項據實回報大總管（本次未改，避免逾 Packet「零其他條文修改」硬約束）**：(i) **§7.2 之 `VoidInfoDto` 宣告區塊**（:444 起）仍為三鍵，未同步 `totalAmount`——建議併累積勘誤批次或由 T15c Packet 授權同批補；(ii) **§11.2／§11.3 測試策略表**未列 AC-41（§11.2「三型 DTO void 區塊」與 §11.3「三型詳情頁」兩列之重點欄可加註），§12 為權威映射故不影響機械可查性；(iii) **§15.4 Mock Gate 內容欄**未列本欄位之目視項（沿 `SPEC-REV-9T16` 殘留 (ii) 之同型建議）。 | **人類 leonchih 2026-08-08 Mock Gate 定案③＝(b)**（`PROJECT_STATE.md` `5436fd8` Mock Gate 通過列；`docs/verification/WALKTHROUGH-PHASE-009-MOCK.md` 定案清單第③項；源自 T15 即審 **AR-2**）：「已作廢詳情頁補『作廢當時金額』欄位＝該申請完成時快照之最終整筆金額（新臺幣整數，與 PDF 一致）；單一欄位、純顯示、不恢復整個快照區塊（單價／比例等仍不顯示）；三型皆適用」；大總管 2026-08-08 `PHASE-009-SPEC-REV-9GATE` Packet |
| 2026-08-09 | `PHASE-009-DOC-SYNC-A`（`SPEC-REV-9DOCA`） | **結案勘誤批次 ＋ §12 全表實名回填轉綠 ＋ 基準線更新（零 AC 語意變更；全部為勘誤／回填／加註）**。**A. 基準線三處**——AC-37 條文（§2 J 節）、§12 AC-37 列、§15 表後 AC-37 註：由 **後端 2952／前端 254／E2E 44**（PHASE-008 終審實測之**起點**）更新為本 Phase 終值 **後端 3230／前端 346／E2E 50**（3230＝T18 結案後大總管獨立驗收實跑；346＝T15d 結案實跑；50＝T17 實跑 ＋ 大總管親跑複驗）。三數皆為**淨增**（+278／+92／+6），**零刪除零 skip**；屬 AC-01(f) 型之機械基線重錨定，**非弱化**。**B. §12 全表實名回填與轉綠（最重；終審硬檢查之依據）**——48 列逐列以 `rg`／逐檔實開覈實，預定名一律替換為 `describe › it` 實名，狀態 `PENDING → GREEN` **47 列**，`PENDING` **1 列**（AC-37，見下未轉綠清單）。逐列遵守各即審已裁定之回填規則：**AC-15(a)(b)(c) 實為單一 `it`**（T9 即審 **AR-1**——原三條分列預定名於 repo 不存在，明令不得照抄）；**AC-23 為四段合成**（T4 作廢 5 格 ＋ T7 修正版 5 格 ＋ **T12b** 下載格 ＋ **T13** 列印格，回歸四格全滿＝T4 **FW-A／SF-1** 正式閉環）；**AC-32 分段**（T16 連結面 ＋ T16R 提醒文案面 ＋ T16R2 之 `!voided` 守門）；**AC-33 owner 補正**為 `T14（對話框）＋ T15（頁面層 Empty／Success）＋ T16（修正版側 Success／Error）`（T14 即審 **SF-1** 之終審假閉環風險銷帳）；**AC-34 兩檔兩測試**（T17 **W-2** 四面跨檔拆分落表）；**AC-35 實為三條 `test`**（差旅／保養／草稿負向）；**AC-36 名稱逐字校正**（原預定名不存在）；**AC-41(b) 負向之實際落點**為 `AC-33 Empty：COMPLETED（未作廢）詳情頁零作廢資訊區塊` 內之末行斷言（T15d 即審 **AR-1**——repo 無名為「AC-41(b) 負向」之測試），並補記 **`0` 邊界三條**（T15c 即審 FW 之 falsy 回退地雷）；**AC-28／AC-38／AC-40** 依 T18 即審實名（AC-40(a) 為**記載型無斷言**，於名稱欄逐字標明證據＝T18 commit `5dfd8ed` 檔頭 diff ＋ 即審 APPROVE，並登記同檔 :709／:1058-1059 兩處**行內** `Type3` 殘留屬 T18 Allowed 外之移交）；**AC-39 Task 欄更正為 T10 全額交付**（T18 僅覈實未重複實作——T10 **W-3** 提前交付屬加法）；**AC-29 owner 補正**加 T15（(a) 入口為頁面層）；**AC-30 加 T15b**（(b) 篩選選項為真缺件補做）；**AC-32／isRevision 列**之 owner 補正為 `T16pre（後端投影＋鍵集基線）＋ T16（前端徽章）`。§12 統計句同步（41 AC／48 列／`GREEN` 47・`PENDING` 1）。**C. 條文勘誤十項**——①**§7.2 `VoidInfoDto` 三鍵→四鍵**（補 `totalAmount: number ｜ null` ＋ 補記段落；`SPEC-REV-9GATE` 殘留 (i) 銷帳）②**§L AC-41(a)「暫定」二字去除**（Mock Gate 定案③人類原話即「作廢當時金額」逐字；T15c 結案列已記「不再另問」）＋**同型處理 AC-32(c) 之「暫定逐字字串」**（Mock Gate **定案①** DUPLICATE_COUNT_WARNING 照現值定案——同一 Gate 同一裁定，不同步將造成 Spec 內部不一致）③**§8.2 `storageKey` 註解 `void.pdf` → `void`**（T12a **W-2**：既有 storage key 白名單拒 `.`；`fileName` 與 `storageKey` 為兩欄，AC-21(b) 取 `fileName` 故零行為影響）＋加註同源殘留（`schema.prisma` 同欄註解不在本 Task Allowed，待另派）④**§16 D6 (c) 成本敘述加註**（**原句一字未動**——該句為人類 Spec Gate 批准 D6=(c) 之依據）：明記「≤5 × 10 MB／次」僅對保養／折舊成立、**差旅無總量上界**（B-14 未設段數上限）、人類係在低估成本前提下批准、**須於下次 Human Gate 告知**⑤**§16 D4 推薦段「T12 取消」→「T12a／T12b 取消」**（`SPEC-REV-9T12` 殘留項銷帳；該句描述未被採納之選項 (a)，屬歷史假設語境）⑥**§8.5 補第七類機械連動**（列表 DTO 鍵集基線——`phase9-contract.test.ts` **§M** 一處；T16pre **首次開列**非重錨定）＋預授權句「六類」→「**七類**」⑦**鍵數勘誤 14→15**（§7.2 之「既有 17 鍵」與 §12 之「17→18」皆為記載失準；T16pre **W-1** 實查，前端獨立副本交叉印證）⑧**§11.2／§11.3 測試策略表補 AC-41**（投影面／呈現面）＋補記 AC-33 兩子態之頁面層落點與列表徽章（`SPEC-REV-9GATE` 殘留 (ii) 銷帳）⑨**§15.4 Mock Gate 內容欄補兩項**（版本關係區塊提醒文案之目視與逐字定案／已作廢詳情頁「作廢當時金額」之目視——**兩項皆已實際發生**，走查腳本欄同步標註結果已回填；`SPEC-REV-9T16` 殘留 (ii) 與 `SPEC-REV-9GATE` 殘留 (iii) 一併銷帳）⑩**同型措辭精確化**：AC-03「四個作廢欄」→「三個作廢欄」（附測試零修改義務之說明——`phase9-void.test.ts` 之既有測試名沿用四欄係其 fixture 非修正版，斷言更強）、§19 #4「四個作廢欄」→「四個新欄（三作廢欄 ＋ `supersedesId`）」；併 **§17.1 新增 #9**（`REPORT_PDF_TIMEOUT_MS` 之 ≤60 s 運維約束——T12a 即審 **AR-3**：`VOID_TX_TIMEOUT_MS = 60_000` 為硬編常數、與該 env **無程式耦合**，調高即在渲染中被 P2028 搶先中止）＋ **§17.2 PHASE-011 列補 ④ RUNBOOK 義務**＋ **§17.1 #8 補記** `e2e/report-print-layout.spec.ts` 兩處行內 `Type3` 殘留＋ **§16 前言**之「8 列條件 AC」現況註記。**一律不變**：**全部 41 條 AC 之驗收語意與守門強度**（本次零條文語意變更——`GREEN` 轉綠為狀態欄回填，勘誤為措辭／計數／記載精確化，加註為既有事實之補記）、**§16 全部決策 D1~D16 之裁定結果**（D6 原句一字未動）、§3~§10 全部流程與合約之行為敘述、§15 全部 Task 結構與 Files Allowed、§12 **列數 48 零變動**。**未修改** `userstory.md`／`docs/PRD.md`／任何程式或測試（本 Task 唯一 Allowed 檔為 `docs/specs/PHASE-009.md`）。Spec 狀態維持 `ACTIVE`（Phase 尚未終審，不轉 `COMPLETED`）。**§11 交付前自查（治理 2026-08-07.1）結果**：**(1) 實名抽驗 ≥10 列**——AC-01(a)、AC-03、AC-05(a)、AC-10、AC-13、AC-15、AC-18、AC-20(d)、AC-21(a) 失敗矩陣、AC-22 §K、AC-23 下載格、AC-32/isRevision §M、AC-35、AC-41(b) 之實際落點，共 **14 列**逐條實開測試檔核對 `describe`／`it` 逐字在場，**全中零落差**；**(2) 跨 AC／跨節一致性掃描 8 組**（AC-37 ⇔ §12 AC-37 列 ⇔ §15 註；§7.2 `VoidInfoDto` ⇔ §L AC-41 實作註記 ⇔ §12 AC-41 列；§7.2 鍵數 ⇔ §8.5 第七類 ⇔ §12 isRevision 列；§8.2 註解 ⇔ AC-21(b) 檔名不變；§16 D6 加註 ⇔ §10 NFR 表 ⇔ §17.1 #5；§16 D4 ⇔ §15 T12a／T12b；AC-03 措辭 ⇔ AC-09 措辭 ⇔ §19 #4；§15.4 ⇔ AC-32(c) ⇔ §L AC-41），**修訂後全部一致、零新增矛盾、零需人類裁定之衝突項**；**(3) 統計與列數機械相符**——`\| ` 起始之表列實數 **48**、`GREEN` **47**、`PENDING` **1**，與統計句三數逐一相符；**(4) 方法論 spike 不適用**（本次零新增量測手段，回填手法為 `rg` ＋ 逐檔實開之既有手法）。**未轉綠清單（1 列，附原因）**：**AC-37**——無單一測試載體，現有三數（3230／346／50）為**分次量測**（T18／T15d／T17 各自結案時點），非同一時點之三套件齊跑；依「無測試證據之列不得轉綠」及本 Spec 自身「終審須自行實跑」之要求，保留 `PENDING`，**由終審實跑後轉綠**。 | 各 Task 結案列與即審裁定（`PROJECT_STATE.md` 2026-08-07~08 之 T1~T18 全鏈紀錄）：T9 即審 **AR-1**（AC-15 單一 it）／T4 即審 **SF-1・FW-A**（AC-23 下載格歸屬）／T4b 即審 **AR-2**（AC-08(b) 落點）／T5 即審 **AR-3**・T6 即審 **SF-3**（計數與常數勘誤）／T10 即審 **AR-5**（九檔判定）／T12a 即審 **AR-3**（`REPORT_PDF_TIMEOUT_MS` 耦合）・**W-2**（`storageKey` 無副檔名）／T12b 即審 **FW**（`schema.prisma` 註解）／T14 即審 **SF-1**（AC-33 owner）／T15 即審 **M-1**（AC-30(b) 缺件）／T16pre 即審 **W-1・W-2・AR**（鍵數 14→15、第七類連動）／T16 即審 **M-1**（D15 提示）／T17 即審 **DOC-SYNC 三組登記**（AC-37 三處 44→50、AC-34~36 實名）／T15d 即審 **AR-1**（AC-41(b) 實名）・**AC-41 整條轉綠裁定**／T18 即審 **A-1・A-2・W-3**（記載型登記）；**人類 leonchih 2026-08-08 Mock Gate 通過**之定案①（DUPLICATE_COUNT_WARNING 照現值）與定案③（作廢當時金額）；大總管 2026-08-09 `PHASE-009-DOC-SYNC-A` Packet |
...
| 4 | PRD :499「Rollback 概要」 | 補記「`AuditAction` 新 enum 值不可移除（PostgreSQL 限制）」與「回滾前須匯出 `Application` 之**四個新欄**（三個作廢欄 `voidReason`／`voidedAt`／`voidedById` ＋ 版本關聯欄 `supersedesId`）」 | §14 之落地約束（`SPEC-REV-9DOCA` 用詞精確化：原作「四個作廢欄」，與 AC-03／AC-09 同型——`supersedesId` 不屬作廢欄但同受 `DROP COLUMN` 影響，故匯出範圍確為四欄） |
