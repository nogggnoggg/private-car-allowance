...
- **(a)** `Application` 新增**四個 nullable 欄位**（§8.1 逐欄相符，以 `information_schema.columns` 斷言）：`voidReason`（text，nullable）／`voidedAt`（timestamptz，nullable）／`voidedById`（text，nullable，**無 FK**，沿 `Report.generatedById`／003a／005a 慣例）／`supersedesId`（text，nullable，**`@unique`**，自關聯指向被修正之原申請，依 §16 **D7**）。**四欄皆無 `DEFAULT`、皆不回填**（既有列該四欄恆為 `NULL`）。
...
| 對象 | 檔案 | 重點 |
|---|---|---|
| 狀態機擴充 | `backend/test/unit/application-state-machine.test.ts`（既有，**只增不改**） | AC-02：轉換集合長度 2、四種非法轉換逐一、錯誤碼分流不變 |
...
| AC-01(b) | integration | `phase9-migration-safety.test.ts` | `AC-01(b): VoidedReportFile 表之欄位集合、兩個唯一約束與 FK(onDelete: Restrict) 逐欄符合 §8.2` | T1 | **BLOCKED（待 D4 裁定）** |
| AC-01(e) | integration | `phase9-migration-safety.test.ts` | `AC-01(e): AuditAction 恰新增 APPLICATION_VOIDED，其餘六個 enum 聯集全等（多一／少一必紅）`；`AC-01(e): ALTER TYPE 新值於獨立交易後方可使用（raw SQL fixture 手法自證）` | T1 | **BLOCKED（待 D10 裁定）** |
...
| AC-02 | unit | `application-state-machine.test.ts` | `AC-02: ALLOWED_TRANSITIONS 恰兩條（DRAFT→COMPLETED、COMPLETED→VOIDED）`；`AC-02: VOIDED→COMPLETED／VOIDED→DRAFT／DRAFT→VOIDED／COMPLETED→DRAFT 逐一被拒且錯誤碼分流不變`；`AC-02 結構: Application 無任何與 status 正交之作廢布林欄（D8(a) 硬約束）` | T2 | PENDING |
...
| AC-05 | integration | `phase9-void.test.ts`；`phase9-contract.test.ts` | `AC-05(a) 結構: backend/src 全域零 VOIDED→COMPLETED／DRAFT 之寫入路徑（白名單外必紅）`；`AC-05(b): 已作廢之 complete／三型 PUT／DELETE 皆 403 且零寫入` | T2 ＋ T4 | PENDING |
...
| AC-14 | integration | `phase9-revision-attachment.test.ts` | `AC-14(a)(b): 每張 LINKED 附件複製為新列與新 storageKey，位元組 SHA-256 相同、縮圖同步複製`；`AC-14(c): 差旅段—圖對位複製，零跨段錯置`；`AC-14(d): 複製後各容器附件數仍在上限內`；`AC-14(e): storage 失敗 → 整筆回滾、零殘留、日誌零 storage key` | T6 | **BLOCKED（待 D6 裁定）** |
...
| AC-18 | integration | `phase9-reference-guard.test.ts` | `AC-18(a): 五型別逐一——引用來源被作廢後仍判定為已被引用（COMPLETED-only mutant 必紅）`；`AC-18(b): 僅被草稿引用者仍判定為未被引用`；`AC-18(c): 既有 COMPLETED 正向判定逐型別零弱化` | T8 | **BLOCKED（待 D2 裁定）** |
