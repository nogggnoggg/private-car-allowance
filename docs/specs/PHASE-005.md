...
| AC-17 | integration | `phase5-mileage-endpoint.test.ts` | `dateFrom > dateTo returns 400 with fields[].field === "dateRange"` | T4 | PENDING |
| AC-18 | integration | `backend/test/integration/phase5-mileage-authz.test.ts` | `appliedFilters reports the effective dateFrom/dateTo/ownerId` | T5 | PENDING |
| AC-19 | integration | `backend/test/integration/phase5-mileage-contract.test.ts` | `ignores client-submitted totalKm in query and body (backend authority)` | T6 | PENDING |
| AC-20 | unit(structural) | `phase5-mileage-contract.test.ts` | `mileage module introduces no new ErrorCode beyond the existing union` | T6 | PENDING |
| AC-21 | integration | `phase5-mileage-authz.test.ts` | `unauthenticated request returns 401 with no numeric fields` | T5 | PENDING |
| AC-22 | integration | `phase5-mileage-authz.test.ts` | `mustChangePassword session returns 403 PASSWORD_CHANGE_REQUIRED` | T5 | PENDING |
| AC-23 | integration | `phase5-mileage-endpoint.test.ts` | `normal user passing another ownerId gets 403 and no silent downgrade to own data` | T4 | PENDING |
| AC-24 | integration | `phase5-mileage-authz.test.ts` | `admin with ownerId=U gets only U's statistics` | T5 | PENDING |
| AC-25 | integration | `phase5-mileage-authz.test.ts` | `admin without ownerId defaults to self` | T5 | PENDING |
| AC-26 | integration | `phase5-mileage-contract.test.ts` | `response key set is an exact whitelist (no unitPrice/amount/purpose/displayName)` | T6 | PENDING |
| AC-27 | frontend | `frontend/test/MileageSummarySection.test.tsx` | `renders all five states (loading/empty-zero/error+retry/success/permission-denied)` | T7 | PENDING |
| AC-28 | frontend | `MileageSummarySection.test.tsx` | `shows inline zh-TW range error when dateFrom > dateTo (client and server paths)` | T7 | PENDING |
| AC-29 | frontend | `MileageSummarySection.test.tsx` | `zero result renders "0.00 公里" with explanatory copy, not an error style` | T7 | PENDING |
