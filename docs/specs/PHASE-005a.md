...
| AC-19 | integration | `backend/test/integration/phase5a-travel-complete.test.ts`＋`phase5a-resolve.test.ts` | `AC-19 取整恰兩處（整合層） > segRaw = fuel + etc is rounded once (0.50km × 3 + 0.25km × 2.0000 → 2, not 3)`；`AC-19 取整恰兩處 > calculateTravel is reused verbatim — only the fuelUnitPrice source changed`（另 sha256 雜湊守門） | T8 | GREEN |
