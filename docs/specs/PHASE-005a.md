...
|---|---|---|---|---|---|
| AC-01 | integration | `backend/test/integration/phase5a-fuel-price.test.ts` | `AC-01 建立油價版本 > creates a version for each of the four fuel types (201, DTO shape key by key)` | T3 | PENDING |
| AC-02 | integration | `phase5a-fuel-price.test.ts` | `AC-02 同油種重疊 > same fuelType + same effectiveFrom returns 409 with details.conflictVersion`；`… > concurrent duplicate create resolves to exactly one 201 and one 409 (never 500)` | T3 | PENDING |
| AC-03 | integration | `phase5a-fuel-price.test.ts` | `AC-03 跨油種時間軸獨立 > all four fuel types may share the same effectiveFrom (201 each)`；`… > four concurrent effective versions coexist and are independently retrievable` | T3 | PENDING |
...
| AC-05 | integration | `phase5a-fuel-price.test.ts` | `AC-05 油種列舉封閉 > unknown / empty / null / numeric / array fuelType returns 400 locating fuelType (never 500)` | T3 | PENDING |
| AC-06 | integration | `phase5a-fuel-price.test.ts` | `AC-06 油價欄位驗證（格式層四道＋值域） > ordered gate table: type / literal / scale / magnitude / range each returns its own reason`；`… > boundary positives: 0, "999999.9999", " 30.5 " are accepted (201)` | T3 | PENDING |
...
| AC-18 | unit | `backend/test/unit/fuel-price-engine.test.ts` | `deriveFuelUnitPrice > rounds HALF_UP to an integer NTD per-km price (eight-row exact table)`；`… > 45 ÷ 10 yields 5 — banker's rounding mutant must fail`；`… > uses Decimal.div, never JS floating-point division` | T2 | PENDING |
...
| AC-20 | unit + integration | `fuel-price-engine.test.ts`；`phase5a-travel-complete.test.ts` | `deriveFuelUnitPrice > derived unit price at or beyond the column capacity is reported, never silently truncated`；`AC-20 容量守門 > an overflowing price/consumption combination is rejected per D4, never 500` | T2, T8 | PENDING |
...
| AC-27 | integration | `backend/test/integration/phase5a-migration-safety.test.ts` | `PHASE-005a-T1 migration safety net — existing DB (AC-27) > AC-27 / Done When #1: every pre-existing column is byte-for-byte unchanged after migration`（五表逐欄，T1R2 擴充）；`… > AC-27: the five new columns are NULL (zero backfill) and §8.4 discriminator reads as OLD model`；`… > AC-27: GET the migrated application still returns 200 with a non-null snapshot`；另 `— clean DB > Done When #2` 兩條與 `— account deletion guard > 409 CONFLICT (not 500)` 一條 | T1 | GREEN |
| AC-28 | unit(structural) | `fuel-price-engine.test.ts` | `AC-28 零浮點中介 > PHASE-005a source files contain no Number()/parseFloat/parseInt on the amount path (date-parse whitelist only)`；`… > every Prisma.Decimal construction in this Phase takes a string argument` | T2 | PENDING |
