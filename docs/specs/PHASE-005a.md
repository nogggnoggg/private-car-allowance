...
| AC-36 | integration | `phase5a-fuel-consumption-authz.test.ts`（油耗三欄）＋`phase5a-fuel-price.test.ts`（油價兩欄，T3 已 GREEN）＋自身唯讀欄（T6） | `AC-36 授權矩陣（T5 三欄 × 5 身分 = 15 必要格） > AC-36[$身分 × $欄] → …`——15 條獨立 it（未登入/PCR/一般/管理員/停用 × 寫他人/寫自己/管理讀取；管理員寫他人格含 M-1 鍵集合守門 `201, body 為 { version } 且 version 鍵集合精確等於 §7.2 DTO + state`） | T3, T5, T6 | 部分 GREEN（油價＋油耗欄 GREEN；自身唯讀欄待 T6） |
