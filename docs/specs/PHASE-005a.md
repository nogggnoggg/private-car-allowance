...
| AC-27 | integration | `backend/test/integration/phase5a-migration-safety.test.ts` | `AC-27 舊模型零影響 > every pre-existing column value of a legacy completed travel is bit-identical after migration`；`… > the five new columns are NULL for legacy rows and the DTO still returns a non-null snapshot` | T1 | PENDING |
