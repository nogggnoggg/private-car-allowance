...
| AC-07 | integration | `backend/test/integration/phase5a-fuel-consumption.test.ts` | `AC-07 管理員建立油耗版本 > admin creates a version for a target user (createdById = admin, DB row exists)`；`… > allowed for an isActive=false target user (B-12: historical data maintenance)`；`… > unknown userId returns 404 NOT_FOUND (never 500, message reveals nothing beyond existence)` | T4 | GREEN |
| AC-08 | integration | `phase5a-fuel-consumption.test.ts` | `AC-08 油耗必須大於 0 > kmPerLiter=$label returns 400 locating kmPerLiter with reason 油耗必須大於 0 (never 500, no row created)`（it.each 四列：0／"0"／-1／"-0.0001"）；`… > "0.0001" (smallest representable positive) is accepted` | T4 | GREEN |
| AC-09 | integration | `phase5a-fuel-consumption.test.ts` | `AC-09 依據備註必填 > basisNote=$label returns 400 locating basisNote with reason 請填寫核對依據 (no row created)`（it.each 四列：缺欄位／null／""／全空白）；`… > basisNote of exactly 500 chars is accepted (boundary positive)`；`… > basisNote of 501 chars (over-length) returns 400 locating basisNote (§16 D3(c-1))`；`… > leading/trailing whitespace is trimmed before storage (B-13)` | T4 | GREEN |
| AC-10 | integration | `phase5a-fuel-consumption.test.ts` | `AC-10 同使用者重疊 > same user + same effectiveFrom → PARAMETER_PERIOD_OVERLAP with conflictVersion`；`AC-10 跨使用者互不干涉 > different user + same effectiveFrom succeeds`；`AC-10 並發雙寫 > concurrent duplicate create for the same user + effectiveFrom resolves to exactly one success and one 409 (never 500/uncaught)` | T4 | GREEN |
| AC-11 | integration | `phase5a-fuel-consumption.test.ts` | `AC-11 油耗欄位驗證（格式層四道＋值域） > ordered gate table: $label`（it.each 十九列：18 錯誤列＋" 15.5 " 接受控制列）；`AC-11 油耗欄位驗證 > ordered gate table mirrors the parameter contract, with 油耗必須大於 0 as the range-layer reason (mutant tripwire: swapping in AC-06's 單價不得小於 0 wording must fail this test)` | T4 | GREEN |
| AC-12 | integration | `backend/test/integration/phase5a-fuel-consumption-authz.test.ts` | `AC-12 版本清單（管理員視角） > returns HISTORICAL / CURRENT / FUTURE states relative to today, ascending by effectiveFrom (AR-5: created out of order)`；`… > a user with no versions returns 200 with an empty array (not 404)`；`… > unknown userId → 404 NOT_FOUND` | T5 | GREEN |
| AC-13 | integration | `phase5a-fuel-consumption-authz.test.ts` | 寫入拒絕與零寫入證明由 AC-36 兩格承載：`AC-36[一般使用者 × 油耗寫入(他人)] → 403 FORBIDDEN, zero DB writes`；`AC-36[一般使用者 × 油耗寫入(自己)] → 403 FORBIDDEN (含 userId=自己), zero DB writes`；無側信道：`AC-13 判定順序：授權失敗回應不得因輸入合法與否而有差異 > malformed body as a normal user still returns 403, never 400 (no side channel)`；`… > missing body entirely (no Content-Type/payload) as a normal user still returns 403, never 400/500`；`… > S-1 釘住測試：未登入 + 畸形 JSON body → 400 VALIDATION_ERROR（wire-level body parser 先於 preHandler／授權，屬 CHORE-001/002 已批准之全站行為…）` | T5 | GREEN |
| AC-14 | integration | `phase5a-fuel-consumption-authz.test.ts` | `AC-14 油耗異動稽核 > one AuditLog row per successful create, with actor/target/before(null)/after/basisNote`；`… > AR-5-adjacent discriminator: a new version created EARLIER than an existing version gets before=null (strict '<', not the existing version's own snapshot)`；`… > S-2 真交易回滾證據：真實 Prisma $transaction 下，onCreated（audit）hook 拋出 → create() reject、該 userId 版本列數不變、無對應 AuditLog 列（非 stub，證明真的有回滾）`；`… > stub: onCreated hook throwing propagates the error uncaught, and create() is invoked through the tx client…` | T5 | GREEN |
| AC-15 | integration | `backend/test/integration/phase5a-self-consumption.test.ts` | `AC-15 自身唯讀 — 今日有生效版本 > returns exactly { current: { fuelType, kmPerLiter, effectiveFrom } } for the current version (key-set equality)`；`… > never exposes basisNote / id / createdById / state / userId / other users' data`。**措辭澄清（T6 實作記錄）**：AC-15 敘述之三鍵指 `current` 內層物件；頂層 wire 形狀以 §7.3 為準（`{ current: {...} | null }`，鍵集合封閉）——實作與測試同時驗證頂層＝["current"]、內層恰三鍵 | T6 | GREEN |
| AC-16 | integration | `phase5a-self-consumption.test.ts` | `AC-16 空狀態 > user without any version returns 200 { current: null } (not 404)`；`… > only-future versions also yield current: null` | T6 | GREEN |
| AC-17 | integration | `phase5a-self-consumption.test.ts` | `AC-17 不得成為他人資料通道 > baseline: user1's own response (for byte-identical comparison below)`；`… > query-string userId tampering (?userId=他人) never returns another user's data — response identical to baseline`；`… > query-string ownerId tampering …`；`… > JSON body userId tampering (GET with a body payload) …`；`… > path tampering (/me/fuel-consumption/他人ID) is structurally unreachable — 404, not another user's data`；`… > discriminating control: user2 reading their OWN self endpoint gets user2's data …`。**證據來源註記（T6 複審 SF-2）**：query／path 兩向量由上列測試提供行為鑑別；**body 向量之保證為結構性**（handler 不讀 body＋Fastify 不解析 GET body，wire 探針實證），該條測試僅釘住現狀、非行為鑑別 | T6 | GREEN |
...
| AC-19 | integration | `backend/test/integration/phase5a-travel-complete.test.ts` | `AC-19 取整恰兩處（整合層） > segRaw = fuel + etc is rounded once (0.50km × 3 + 0.25km × 2.0000 → 2, not 3)`；`AC-19 取整恰兩處 > calculateTravel is reused verbatim — only the fuelUnitPrice source changed`（另 sha256 雜湊守門） | T8 | GREEN |
| AC-20 | unit + integration | `fuel-price-engine.test.ts`；`phase5a-travel-complete.test.ts` | 單元層（T2）：`deriveFuelUnitPrice — AC-20 derived unit price at or beyond the column capacity is reported, never silently truncated`（describe 全組）。整合層（T8/T8R）：`AC-20 容量守門 > an overflowing price/consumption combination is rejected per D4, never 500`；`… > SF-2: draft GET exposes parameterAvailable=false + FUEL_UNIT_PRICE_OUT_OF_RANGE via completionBlockers and computed (never amount 0 as final)`；`… > SF-2: preview endpoint exposes parameterAvailable=false + FUEL_UNIT_PRICE_OUT_OF_RANGE (never amount 0 as final)`；T8R 另補 `S-1 修復：草稿/預覽亦暴露 AMOUNT_OUT_OF_RANGE` 兩條與 `AR-2 邊界正例：合法大額完成成功（防守門誤殺）` | T2, T8 | GREEN |
| AC-21 | integration | `phase5a-resolve.test.ts` | `AC-21 缺油耗 > PHASE-005a-T7R SF-1修復: draft's computeCompletionBlockers includes FUEL_CONSUMPTION_NOT_AVAILABLE (owner has no consumption version at all)`；`… > assertParametersAvailable throws 409 with details.missing containing FUEL_CONSUMPTION (message differs verbatim from missing-price)`；`… > another user's effective consumption value never appears in the result (no fallback) — real DB round-trip`。**證據來源註記（T8 複審 A-3）**：blocker 覆蓋為純函式層＋resolve 直呼；missing→DTO 接線由 PHASE-004 既有 completionBlockers 測試泛用覆蓋 | T7 | GREEN |
| AC-22 | integration | `phase5a-resolve.test.ts` | `AC-22 缺該油種油價 > the other three fuel types having prices does not satisfy the owner's fuel type (GASOLINE_95 has no price at this date)`；`… > the 409 message differs verbatim from the missing-consumption message` | T7 | GREEN |
| AC-23 | integration | `phase5a-resolve.test.ts` | `AC-23 代操作以擁有人資料為準 > resolving with the OWNER's id yields the owner's derived price, not the admin's (distinct derived prices)`；`… > admin acting on behalf via completion resolution uses the OWNER's consumption id, never the admin's` | T7 | GREEN |
| AC-24 | integration | `phase5a-travel-complete.test.ts` | `AC-24 後端權威 > body carrying fuelUnitPrice / pricePerLiter / kmPerLiter / totalAmount is ignored, response is the backend value`；`… > malformed injected field VALUES (still a JSON object body) never cause a 500`；`… > a non-object top-level request body (null / string / array) still never causes a 500`。**證據來源註記（T8 複審 A-4）**：query 向量為結構性保證（complete handler 僅讀 params.id 與 currentUser） | T8 | GREEN |
| AC-25 | integration | `phase5a-travel-complete.test.ts` | `AC-25 快照七項＋版本 id > snapshot stores fuelType / pricePerLiter / consumption / derived unit price / etc / raw / total`；`… > recomputing ROUND_HALF_UP(price ÷ consumption) from the snapshot reproduces fuelUnitPrice exactly`；`… > recomputing segment amounts from the snapshot reproduces snapshotAmount exactly` | T8 | GREEN |
| AC-26 | integration | `phase5a-travel-complete.test.ts` | `AC-26 快照不可變 > adding an earlier-effective price or consumption version leaves the completed snapshot bit-identical`（T8R 擴含各段 snapshot* 逐位元）；`… > GET of a completed application performs no parameter resolution`（T8R 改正對照：先 GET DRAFT 計數+1、再 GET 已完成+0） | T8 | GREEN |
...
| AC-29 | frontend | `frontend/test/ParametersPage.test.tsx` | `油價維護（PHASE-005a） > 表單含油種選擇與「每公升油價（元／公升）」欄位`；`油價維護（PHASE-005a） > 版本列表依油種分組呈現，四油種各自時間軸可辨識`；`油價維護（PHASE-005a） > 五態：Loading／Empty／400 就地標示／409 衝突／Success`；`油價維護（PHASE-005a） > Permission denied：非管理員不呼叫寫入 API（POST /parameters/fuel-price 從未被呼叫）`；`油價維護（PHASE-005a） > 鑑別力：建立呼叫精確命中 /api/parameters/fuel-price，而非已凍結之舊制 /api/parameters/fuel` | T9 | GREEN（T9 `eaa4b61`；verbose 逐字回填） |
| AC-30 | frontend | `frontend/test/AdminFuelConsumptionPage.test.tsx` | `管理員油耗維護頁 > 版本列表顯示歷史／目前生效／未來生效與依據備註`；`管理員油耗維護頁 > 建立表單五欄齊備，basisNote 為必填`；`管理員油耗維護頁 — 五態：Loading／Empty／400／404／409／Success／Permission denied`（describe 拆 7 it：Loading／Empty／400 含 basisNote／404／409 衝突生效日／Success 含 state 標示／Permission denied 零寫入呼叫）；附加 `管理員油耗維護頁 > state 不自算：mock 後端回傳 FUTURE 但 effectiveFrom 為過去日期 → 前端仍顯示 FUTURE（§11.3 鑑別）` | T10 | GREEN（T10 `410e7f4`；verbose 逐字回填） |
| AC-31 | frontend | `frontend/test/MyFuelConsumptionSection.test.tsx` | `個人車輛油耗檢視 > 顯示目前生效的油種、油耗與生效日期`；`個人車輛油耗檢視 > 無資料時顯示空白狀態與「請聯絡管理員建立」`；`個人車輛油耗檢視 > 畫面不存在任何新增／修改／刪除入口（負向斷言）`；`個人車輛油耗檢視 > 固定顯示「如與實際車輛不符，請聯絡管理員核對後更新」`；補充五態：`個人車輛油耗檢視 > Loading：載入中顯示「載入中…」`；`個人車輛油耗檢視 > Error：5xx 顯示 zh-TW 錯誤訊息與重試`；`個人車輛油耗檢視 > 401：導向登入（沿用既有 parseApiResponse 慣例）` | T11 | GREEN（T11 `2ed582d`；verbose 逐字回填） |
| AC-32 | frontend | `frontend/test/TravelApplicationPage.test.tsx` | `TravelApplicationPage > 缺參數提示（PHASE-005a） > 僅缺油耗 → 顯示「請聯絡管理員建立油耗資料」`；`TravelApplicationPage > 缺參數提示（PHASE-005a） > 僅缺油價 → 顯示「請聯絡管理員設定參數」`；`TravelApplicationPage > 缺參數提示（PHASE-005a） > 兩者皆缺 → 兩則訊息同時可見且逐字互異`；`TravelApplicationPage > 缺參數提示（PHASE-005a） > 缺油資依據時預覽顯示「油資無法計算」而非金額 0`；A-2 三代碼文案：`TravelApplicationPage > 缺參數提示（PHASE-005a） > 不可計算代碼 FUEL_DATA_CORRUPTED → 顯示對應文案`；`TravelApplicationPage > 缺參數提示（PHASE-005a） > 不可計算代碼 FUEL_UNIT_PRICE_OUT_OF_RANGE → 顯示對應文案`；`TravelApplicationPage > 缺參數提示（PHASE-005a） > 不可計算代碼 AMOUNT_OUT_OF_RANGE → 顯示對應文案`；T11R 補集：`… > 僅缺 ETC → 仍顯示各段與合計油資金額…`；T13R2 SF-1 護欄：五碼 it.each「油資無法計算抑制清單覆蓋 {CODE}」 | T11/T11R/T13R2 | GREEN（T11 `2ed582d`＋T11R `1fcdd60`＋T13R2 `7652f47`） |
| AC-33 | E2E | `e2e/fuel-model.spec.ts` | `油資模型修正 — PHASE-005a Gate E2E（Spec §11.4） > 情境6：響應式375px——油價頁／油耗維護頁／個人區塊body皆無水平溢位`（另五情境同檔全綠；§11.4(5) 之「快照三來源值顯示」子項省略——缺口併 PHASE-008 D10(a)，見 §18） | T13/T13R | GREEN（T13 `c92c42f`＋T13R `1a0ed7a`；全套 23/23 兩輪＋大總管親跑於 dev 拓撲） |
| AC-34 | integration | `backend/test/integration/phase5a-contract.test.ts` | describe `AC-34/AC-35 — PHASE-005a 新增端點錯誤合約`（18 it：fuel-price 401/403/400/409、舊端點三身分 404、fuel-consumption 401/403/400/404/409、/me 401、complete 409 details.missing——逐條統一形狀 Object.keys 封閉集合）；describe `AC-34 日誌安全：logStream 掃描（不含 console.error，見 A-6 獨立驗證）`（6 it：cookie 值／password=與 bearer pattern／SQL 關鍵字與 Prisma 類名／絕對路徑／`basisNote content never appears in application logs (success or 409 path alike)`／錯誤仍有日誌不靜默）；describe `A-6 console.error 路徑之獨立驗證（logStream 掃描不到 console.error）`（`preview endpoint triggers console.error with sanitized args only (no stack/basisNote/raw message)`） | T12 | GREEN（T12 `0c5ab0c`；verbose 逐字回填） |
| AC-35 | unit(structural) | `phase5a-contract.test.ts` | `AC-35 不新增 ErrorCode > errors.ts ErrorCode union equals the known baseline`；`AC-35 不新增 ErrorCode > PHASE-005a source references only baseline ErrorCode literals`（mutant：union 加 BOGUS 成員必紅，已實證） | T12 | GREEN（T12 `0c5ab0c`；verbose 逐字回填） |
| AC-36 | integration | `phase5a-fuel-consumption-authz.test.ts`（油耗三欄）＋`phase5a-fuel-price.test.ts`（油價兩欄，T3 已 GREEN）＋自身唯讀欄（T6） | `AC-36 授權矩陣（T5 三欄 × 5 身分 = 15 必要格） > AC-36[$身分 × $欄] → …`——15 條獨立 it（未登入/PCR/一般/管理員/停用 × 寫他人/寫自己/管理讀取；管理員寫他人格含 M-1 鍵集合守門 `201, body 為 { version } 且 version 鍵集合精確等於 §7.2 DTO + state`）；自身唯讀欄五格（T6）：`AC-36 授權矩陣（自身唯讀欄 × 5 身分 = 5 必要格，T6 補齊） > AC-36[$身分 × 自身唯讀] → …`（含管理員無版本回 current: null 非錯誤） | T3, T5, T6 | GREEN（15+5+油價 6 格全數落地） |
...
| **T11** | 前端：個人油耗檢視 ＋ 差旅缺參數提示 | FE | AC-31, 32 | `frontend/src/components/MyFuelConsumptionSection.tsx`、`frontend/src/pages/HomePage.tsx`、`frontend/src/pages/TravelApplicationPage.tsx`、`frontend/src/api/applications.ts`（**T7 複審 SF-4 補列**：`MissingParameter` union 依 D5(a) 更名為 `FUEL_PRICE`/`FUEL_CONSUMPTION`/`ETC`、`TravelSnapshot` 之 `fuelParameterVersionId` 改 nullable＋新模型雙版本 id 欄位；`TravelApplicationPage.tsx:751` 之 `includes("FUEL")` 判斷同步——現況該提示恆 false 屬 Phase 內過渡態）、`frontend/test/MyFuelConsumptionSection.test.tsx`、`frontend/test/TravelApplicationPage.test.tsx` | Medium | T6, T7 | 負向斷言（無寫入入口）全綠；三情境文字逐字互異全綠；D5(a) 前端契約漂移全數收斂（api 型別＋UI 判斷＋mock） |
...
| 日期 | 版本／狀態 | 內容 | 依據 |
|---|---|---|---|
| 2026-08-04 | `ACTIVE`（Mock/整合 Gate 通過＋三裁定＋T14） | **人類 leonchih 於 Gate 現場裁定（2026-08-04 上午）**：①Gate 走查步驟 1~7 全數通過（新模型表單/油耗維護/個人唯讀/金額 50 元/缺參數擋完成/舊快照 71/40 不變/375px）②快照三來源值前端顯示**追認併 PHASE-008 D10(a)**（§18 前列之待追認就此關閉）③AC-32 文案並列**接受現狀（選項 c）**，PHASE-008 文案總整理時再議④合併方式 **Merge**。**T14（Gate 反饋①，本 Phase 內修）行為定義**：差旅表單存在未儲存變更時點擊「完成申請」，確認框不得以伺服器舊狀態渲染「請填寫出差日期…」等誤導紅字，改顯示「有未儲存的變更，請先儲存草稿」且確認送出停用；無未儲存變更時行為完全不變（完成仍以伺服器已存狀態為準）。此為 PHASE-004 既有誤導行為之修正，經人類 Gate 現場裁定。 | 人類 leonchih Gate 驗收（2026-08-04）；PROJECT_STATE Gate 反饋列 |
| 2026-08-04 | `ACTIVE`（終審處置：T13R2） | **AR-4 去重回退（終審 SF-2/SF-3 處置，大總管裁定）**：T13 引入之「blocker 同碼時抑制前端訊息」去重使 AC-32 定稿文字於主流程不可達（草稿帶 tripDate 時後端 DTO 恆帶 blocker）且 `FUEL_PRICE`/`ETC` 共用粗粒度碼有跨項誤抑制——屬未經 Spec 批准之可見行為變更，**整體回退**（`7652f47`），恢復 T11R 已審狀態（blocker 與前端訊息並列顯示）。**文案並列/重複問題（原合併複審 AR-4）回歸開放項，交人類裁定**：選項 (a) Spec 修訂令後端 blocker 文案承擔顯示（重新引入去重）(b) 統一兩處文案逐字 (c) 接受並列現狀。另 SF-1 護欄（五碼 it.each 抑制清單全覆蓋，AMOUNT_OUT_OF_RANGE 移除 mutant 必紅）同輪落地；§12 AC-32 列補掛 T11R/T13R2。 | 終審報告（SF-1/2/3）；大總管白名單；待晨間人類裁定文案取向 |
| 2026-08-04 | `ACTIVE`（T13/T13R 結案同步） | **§11.4(5)「報表區塊可見快照三來源值」子項處置（大總管裁定，待晨間人類追認）**：前端 `TravelSnapshotDto`/詳情頁現況未接 T8 三快照欄（wire 已回傳），該子項之顯示驗證於 E2E 情境 5 省略；**缺口併入 PHASE-008 D10(a) 既有硬約束**（報表版型條件呈現——新模型顯示油種/每公升油價/油耗/每公里單價，本即人類已批之 008 範疇；§11.4 為測試策略層文字，D10(a) 人類裁定為較高權威）。合併複審 AR-3（型別缺三欄）同此落點，PHASE-008 開工 Packet 必引。另記 T13R 之 E2E 斷言誤植修正（情境 4 blocker 逐字文案、情境 5 ETC 動態單價）與三檔新模型播種擴充（AC-37(e) 預告落點之落實）。 | T13/T13R Handoff；T9~T12 合併複審 AR-3；D10(a) 批准原文；大總管白名單 |
...
| 2026-08-04 | `ACTIVE`（T8R 結案同步） | **T8R-LITE（`c3545b6`）之公開值域擴充記錄（T8R 複審 SF-1）**：即審 S-1 修復使 `AMOUNT_OUT_OF_RANGE` 進入兩個對外欄位——`computed.missingParameters`（`PreviewMissingCode`）與 `completionBlockers[].code`——屬 T8 已追認之 AR-2 處置（409 `details.missing`）× SF-2 暴露模式（預覽/草稿不可計算）之**合併套用，非新產品決策**。§16 D5 之 wire 值域自此為：`FUEL_PRICE`／`FUEL_CONSUMPTION`／`ETC`／`FUEL_UNIT_PRICE_OUT_OF_RANGE`／`FUEL_DATA_CORRUPTED`／`AMOUNT_OUT_OF_RANGE`（前三者為缺參數、後三者為不可計算原因——T11 前端接用時之權威清單）。另 S-2 spy 正對照、S-3 段快照斷言、AR-2 邊界正例（1 億完成成功）同輪落地。複審 AR-a（totalAmount 外之歸零斷言）、AR-b（S-3 自證前提）列後續批次；AR-c/AR-d 併 T11 文案清單。 | T8R 複審報告；大總管白名單 |
| 2026-08-04 | `ACTIVE`（T8 結案同步） | **AR-2 金額容量守門之處置追認（大總管裁定）**：T8 新增 `assertCalculationWithinStorageCapacity`——段金額/整筆金額/總里程寫入前檢查 `Decimal(14,4)`/`Decimal(12,2)`/int4 容量，超出→409 `PARAMETER_NOT_AVAILABLE`（`details.missing=["AMOUNT_OUT_OF_RANGE"]`，複用既有 ErrorCode）。裁定依據：與 D4(a) 已批處置**同型**（值域 vs 欄位容量→409 可行動錯誤、絕不 500、絕不靜默截斷），implementer 紅燈實測證明未守門時為真實 Postgres INT4 500（`Unable to fit integer value '99999899990000' into an INT4`）；屬已批決策之下游技術延伸，非新產品決策。**SF-2 關閉**：預覽/草稿 DTO 於 out-of-range 與 data-corrupted 兩情境暴露不可計算（parameterAvailable=false＋missingParameters 追加代碼＋blocker），AC-20 整合層落地。 | T8 Handoff；D4(a) 批准原文；大總管白名單 |
| 2026-08-03 | `ACTIVE`（T7 結案同步） | **T7 實際檔案集合補記（複審 SF-5）**：§15 T7 列宣告 5 檔，實際 16 檔（`eb24152`）——超出部分為 D5(a) 破壞性更名之 fixture 機械傳播至 6 個清單外測試檔（`phase5-mileage-engine`、`phase4-admin-on-behalf`、`phase4-concurrency-freeze`、`phase4-on-behalf-audit`、`phase4-travel-draft`、`phase4-reference-protection`），reviewer 逐檔 diff 驗證**零斷言變更**；另 4 個 D5(a) 清單內檔之斷言更新屬 Spec 授權之再定向。**T11 檔案清單修訂（複審 SF-4）**：補列 `frontend/src/api/applications.ts`（見 §15 T11 列）——D5(a) 前端契約漂移（`includes("FUEL")` 恆 false）為 Phase 內已知過渡態，T11 收斂。SF-2（預覽未暴露 out-of-range）逐字轉 T8 Packet 必辦。 | T7 合併即審報告（0 Must/5 Should/7 AR）；大總管白名單 |
