...
|---|---|---|---|---|---|
| AC-01 | integration | `backend/test/integration/phase7-migration-safety.test.ts` | `AC-01(a): clean DB — DepreciationApplication columns match §8.2 exactly, with Decimal round-trip`；`AC-01(b): existing DB — every pre-existing column across all eight tables is byte-for-byte unchanged`；`AC-01(c): the new table is created empty and Application receives zero ALTER`；`AC-01(d): migrate deploy succeeds on both clean and existing DB`；`AC-01(e): enum unions exactly match the pre-T1 baseline`；`AC-01(f): infra001 business-table count is 15` | T1 | PENDING |
