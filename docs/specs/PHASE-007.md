...
| AC-08 | integration | `phase7-depreciation-year.test.ts` | 建立面實名：`AC-08` describe（皆 201＋唯一約束等效 mutant 靜態論證成立）。**「兩筆皆可完成（皆 200）」待 T9**（完成端點尚不存在；T8 即審 SF-2） | T5 ＋ T9 | **PARTIAL**（建立面 GREEN，完成面待 T9） |
...
| AC-13 | integration | `phase7-annual-mileage.test.ts` | `AC-13: a null snapshot row is counted by applicationCount but ignored by totalKm; only totalKm feeds the amount`（describe 逐字實名）。**「金額只用 totalKm」正向面待 T7**（本檔零播種參數版本，金額欄恆 null——T6 即審 S-1；T7 須播種版本＋補正向鑑別） | T6 ＋ T7 | **PARTIAL**（不對稱行為 GREEN，金額條款待 T7） |
| AC-14 | integration | `phase7-annual-mileage.test.ts` | `AC-14: ownerId is always the application owner; preview resolves ownerId fail-closed (403 for another user) and authorises before field validation`（describe 逐字實名）。**草稿 DTO 側待 T7**（computed 尚未接入 DTO） | T6 ＋ T7 | **PARTIAL**（預覽/服務面 GREEN，DTO 側待 T7） |
...
| AC-16 | unit ＋ integration | `backend/test/unit/depreciation-blockers.test.ts`；`phase7-depreciation-parameters.test.ts` | 判定層實名：`AC-16(b): PARAMETER_NOT_AVAILABLE blocker is produced when no effective version exists`。整合層（`AC-16(a)(c)` 完成 409＋details.missing＋零寫入；草稿/預覽同步暴露）待 T7 | T3 ＋ T7 | **PARTIAL**（判定層 GREEN） |
...
| AC-18 | unit ＋ integration | `depreciation-blockers.test.ts`；`phase7-depreciation-parameters.test.ts` | 判定層：兩來源（查無版本／推導失敗）於本層折為同一碼之全等測試在場（`depreciation-blockers.test.ts` §4）；**409 訊息逐字互異＋details.missing 追加碼之區辨義務全落 T7（T3 即審 FW-6，本層零區辨資訊）** | T3 ＋ T7 | **PARTIAL**（判定層 GREEN） |
...
| AC-24 | unit ＋ integration | `depreciation-blockers.test.ts`；`phase7-depreciation-complete.test.ts` | 判定層實名：AC-24 判定層 0/1/5 張測試（`depreciation-blockers.test.ts` §2）。整合層（完成交易內 FOR UPDATE 後 count 現算）待 T9（T3 即審 FW-10：判定層守門不得作為交易外快照安全之理由） | T3 ＋ T9 | **PARTIAL**（判定層 GREEN） |
| AC-25 | integration | `phase7-depreciation-attachment.test.ts` | describe 逐字實名：`AC-25 完成後鎖定（deriveContainerState DEPRECIATION 分支）`（7 條；修復前紅燈 3 條 200→403 實證＋reviewer 獨立重現）。真實完成流程之端到端重驗待 T9（T8 即審 FW-4） | T8 | GREEN（fixture 面；T9 補真實流程重驗） |
