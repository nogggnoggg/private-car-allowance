...
> **回填完成記錄（`PHASE-007-DOC-SYNC-REV`，2026-08-05）**：§12R.2 之 17 列與 §12R.3 之 15 列全數回填完畢，`PENDING` 於本兩表零殘留。實名來源為本 Task 於基準 commit `1def72e` 親跑之 `npx vitest run --reporter=verbose`——backend **2571 passed / 100 files**、frontend **238 passed / 18 files**（與大總管基準線一致）。**AC-45／AC-46 為 Playwright E2E，非本 Task 執行**：其實名取自 `e2e/depreciation-allowance.spec.ts` 之測試宣告，綠燈證據為大總管親跑之 **38/38**（commit `4fb6dc9`），該兩列已於狀態欄明示標註證據來源。
...
| AC | 受影響原因 | Task | 狀態 |
|---|---|---|---|
| AC-01 | §8.2 欄位集合變更（新增 4 欄）＋ `DepreciationParameterVersion.estimatedAnnualKm` 轉 nullable ⇒ **ALTER 型 M2 migration**；安全網 fixture 須依 **T1 FW-8** 改 raw SQL 寫法；業務表總數**不變（15）** | R1 | `GREEN` — 實名：`phase7-migration-safety.test.ts` 之 `PHASE-007-R1 M2 migration safety net — existing DB (M1 applied, M2 pending)`（AC-01 共 7 條：`八表逐欄零改寫 — every pre-existing column across all eight tables is byte-for-byte unchanged after M2`／`DepreciationApplication 既有列 — 原 10 欄逐欄零改寫，新增 4 欄一律 NULL（零回填；多餘 ADD COLUMN 必紅）`／`DepreciationParameterVersion 既有列 — 鍵集全等且 estimatedAnnualKm 原值保留（nullable 轉換不動資料）`／`estimatedAnnualKm 於既有 DB 上確實轉為 NULLABLE，且可寫入 NULL 之新版本`／`AC-01 突變自證: ALTER 型比較器具鑑別力…各自必紅`／冪等重跑）＋ `PHASE-007-R1 M2 migration safety net — clean DB`（AC-01 共 9 條：`業務表總數仍為 15（M2 零建表、零刪表）`／`DepreciationApplication 欄集合／nullability／精度逐欄符合 Spec §20.7.1（14 欄）`／`AC-01 精度往返: Decimal(9,1)／(9,6)／(12,2) 邊界值逐位元往返`／`M2 SQL 靜態檢查 — 僅 ADD COLUMN ×4 ＋ DROP NOT NULL ×1；零 DROP COLUMN／零回填／零 enum／零建表／零索引異動`／`AC-01 drift guard: prisma migrate diff --exit-code ＝ 0`／`enum 聯集全等`） |
| AC-02 | 建立端點新增 `annualTotalKm`（**應予採用**，S-3）；「全部快照欄為 `null`」之逐欄斷言須含新增 4 欄 | R5 | `GREEN` — 實名：`phase7-depreciation-draft.test.ts` 之 `AC-02: POST /applications/depreciation creates a DRAFT with all-null snapshot columns and ignores body-supplied ownerId/status/totalAmount`（5 條；含 `空 body → completionBlockers ＝ [YEAR_REQUIRED, ANNUAL_TOTAL_KM_REQUIRED]（§20.5.1 兩段式第一段，順序穩定）` 與 `body 夾帶 ownerId／createdById／status／totalAmount → 一律不採用（DB 層斷言）`） |
| AC-05 | `DepreciationApplicationDto`／`ComputedDto`／`SnapshotDto` 三形狀全部變更（§20.6） | R5 ＋ R6 | `GREEN` — 實名：`phase7-depreciation-draft.test.ts` 之 `AC-05: GET returns blockers+computed for DRAFT and snapshot for COMPLETED; DELETE detaches DEPRECIATION attachments in the same transaction; wrong-type id is 404`（10 條；含 `GET COMPLETED（新模型快照欄已寫入）→ 200，snapshot 非 null（model=CURRENT）`／`GET COMPLETED（舊模型列）→ snapshot.model=LEGACY，perKmUnitPrice 為原值、新模型四欄為 null（AC-56(a)(c)）`／`§16 D8／§20.6 揭露面不變式：DTO 之 snapshot 不外露 vehiclePrice／usefulLifeYears／estimatedAnnualKm／版本 id`） |
| AC-18 | 推導失敗之來源函式改為 `deriveAnnualDepreciation`（兩參數）；`estimatedAnnualKm` 不再是失敗來源 | R4a ＋ R6 | `GREEN` — 實名：`depreciation-blockers.test.ts` 之 `computeDepreciationBlockers — AC-16(b)/AC-18 缺參數（第 4 項）`（4 條；含 `AC-18: deriveAnnualDepreciation ok:false is mapped to the same PARAMETER_NOT_AVAILABLE blocker…` 與 `AC-18 絕不以每年費用 0 完成：\`annualDepreciation=null\` 不得被當成 \`0\` 而靜默通過（零 blocker）`）＋ `phase7-depreciation-parameters.test.ts` 之 `AC-49(e): deriveAnnualDepreciation returning ok:false is treated as a missing parameter — never 500, never a zero annual expense`（5 條） |
| AC-21 | `ROUND_HALF_UP` 紀律不變，但 kill pair 之輸入組須依新公式重建（§20.4.3 已給定數對） | R2 | `GREEN` — 實名：`depreciation-calculation.test.ts` 之 `calculateDepreciation — AC-21 rounding is ROUND_HALF_UP (away from zero), never ROUND_HALF_EVEN`（4 條；§20.4.3 kill pair row 1 鑑別列 ＋ row 2 配對列） |
| AC-22 | 容量欄位集合變更：`snapshotPerKmUnitPrice` 退出判定、新增 `snapshotAnnualDepreciation`／`snapshotAnnualTotalKm`／`snapshotRatio`；常數表重整（零魔術數紀律不變） | R2 ＋ R7 | `GREEN` — 實名：`depreciation-calculation.test.ts` 之 `isDepreciationCalculationOverCapacity — AC-22(a)(d) §20.7.2 判定集合重整（六項），boundary positives included (no false kills, no silent truncation)`（25 條；含 `each bound equals 10^(precision − scale) of its target column (零魔術數，AC-22(a))`）＋ `depreciation-blockers.test.ts` 之 `computeDepreciationBlockers — AC-22(c)(d) 容量（第 6 項）`（7 條）＋ `phase7-depreciation-complete.test.ts` 之 `PHASE-007-T9b — AC-22 容量守門三處一致`（4 條；含 `AC-22(d) 防誤殺對照組…完成 200 且金額精確等值`） |
| AC-27 | §9.3 步驟②之附件計數守門退場、新增比例守門；②③④順序紀律不變（§20.8） | R7 | `GREEN` — 實名：`phase7-depreciation-complete.test.ts` 之 `AC-27 完成流程與 §20.8.2 步驟順序`（8 條） |
| AC-30 | 原子性語意不變，但「快照全 `null`」之逐欄回滾斷言須含新增 4 欄 | R7 | `GREEN` — 實名：`phase7-depreciation-complete.test.ts` 之 `AC-30 原子性`（3 條） |
| AC-31 | 「8 欄逐位元不變」→「**9 欄**逐位元不變」；spy 零重算之被監視函式改為 `deriveAnnualDepreciation` | R8 | `GREEN` — 實名：`phase7-snapshot-immutability.test.ts` 之 `AC-31 事後改參數，歷史不變`（8 條） |
| AC-32 | 夾帶矩陣變更：新增 `ratio`／`ratioPercent`／`annualDepreciation` 為**不採用**；`annualTotalKm` 為**採用之例外**，須與 AC-48 同場對照（同一請求一採一不採） | R8 | `GREEN` — 實名：`phase7-snapshot-immutability.test.ts` 之 `AC-32 後端為唯一權威 — 建立端點（POST /applications/depreciation）`（27 條）／`AC-32 後端為唯一權威 — 更新端點（PUT /applications/depreciation/:id）`（26 條）／`AC-32 後端為唯一權威 — 完成端點（POST /applications/:id/complete）`（28 條），共 81 條；`annualTotalKm` 之「採用之例外」由同檔 `AC-48: annualTotalKm is 申請資料 and IS adopted from the request body…（same-request contrast matrix）` 三條同場承載 |
| AC-35 | 代修改稽核 `summary` 之前後值須含 `annualTotalKm`（欄位摘要完整性） | R8 | `GREEN` — 實名：`phase7-on-behalf.test.ts` 之 `AC-35 代修改 — 管理員 PUT 他人草稿 → 200 且寫 APPLICATION_UPDATED_ON_BEHALF`（4 條；含 `前後值忠實反映兩個方向：null → 值、值 → null（applicationYear 與 annualTotalKm 兩欄同步驗證）` 與 `R8b 鑑別：annualTotalKm 之前值取自**交易內新鮮讀取**，且 key 缺席時前後值相同`） |
| AC-38 | 表單由「僅年度一欄」改為「年度 ＋ 年度總里程兩欄」；年度公務里程無輸入欄之負向斷言**保留** | R9 | `GREEN` — **本列實名由三條 AC-58 測試承載**（`DepreciationApplicationPage.test.tsx`）：`AC-58：表單提供申請年度與該車年度總里程兩個可輸入欄位（逐字標籤互異）`（內含年度公務里程無 textbox 之負向斷言）／`AC-58：兩者皆可為空值儲存（null 為合法值，前端轉為 JSON number｜null）`／`AC-58：填入年度與年度總里程後皆以 JSON number（非字串）送出`。本 AC 不另立同名測試，避免同一行為兩份斷言 |
| AC-40 | 五態語意變更：`Empty`（公務里程 0 但總里程有值 ⇒ 金額 0 合法）與「未輸入總里程 ⇒ 不可計算、不顯 0 元」為**兩種不同狀態**，須各自有測試 | R10 | `GREEN` — 實名：`DepreciationApplicationPage.test.tsx` 之 `AC-40：五態`（7 條）；兩狀態各自成條：`Empty：年度無任何有效差旅（totalKm=0.00、applicationCount=0）時顯示 0 值＋說明，非錯誤、非空白頁` 與 `R10／AC-40／AC-53(d)：未輸入年度總里程時顯示「無法計算」，不得出現金額 0（與 Empty 態互異）`；另含 `B-10（終審 SF-3）：年度里程 > 0 但每年折舊費用極小…補貼 0 元非錯誤`、`Error：讀取草稿 500…重試`、`blockingCodes 驅動顯示：未知代碼原樣顯示` |
| AC-41 | 缺參數語意不變，但「條件齊備 ⇒ 完成鈕可用」之前置條件須補填 `annualTotalKm` | R10 | `GREEN` — 實名：`DepreciationApplicationPage.test.tsx` 之 `AC-41：條件齊備（無 blocker）時「完成申請」鈕可用，點擊後呼叫完成端點`／`AC-41：缺參數時顯示聯絡管理員文案、停用「完成申請」，但「儲存草稿」仍可用`／`R10／AC-41：僅缺年度總里程時仍停用「完成申請」，「儲存草稿」不受影響`（3 條） |
| AC-42 | 「完成前無證明時完成鈕停用」條款退場（證明選填）；重複提醒、上限 5、已完成無入口三條**保留** | R10 | `GREEN` — 實名：`DepreciationApplicationPage.test.tsx` 之 `AC-42：重複提醒與證明`（9 條；含 `AC-54(b)：完成前無證明 → 完成鈕仍可用（證明改選填，裁定②反轉）；草稿仍可儲存`、`已有 5 張再上傳（第 6 張）→ 前端拒絕並顯示上限訊息、零上傳呼叫`、`已完成之折舊申請不存在任何上傳／刪除入口（負向斷言）`） |
| AC-45 | E2E 流程新增「輸入年度總里程」步驟、證明改選填、預覽與完成詳情改驗五值 | R12 | `GREEN`（**E2E 由大總管親跑，非本 Task 執行**：`4fb6dc9`，38/38 全綠）— 實名：`e2e/depreciation-allowance.spec.ts` 之 `年度折舊補貼（新模型）— PHASE-007-R12 Gate E2E` 情境 1~7：`情境1：完整走新模型流程——輸入年度總里程、不上傳證明即完成、快照五值一致`／`情境2：年度總里程小於年度公務里程（比例>100%）擋完成並顯示提示`／`情境3：未輸入年度總里程——不顯示 0 元，完成鈕停用`／`情境4：管理員改參數後，已完成申請之快照五值逐字不變`／`情境5：再次嘗試修改已完成之折舊申請→403`／`情境6：列表出現已完成折舊列且金額一致`／`情境7：管理員參數頁折舊參數區塊縮欄——建立三欄＋歷史唯讀「—」` |
| AC-46 | 響應式四畫面之版型因新增欄位與五值列而改變 | R12 | `GREEN`（**E2E 由大總管親跑，非本 Task 執行**：`4fb6dc9`，38/38 全綠）— 實名：`e2e/depreciation-allowance.spec.ts` 之 `情境8：響應式 375px——折舊表單／預覽／證明／列表／已完成詳情／參數頁皆無水平溢位`（覆蓋面多於條文四畫面，另含參數頁） |

### 12R.3 新增列（修訂段新 AC；條文見 §20.3）

| AC | 層級 | 測試檔（預定路徑） | 測試名（預定名稱） | Task | 狀態 |
|---|---|---|---|---|---|
| AC-47 | integration | `backend/test/integration/phase7-depreciation-draft.test.ts` | `AC-47: annualTotalKm validation gate table — type/decimal-places(≤1)/≤0/capacity, with fields[].field asserted`（建立與更新兩端點各一組） | R5 | `GREEN` — 實名與預定名**逐字相同**（`phase7-depreciation-draft.test.ts`，該 describe 下 62 條）；另含判定順序三條 `AC-47(f)：未登入 ＋ 畸形 annualTotalKm → 401（認證先於一切）`／`AC-47(f)：授權先於格式驗證——他人以畸形 annualTotalKm PUT → 403（不得洩漏為 400）`／`AC-47(f)：已完成申請 ＋ 畸形 annualTotalKm → 403（狀態機守門先於 400 格式驗證）`，全檔 AC-47 命中 101 條 |
| AC-48 | integration | `phase7-depreciation-draft.test.ts`；`phase7-snapshot-immutability.test.ts` | `AC-48: annualTotalKm is 申請資料 and IS adopted from the request body, while officialKm/ratio/amount are never adopted (same-request contrast matrix)` | R5 ＋ R8 | `GREEN` — 實名與預定名**逐字相同**；`phase7-depreciation-draft.test.ts` 該 describe 4 條（含 `AC-48(a)：POST 之 annualTotalKm 寫入 DB 之值與請求值逐位元相同，DTO 為固定 1 位小數字串`／`AC-48(b)：POST 同一請求夾帶 annualTotalKm（採用）＋ officialKm／ratio／amount 等（一律不採用）→ 與未夾帶之對照組逐位元相同`）＋ `phase7-snapshot-immutability.test.ts` 3 條（建立／更新／`AC-48(b) 同一請求對照矩陣 — 完成端點：七欄夾帶一律不採用，快照仍為後端權威值`） |
| AC-49 | unit ＋ integration | `backend/test/unit/depreciation-engine.test.ts`；`phase7-depreciation-parameters.test.ts` | `AC-49: deriveAnnualDepreciation returns 車價÷年限 at 2dp and rejects ≤0 inputs`；`AC-49: the application path calls deriveAnnualDepreciation and never deriveDepreciation (import wiring + live mutation self-proof + negative spy)`；`AC-49 凍結證明: deriveDepreciation 之既有測試零改動全綠` | R4a ＋ R6 | `GREEN` — 實名：`depreciation-engine.test.ts` 之 `AC-49: deriveAnnualDepreciation returns 車價÷年限 at 2dp and rejects ≤0 inputs`（9 條）／`AC-49(a) deriveAnnualDepreciation — 取整為 ROUND_HALF_UP，非銀行家取整`（3 條）／`AC-49(a) …非有限值與不可解析輸入一律 ok:false，絕不拋錯`（8 條）／`AC-49(b) deriveAnnualDepreciation 與 deriveDepreciation 共用單一內部推導`（13 條，即凍結證明）；`phase7-depreciation-parameters.test.ts` 之 `AC-49: annualDepreciation comes verbatim from deriveAnnualDepreciation (2dp); deriveDepreciation is never called on the application path`（5 條，含 `AC-49(c) 接線斷言…不再 import deriveDepreciation`／`AC-49(c) 活體突變自證…引數恰為所選版本之車價與年限`／`AC-49(c) 負向 spy：預覽 → 草稿 DTO → 完成之全程，deriveDepreciation 呼叫次數恰為 0`）。全庫 AC-49 命中 45 條 |
| AC-50 | unit | `backend/test/unit/depreciation-calculation.test.ts` | `AC-50: ratio = officialKm ÷ annualTotalKm at full precision; ratioString(6dp)/ratioPercentString(4dp) are display-only and never feed the amount (mutant: amount computed via .times(ratio) must go red)` | R2 | `GREEN` — 實名：`depreciation-calculation.test.ts` 之 `calculateDepreciation — AC-50(a)(b) ratio = officialKm ÷ annualTotalKm at full precision; ratioString(6dp)/ratioPercentString(4dp) are display-only`（6 條）／`calculateDepreciation — AC-50(c) the ratio NEVER feeds the amount (kill case for the \`.times(ratio)\` mutant)`（4 條；含 `AC-50(c) 結構性斷言（第二道防線）：the implementation never multiplies by the ratio nor by its display strings`）／`calculateDepreciation — AC-50(d) triple guard: non-finite inputs and annualTotalKm ≤ 0 (never divides by zero, never returns NaN/Infinity, never silently returns 0, never throws)`（18 條）。全檔 AC-50 命中 29 條 |
| AC-51 | unit ＋ integration | `depreciation-calculation.test.ts`；`phase7-depreciation-parameters.test.ts` | `AC-51 PRIMARY kill case — A=100000.00 × O=1001.66 ÷ I=12345.6 → raw=8113.4979263867288751 → 8113 (mutant ratio6→8114, ratio4→8110, raw2→8114, okmInt→8116 each must go red)`；`AC-51 source-level scan: exactly one toDecimalPlaces(0) call exists in the implementation` | R2 ＋ R6 | `GREEN` — 實名：`depreciation-calculation.test.ts` 之 `calculateDepreciation — AC-51(c) §20.4.3 PRIMARY kill case discriminates ALL FOUR rounding-layer mutants`（5 條）／`calculateDepreciation — AC-51(c) §20.4.3 SECONDARY kill case (independent discrimination of a pre-rounded ratio)`（2 條）／`calculateDepreciation — AC-51(a) rawAmount = annualDepreciation × officialKm ÷ annualTotalKm (multiply-then-divide, unrounded), amount = ROUND_HALF_UP(raw, 0)`（10 條）／`calculateDepreciation — AC-51(b)(d) rounding happens exactly once (source-level scan)`（4 條；含 `AC-51(d) source-level scan: exactly one toDecimalPlaces() call exists in the implementation (the final amount)`）／`AC-51(a) 零浮點中介 — PHASE-007 source files contain no Number()/parseFloat/parseInt on the amount path`（20 條）；`phase7-depreciation-parameters.test.ts` 之 `AC-51: the §20.4.3 kill cases hold end-to-end (rounding happens exactly once, at the final amount)`（7 條）。全庫 AC-51 命中 49 條 |
| AC-52 | unit ＋ integration | `backend/test/unit/depreciation-blockers.test.ts`；`phase7-depreciation-parameters.test.ts`；`phase7-depreciation-complete.test.ts` | `AC-52: officialKm > annualTotalKm produces OFFICIAL_KM_EXCEEDS_ANNUAL_TOTAL_KM with field=annualTotalKm`；`AC-52 邊界: ratio 恰 100% (O == I) is allowed to complete`；`AC-52: the blocker is exposed identically in draft DTO, preview and completion` | R3 ＋ R6 | `GREEN` — 實名：`depreciation-blockers.test.ts` 之 `computeDepreciationBlockers — AC-52 比例守門（第 5 項）`（6 條；含 `AC-52(a): officialKm > annualTotalKm produces OFFICIAL_KM_EXCEEDS_ANNUAL_TOTAL_KM with field=annualTotalKm`／`AC-52(b) 邊界: ratio 恰 100%（\`officialKm == annualTotalKm\`）is allowed to complete（\`>\` 改 \`>=\` 必紅；M9 之直接對象）`／`AC-52(c): 比較以兩里程之 \`Decimal\` 直接進行——取整後之 ratio 看似恰 100% 者仍須擋（M11）`）；`phase7-depreciation-parameters.test.ts` 之 `AC-52: officialKm > annualTotalKm is blocked identically in the draft DTO, the preview, and at completion`（3 條；含 `AC-52(d) 三處一致…同步暴露 OFFICIAL_KM_EXCEEDS_ANNUAL_TOTAL_KM 且 calculable=false`）。全庫 AC-52 命中 11 條 |
| AC-53 | unit ＋ integration ＋ frontend | `depreciation-blockers.test.ts`；`phase7-depreciation-parameters.test.ts`；`DepreciationApplicationPage.test.tsx` | `AC-53: a null annualTotalKm saves as a draft, yields ANNUAL_TOTAL_KM_REQUIRED + calculable=false, and blocks completion with 400`；`AC-53: 未輸入年度總里程時畫面顯示「無法計算」，不得出現金額 0` | R3 ＋ R6 ＋ R10 | `GREEN` — 實名：`phase7-depreciation-parameters.test.ts` 之 `AC-53: a draft without annualTotalKm saves fine, previews as not-calculable, and is blocked at completion with zero writes`（3 條：`AC-53(a)(b)：草稿寬容（201）＋ computed.calculable=false ＋ [ANNUAL_TOTAL_KM_REQUIRED]，且 amount／rawAmount／ratio 皆 null`／`AC-53(b)：預覽 body 未帶 annualTotalKm → 同樣 [ANNUAL_TOTAL_KM_REQUIRED]，且不查詢年度里程（§20.8.1 逐字）`／`AC-53(c)：完成端點 → 400 VALIDATION_ERROR ＋ fields[annualTotalKm] ＋ details.blockers[]，且零寫入（status 仍 DRAFT、全部快照欄仍 null）`）；`depreciation-blockers.test.ts` 5 條（含 `AC-53(e) 保守失敗：非有限 \`annualTotalKm\`（\`NaN\`／\`±Infinity\`）一律 \`ANNUAL_TOTAL_KM_INVALID\`（M4）`）；`phase7-depreciation-draft.test.ts` 3 條；前端 `R10／AC-40／AC-53(d)：未輸入年度總里程時顯示「無法計算」，不得出現金額 0（與 Empty 態互異）` |
| AC-54 | integration | `phase7-depreciation-complete.test.ts`；`phase7-contract.test.ts` | `AC-54: a draft with zero attachments completes successfully (200) and writes the full snapshot`；`AC-54: DEPRECIATION_ATTACHMENT_REQUIRED no longer appears in any response nor anywhere in src (structural scan)`；`AC-54: the 5-attachment limit still applies` | R7 | `GREEN` — 實名：`phase7-depreciation-complete.test.ts` 之 `AC-54 折舊證明改選填（零附件亦可完成）`（2 條：`AC-54(a)：草稿零附件可建立(201)可更新(200) → **完成成功(200)** ＋ 9 欄快照完整寫入`／`AC-54(e) 反轉：交易外曾有 1 張證明、完成前被刪 → 完成仍**成功 200**（附件計數已非 blocker 來源）`）／`AC-54(b) — DEPRECIATION_ATTACHMENT_REQUIRED 之退場（後端 src 全庫結構性掃描）`（含 `後端 src 全庫掃描該字面值命中數 ＝ 0` 與 `草稿 DTO／預覽／完成三處之回應皆不含…（含 400 回應本文之字面掃描）`）／`經真實完成端點完成之折舊，其證明附件 DELETE → 403（AC-25／AC-54(d) 真實流程重驗）`；`depreciation-blockers.test.ts` 之 `AC-54(b) 退場碼於本檔零殘留…（M13）`。**上限 5 仍適用**之實名落於 `phase7-contract.test.ts` 之 `409 TOO_MANY_ATTACHMENTS (6th attachment exceeds AC-23 limit of 5) — unified shape, no fields/details` 與 `phase7-depreciation-attachment.test.ts`（AC-23 續 `GREEN`，不受影響） |
| AC-55 | integration | `phase7-depreciation-complete.test.ts` | `AC-55 快照欄位與精度（9 欄）— snapshotVehiclePrice/UsefulLifeYears/AnnualDepreciation/OfficialKm/AnnualTotalKm/Ratio/RawAmount/calculatedAt/depreciationParameterVersionId, with 三來源逐位元重算`；`AC-55: the two frozen legacy columns (snapshotEstimatedAnnualKm, snapshotPerKmUnitPrice) are always NULL for new-model rows` | R7 | `GREEN` — 實名：`phase7-depreciation-complete.test.ts` 之 `AC-55 快照欄位與精度（9 欄）`（5 條）：`AC-55(a)(b)：9 個快照欄全部寫入且值正確；舊二欄（EstimatedAnnualKm／PerKmUnitPrice）恆 null`／`AC-55(c) 顯式定精度：六個 Decimal 快照欄之小數位與 §20.7.1 DDL scale 逐欄相符（不裁尾零）`／`AC-55(d)：三來源（annualDepreciation／officialKm／annualTotalKm）行內重算逐位元還原 rawAmount 與 totalAmount`／`AC-55(d) kill case：rawAmount 量化為 1234.5000 但 totalAmount 為 1234（amount 絕不由 4dp rawAmount 反推）`／`AC-55(e)：totalAmount／rawAmount 絕不經 snapshotRatio 重算（量化後之比例會差 0.04 元）` |
| AC-56 | integration | `phase7-depreciation-complete.test.ts`；`phase7-migration-safety.test.ts` | `AC-56 新舊模型判別: snapshotAnnualTotalKm != null ⇒ new model; snapshotPerKmUnitPrice != null && snapshotAnnualTotalKm == null ⇒ legacy model (raw-SQL seeded legacy row is rendered read-only and never recomputed)` | R1 ＋ R7 | `GREEN` — 實名：`phase7-depreciation-complete.test.ts` 之 `AC-56 新舊模型判別`（2 條：`AC-56(a)(b)(c)：經完成流程產生之列為新模型 → snapshotAnnualTotalKm != null、DTO model=CURRENT、perKmUnitPrice 為 null`／`AC-56(d)：raw SQL 播種之舊模型列 → DTO model=LEGACY、perKmUnitPrice 為原值、新模型四值皆 null，且**零重算**（spy 呼叫增量 0）`）；`phase7-migration-safety.test.ts` 之 `AC-56(d) 來源: FW-8 raw SQL fixture 之列於 M2 後滿足 §20.7.4 舊模型判別；草稿列為兩者皆 null`；`phase7-depreciation-draft.test.ts` 之 `GET COMPLETED（舊模型列）→ snapshot.model=LEGACY，perKmUnitPrice 為原值、新模型四欄為 null（AC-56(a)(c)）` |
| AC-57 | integration | `backend/test/integration/phase3a-parameter-depreciation.test.ts` | `AC-57 折舊參數縮欄（凍結式）: POST no longer accepts estimatedAnnualKm (夾帶不採用，欄位寫 NULL) and no longer returns perKmUnitPrice`；`AC-57: GET still returns the original estimatedAnnualKm/perKmUnitPrice for legacy versions (read-only) and null for new versions` | R4b ＋ R4c | `GREEN` — 實名與預定名**逐字相同**（`phase3a-parameter-depreciation.test.ts`：`AC-57 折舊參數縮欄（凍結式）: POST no longer accepts estimatedAnnualKm (夾帶不採用，欄位寫 NULL) and no longer returns perKmUnitPrice`／`AC-57: GET still returns the original estimatedAnnualKm/perKmUnitPrice for legacy versions (read-only) and null for new versions`）；另含 `AC-57(e): 夾帶 estimatedAnnualKm=0（原 ≤0 違規值）→ 201，且該欄不被採用（DB 寫 NULL）`／`AC-57(e): 夾帶 estimatedAnnualKm=1234.5（原非整數違規值）→ 201，且該欄不被採用（DB 寫 NULL）`（共 4 條）。**實作註記**：新版本之回應仍保留 `estimatedAnnualKm`／`derived.perKmUnitPrice` 兩鍵、值為 `null`（非移除鍵；刻意不以 `0`／哨兵值代位，見 §20.7.5 D21） |
| AC-58 | frontend | `frontend/test/DepreciationApplicationPage.test.tsx` | `AC-58：表單提供「該車年度總里程」可輸入欄位，並與唯讀之「年度公務里程」明確區分（標籤逐字＋公務里程仍無 textbox 之負向斷言）` | R9 | `GREEN` — 實名：`DepreciationApplicationPage.test.tsx` 之 `AC-58：表單提供申請年度與該車年度總里程兩個可輸入欄位（逐字標籤互異）`／`AC-58：兩者皆可為空值儲存（null 為合法值，前端轉為 JSON number｜null）`／`AC-58：填入年度與年度總里程後皆以 JSON number（非字串）送出`（3 條；同時承載 §12R.2 之 AC-38） |
| AC-59 | frontend | `DepreciationApplicationPage.test.tsx` | `AC-59：預覽與完成後詳情顯示五值（每年折舊費用／年度公務里程／年度總里程／公務比例／補貼金額），逐字取自後端回應`；`AC-59：車價、折舊年限、每公里單價三者於草稿頁與已完成頁皆不出現（負向斷言）` | R9 | `GREEN` — 實名：`DepreciationApplicationPage.test.tsx` 之 `AC-59：預覽顯示每年折舊費用／年度公務里程／年度總里程／公務比例／補貼金額，逐字取自後端回應`／`AC-59(b)：前端零自算鑑別——mock 之 amount 與 annualDepreciation×officialKm÷annualTotalKm 刻意不符，畫面仍顯示 mock 值`／`AC-59(c)：車價／折舊年限／每公里補助單價三推導值於草稿頁不出現`／`AC-59(c)：車價／折舊年限／每公里補助單價三推導值於已完成頁亦不出現`／`AC-59：不可計算時不得顯示金額 0，須顯示「無法計算」＋zh-TW 說明`（5 條） |
...
|---|---|---|---|---|---|
| **AC-62**（新增） | frontend | `frontend/test/ApplicationListSection.test.tsx` | `AC-62：操作欄之「檢視」連結三型一致——TRAVEL／MAINTENANCE／DEPRECIATION 各自導向既有詳情頁（草稿與已完成兩態皆適用）`；`AC-62：操作欄不再對任一型別渲染字面「—」（負向斷言）` | **REV2-G1** | `GREEN` — 實名與預定名**逐字相同**（`4115cae`；前端 238→240 兩輪、大總管獨立驗收；REV2 輕量複審 APPROVE） |
| **AC-61 → AC-61R**（修訂） | frontend ＋ e2e | `frontend/test/ParametersPage.test.tsx`；`e2e/depreciation-allowance.spec.ts` | `AC-61R：折舊版本清單之表頭恰為五欄（生效日期／車價／折舊年限／每年折舊費用／建立時間），不含「預估年里程」「每公里單價」`；`AC-61R：歷史版本之預估年里程與每公里單價原值於折舊區塊不出現（新舊版本一律不顯示）`；E2E `情境7`（改寫後名稱見 §20.17.4 #6） | **REV2-G2** | `GREEN` — 兩條 vitest 實名與預定名**逐字相同**；E2E 實名 `情境7：管理員參數頁折舊參數區塊縮欄——建立三欄＋版本清單五欄（預估年里程／每公里單價已移除）`（`9a01f61`；前端 240/240 兩輪、大總管親跑折舊單檔 8/8；REV2 輕量複審 APPROVE。**§12R.3 AC-61 列之 R11 `GREEN` 實名為歷史記錄，保留不改寫**；4 條逐條處置見 §20.17.4；另 2 條同檔連動測試之處置補列見 §18 REV2 複審固化列） |
| **AC-45**（連帶） | e2e | `e2e/depreciation-allowance.spec.ts` | 情境 1~6、8 **零改動**；**僅情境 7** 因 G2 而改寫（名稱與斷言見 §20.17.4 #6） | **REV2-G2** | `GREEN` — 情境 1~6、8 零改動續綠；情境 7 新實名見上列（大總管親跑全套 E2E **38/38**，2026-08-05；§12R.2 AC-45 列之 `4fb6dc9` 綠燈證據為歷史記錄，保留不改寫） |
| **AC-63**（新增） | integration | `backend/test/integration/phase7-on-behalf.test.ts` | `AC-63：折舊代建立稽核 summary 含 annualTotalKm（1 位小數定寬），且鍵集恰為四鍵`；`AC-63：未提供 annualTotalKm 之代建立 → summary.annualTotalKm 為 null（不以 0 代位）` | **REV2-G3** | `GREEN` — 實名與預定名**逐字相同**（`7918826`；後端 2571→2573 兩輪、大總管獨立驗收；G3 輕量複審 APPROVE，含 Decimal 尾零 fixture 鑑別力親證優於 Spec 建議值） |

> **AC-63 之連帶零改動（供終審核對）**：既有 `AC-34 代建立` describe 之四條、C3 邊界、同交易回滾、頂層純量夾帶、`targetLabel` mutant 鑑別**皆零改動且須續綠**（既有斷言為逐鍵包含式，新增鍵不使其變紅——實查見 §20.17.7）；`phase4-on-behalf-audit.test.ts`／`phase6-on-behalf.test.ts`（差旅／保養代建立）**零改動**（AC-63(e)）。
...
| 2026-08-05 | **REV2-G1／REV2-G2 完成與輕量複審 APPROVE（固化＋勘誤）**：G1（`4115cae`）G2（`9a01f61`）依 TDD 落地，reviewer 輕量複審 **APPROVE**（0 Must／0 Should／4 AR 全 Low 記錄在案）；§12R.5 三列已由大總管以實際測試輸出回填 `GREEN`（前端 240/240 兩輪、E2E 大總管親跑折舊單檔 8/8＋全套 38/38）。**勘誤 a（§20.17.4 完整性缺口）**：該表漏列 2 條直接斷言 `6.6667` 在場之同檔連動測試（`ParametersPage — 折舊列表顯示 derived` 之列表顯示條、`ParametersPage — 版本列表渲染` 之 Depreciation 條）——implementer 比照 #5 機械連動原則改為負向斷言（正向斷言全數保留、零刪除），大總管裁量接受、reviewer 判讀「AC-61R(d) 之機械必然、非擴權」並獨立確認無 ETC 誤殺面；正式補列由下一次 Spec 修訂 Task 執行（FW-1）。**勘誤 b（檔名指稱）**：§20.17.2(f)／§20.12 REV2-G2 列所稱「`frontend/src/api/parameters.ts` 之 DTO」實際定義於 `frontend/src/types/api.ts` :57/:63——兩檔皆 Forbidden 且皆零 diff，實質約束已滿足，字面修正併下一次 Spec 修訂 Task。 | reviewer PHASE-007-REV2-REVIEW（APPROVE）；大總管獨立驗收與 E2E 親跑 2026-08-05 |
| 2026-08-05 | **三項裁定（人類 leonchih，AskUserQuestion 全數照推薦；選項文字即操作粒度明確之裁定原文）**：①**Gate 揭露 #8＝補齊**——折舊「代建立」稽核（`APPLICATION_CREATED_ON_BEHALF`，`admin/routes.ts` 折舊分支）summary 加記 `annualTotalKm`＋測試，與「代修改」對稱；本 Phase 內派 Task（**稽核領域 High，事前批准由本裁定構成**）。②**Gate 揭露 #9＝接受現狀**——參數三端點 400 早於 401 **零程式變更**，記 RUNBOOK 待辦（跨 Phase 追蹤既有列）。③**`userstory.md` AD-US-13 第 5 條字面同步＝批准**——修為「歷史值保留於 API／資料庫；畫面不顯示」（純字面精確化、功能不變；派 spec-writer 執行）。 | 人類 leonchih 2026-08-05（AskUserQuestion 三問全數照推薦） |

| 2026-08-05 | **三項裁定之 Spec／US 落地（`PHASE-007-SPEC-REV3`）**：依上列 2026-08-05 末列之人類裁定 **①②③**，四項落地——①**裁定①（Gate 揭露 #8＝補齊）**：新增 **§20.17.7**（實查結論、**AC-63 全文 (a)~(g)**、4 條測試義務、Task **REV2-G3** 派工要點、自查表、移交項）；連動增補 **§20.17.0** 裁定表 G3 列與「範圍延伸」註（原第 2 點「#8／#9 尚未裁定」**逐字保留**，其對 REV2-G1／G2 之範圍約束續存）、**§12R.5** AC-63 新列（`PENDING`）、**§20.11.2** REV2-G3 列、**§20.12** REV2-G3 列＋規模上限自查列＋Gate 反饋段 High 判定註。②**裁定②（#9＝接受現狀）**：**零程式變更**，本 Spec **不新增任何 AC／Task／測試義務**，僅於 §20.17.0 範圍延伸註記載並移交大總管記 RUNBOOK 待辦。③**勘誤 a 落地**：§20.17.4 補列 2 條同檔連動測試（`列表顯示後端回傳的 derived.annualDepreciation 與 derived.perKmUnitPrice`、`Depreciation: 列表顯示 vehiclePrice / usefulLifeYears / estimatedAnnualKm / derived`）之處置（**修改（含改名）**）、改寫後逐字新名、覆蓋面去向（正向斷言全數保留＋負向斷言新增，零流失）與 **reviewer AR-1**（全頁 `queryByText` 形狀；無誤殺面成立之依據＝該二 describe 之 ETC fixture 為 `[]`；下次觸檔 `within` 化）。④**勘誤 b 落地**：§20.17.2 加「勘誤 b」blockquote ＋ (f) 與 §20.12 REV2-G2 列加註——`DepreciationParameterDto` 之兩鍵**實定義於 `frontend/src/types/api.ts` :57／:63**（非 `frontend/src/api/parameters.ts`，後者僅 import）；兩檔皆 Forbidden 且皆零 diff，實質約束不變，**原文逐字保留以維持可追溯性**。⑤**裁定③（US 字面同步）**：`userstory.md` **AD-US-13 第 5 條**之 `Then` 句由「系統應保留該歷史版本內容供唯讀查閱，且不得於新版本提供該欄位。」修為「**系統應將該歷史值保留於 API／資料庫、畫面不顯示，且不得於新版本提供該欄位。**」（`Given`／`When` 兩行與同條其餘內容、其他一切條文 **byte-identical**；機械驗證：`git diff --numstat userstory.md` ＝ `1 1`，單一 hunk 於 :1146）。**歷史零改寫**：§20.17.0~§20.17.6 原文、§12R.1~§12R.4、§20.12 R1~R13 與 REV2-G1／G2 兩列之既有內容一律保留（僅追加勘誤括號註記）。**本次未觸及 #9 之任何程式面、未改變裁定範圍外之任何行為、未修改 `docs/PRD.md`**（實查發現 :301 有同型字面觸點，已列 §20.17.7 移交項 #2，**非阻塞**）**、未修改任何程式或測試**。**Spec 狀態維持 `ACTIVE`。** | 人類 leonchih 2026-08-05「三項裁定」（AskUserQuestion 三問全數照推薦；批准紀錄＝上列同日「三項裁定」列，`ac67acc`）；勘誤 a／b 之批准依據＝上列同日「REV2-G1／REV2-G2 完成與輕量複審 APPROVE」列（該列已載明「正式補列由下一次 Spec 修訂 Task 執行（FW-1）」「字面修正併下一次 Spec 修訂 Task」）；程式現況實查（`backend/src/admin/routes.ts` :499／:580／:679-695、`backend/src/applications/routes.ts` :515／:1092-1129、`backend/src/applications/depreciation-service.ts` :128-201、`backend/test/integration/phase7-on-behalf.test.ts` :161-197／:317-443、`frontend/src/types/api.ts` :52-65、`frontend/src/api/parameters.ts`、`frontend/test/ParametersPage.test.tsx` :810-893／:1060-1085、`9a01f61` 逐 hunk 比對、`docs/PRD.md` :301）；`docs/retrospective/PHASE-007-usage.md` §3 預算帶 |

| 2026-08-06 | **REV2-G3 完成與輕量複審 APPROVE＋G1/G2 人類複驗通過（固化）**：REV2-G3（`7918826`）依 TDD 落地 AC-63，reviewer 輕量複審 **APPROVE**（0 Must／0 Should／4 AR 全 Low——AR-1 截斷型 mutant 窄窗（下次觸檔補 `"12345.6"`）、AR-2 null 案例缺 `not.toBeNull()` 前置、AR-3 1dp 格式常數兩檔重複（Packet Forbidden 結構性強制，FW 抽共用 util 記 PHASE-010 候選）、AR-4 describe 標題風格）；§12R.5 AC-63 列已回填 `GREEN`。**G1/G2 人類複驗通過**（leonchih 2026-08-06「複驗完成」，走查腳本 `docs/verification/WALKTHROUGH-PHASE-007-REV2.md` 結果欄已回填）。G3 複審 FW：§12R.5 回填（已執行）、PRD-SYNC（移交項 #2）、格式器共用化與 fixture 補強（下次觸檔）、travel/maintenance 代建立摘要覆蓋面（PHASE-010 稽核統一議題）。 | reviewer PHASE-007-REV2-G3-REVIEW（APPROVE）；人類 leonchih 2026-08-06 複驗回報 |

> Gate 固化義務已履行（上列 2026-08-04 第二列；修訂段 Gate 見上列 2026-08-05 第 `修訂段 Gate 通過` 列；修訂段 Gate 走查兩項反饋裁定見 2026-08-05 之 `修訂段 Gate 走查結果` 列，其 Spec 落地見其後之 `PHASE-007-SPEC-REV2` 列；G1/G2 完成與三項後續裁定見其後兩列；G3 完成與人類複驗通過見 2026-08-06 列）。
...
| `backend/src/admin/routes.ts`（**僅折舊分支之 `onCreated` summary**）、`backend/test/integration/phase7-on-behalf.test.ts` | **Gate 反饋 G3（裁定①／揭露 #8）**：折舊代建立稽核摘要加記 `annualTotalKm`（新增 **AC-63**）；**零測試退場**（既有代建立四條為包含式斷言，續綠）；travel／maintenance 分支與 `phase4`／`phase6` 兩 on-behalf 測試檔**零改動** | **REV2-G3** |

> **兩段之分界**：上表第 1~31 列為**折舊模型修訂段**（R1~R13）之清單；其後二列為 **Gate 走查反饋段**（G1／G2），逐條處置與覆蓋面去向見 **§20.17.4**；**末列**為三項裁定之 **G3**（§20.17.7）。
...
| **REV2-G2** | **Gate 反饋 G2**：管理員參數頁折舊版本清單**移除**「預估年里程（公里）」與「每公里單價（元）」兩欄（新舊版本一律不顯示） | FE ＋ E2E | **AC-61R** | `frontend/src/pages/ParametersPage.tsx`、`frontend/test/ParametersPage.test.tsx`、`e2e/depreciation-allowance.spec.ts` | **Medium**（純前端顯示面；後端／API／DB 零改動為明文約束） | —（僅依 §18 2026-08-05 末列之裁定 G2；與 R1~R13 無依賴） | ~150 | ~95-130k | 折舊版本清單表頭**恰 5 欄**（生效日期／車價（元）／折舊年限（年）／每年折舊費用（元）／建立時間）之全等斷言，且**不含**該二欄之表頭；歷史版本 fixture（`estimatedAnnualKm=15000`、`perKmUnitPrice="6.6667"`）之兩值於**折舊區塊內**不出現（值層負向斷言，防「僅移表頭留儲存格」）；**斷言須以 `within(折舊參數版本清單)` 限縮**——不得以全頁 `queryByText` 誤殺 ETC 區塊之同名「每公里單價（元）」欄（`ParametersPage.tsx` :449，不在本 Task 範圍）；`frontend/src/api/parameters.ts` **零改動**（DTO 保留該二鍵，AC-61R(f)）〔**檔名勘誤（SPEC-REV3）：該 DTO 實定義於 `frontend/src/types/api.ts` :57／:63，見 §20.17.2「勘誤 b」；兩檔於本 Task 皆 Forbidden 且皆零 diff，約束不變**〕；`num-col` 連動測試（UI-FIX-001-T1）改為三個數值欄；**E2E 情境7 之位置性斷言遷移並由大總管重跑**（基準 38/38）；測試零刪除／零 `skip`，**2 條退場改寫之覆蓋面去向依 §20.17.4 於 Handoff 逐條說明** |
| **REV2-G3** | **Gate 反饋 G3（裁定①／Gate 揭露 #8＝補齊）**：折舊「代建立」稽核（`APPLICATION_CREATED_ON_BEHALF`）之 `summary` 加記 `annualTotalKm`，與「代修改」對稱 | BE | **AC-63** | `backend/src/admin/routes.ts`（**僅折舊分支之 `onCreated` summary 物件**）、`backend/test/integration/phase7-on-behalf.test.ts` | **High**（**稽核領域**；**事前批准已由 §18 之 2026-08-05「三項裁定」列之裁定①逐字構成——「稽核領域 High，事前批准由本裁定構成」，無須另取**） | —（僅依 §18 2026-08-05「三項裁定」列之裁定①；與 R1~R13、REV2-G1／G2 皆無依賴） | ~120 | ~95-130k | `summary` 鍵集**恰四鍵**（`applicationId`／`type`／`applicationYear`／`annualTotalKm`）之**集合全等**斷言；`annualTotalKm` 非 `null` 時為 **1 位小數定寬字串**（`toFixed(1)`，與代修改之 `ANNUAL_TOTAL_KM_SUMMARY_SCALE` 同精度），**未提供時恰為 `null`（`toBeNull()`，不以 `0`／`"0.0"`／`""` 代位）**；值取自**同一交易內**之 `created.depreciation?.annualTotalKm`（零額外查詢）；**travel（:499 起）與 maintenance（:580 起）兩分支 summary 零 diff**（AC-63(e)）；`phase4-on-behalf-audit.test.ts`／`phase6-on-behalf.test.ts` **零改動且續綠**；既有 `AC-34 代建立` 四條、C3 邊界、同交易回滾、頂層純量夾帶、`targetLabel` 鑑別**續綠**；**HTTP 回應（201 與 body）逐字不變**；`backend/src/applications/**`、`backend/prisma/**`、`frontend/**`、`e2e/**` 皆 Forbidden；後端整合測試全綠且**測試零刪除／零 `skip`／零弱化** |
...
> **Gate 反饋段之 High 判定（不併入上列 10 項——上列為修訂段 Gate 之批准包，屬歷史記錄）**：**REV2-G1／REV2-G2 為 Medium**（G1 保守可升 High，見 §20.17.3）；**REV2-G3 為 High（稽核領域）**，其**事前批准由 §18 之 2026-08-05「三項裁定」列之裁定①逐字構成**，**無須另取人類批准**。
> **REV2-G3 之定位**：與 R1~R13 及 REV2-G1／G2 皆無依賴（Allowed 檔案集合與該二 Task 完全互斥），可獨立派工；依 Gate 反饋流程（Spec 修訂 → implementer TDD → reviewer 複審 → 合入）進行。
...
> **落地狀態（`PHASE-007-DOC-SYNC-REV`，2026-08-05）**：#12~#18 已同步至 `docs/ARCHITECTURE.md`（§3 模組表 `parameters`／`depreciation` 兩列、§4.1 折舊三條、§4.2 新增縮欄條、§4.4 快照三條、§4.5 附件條、§4.11 揭露面條）與 `docs/DATA_FLOW.md`（§1.1 `DepreciationApplication`／參數版本兩列、§2.2 折舊差異條與預覽／完成兩流程圖及其後五條、§2.7 折舊參數縮欄條）。**#19 屬 `PROJECT_STATE.md` 之大總管白名單，不在本 Task 之 Allowed 檔案內，仍待大總管處置。**
...
| G3 | 折舊代建立稽核摘要加記 `annualTotalKm` | **裁定①（Gate 揭露 #8＝補齊）**：折舊「代建立」稽核（`APPLICATION_CREATED_ON_BEHALF`，`admin/routes.ts` 折舊分支）summary 加記 `annualTotalKm`＋測試，與「代修改」對稱；本 Phase 內派 Task（**稽核領域 High，事前批准由該裁定構成**）。逐字原文見 §18 之 2026-08-05「**三項裁定**」列（`ac67acc`） | **§20.17.7（新增 AC-63）** |
...
> **範圍延伸（`PHASE-007-SPEC-REV3`，2026-08-05 三項裁定）**：上列第 2 點所稱「#8／#9 **尚未裁定**」係 **REV2 時點之事實**，其對 **REV2-G1／REV2-G2 兩 Task** 之範圍約束**逐字續存**（該二 Task 已分別於 `4115cae`／`9a01f61` 完成並複審 APPROVE，範圍不再變動）。其後人類已就兩項作成裁定（§18 之 2026-08-05「三項裁定」列，`ac67acc`）：
> - **#8＝補齊** → 落地於 **§20.17.7**（新增 **AC-63** 與 Task **REV2-G3**）；**稽核領域 High，事前批准由該裁定構成**。
> - **#9＝接受現狀**（參數三端點 400 早於 401）→ **零程式變更**，**本 Spec 不因此新增任何 AC、Task 或測試義務**；僅記 RUNBOOK 待辦並沿跨 Phase 追蹤既有列（屬大總管 `PROJECT_STATE` 白名單，見 §20.17.7 移交項）。
> - **③`userstory.md` AD-US-13 第 5 條字面同步＝批准** → 已於本次修訂落地（見 §18 SPEC-REV3 列；§20.17.6 移交項 #1 據此結案）。
...
| API／型別層 | `frontend/src/api/parameters.ts` 之 DTO 仍含 `estimatedAnnualKm` 與 `derived.perKmUnitPrice`（後端 AC-57(c) 對歷史版本仍回原值）——**G2 不觸及該檔**（**檔名指稱勘誤見下方「勘誤 b」**） |

> **勘誤 b（`PHASE-007-SPEC-REV3` 落地；批准依據＝§18 之 2026-08-05「REV2-G1／REV2-G2 完成與輕量複審 APPROVE」列所載「勘誤 b」，該列已載明「字面修正併下一次 Spec 修訂 Task」）**：上表「API／型別層」列與下方 **(f)** 所稱之「`frontend/src/api/parameters.ts` 之 DTO」為**檔名指稱錯誤**——`DepreciationParameterDto`（含 `estimatedAnnualKm` 與 `derived.perKmUnitPrice` 兩鍵）**實際定義於 `frontend/src/types/api.ts`**（:57／:63；spec-writer 於基準 `ac67acc` 親查），`frontend/src/api/parameters.ts` 僅 `import` 該型別而不定義之。
> **實質約束未受影響**：兩檔於 §20.17.3 REV2-G2 之 **Forbidden 清單皆已明列**，且 `9a01f61` 實際觸檔恰三（`ParametersPage.tsx`／`ParametersPage.test.tsx`／`e2e/depreciation-allowance.spec.ts`），兩檔皆**零 diff**——「DTO 保留該二鍵」之義務已滿足。**原文逐字保留以維持可追溯性**（本勘誤即為更正依據）；後續一切引用**以本勘誤之檔名為準**。
...
- **(f) 後端／API／DB 零改動（明文約束）**：`GET /parameters/depreciation` 對歷史版本仍回傳原值與 `perKmUnitPrice`（**AC-57(c) 逐字不變**）；`POST` 之縮欄與夾帶不採用（AC-57(a)(b)）不變；`DepreciationParameterVersion.estimatedAnnualKm` 之 nullable 凍結欄、M2 migration、`deriveDepreciation` 之行為凍結**一律零變更**（§20.7.5 其餘各列）；`frontend/src/api/parameters.ts` 之 DTO **保留該二鍵**（Forbidden 檔）〔**檔名勘誤：該 DTO 實定義於 `frontend/src/types/api.ts` :57／:63，見上方「勘誤 b」；兩檔皆 Forbidden、皆零 diff，約束不變**〕。**本 AC 為純前端顯示面之修訂。**
...
**連動測試（補列——`PHASE-007-SPEC-REV3` 落地之「勘誤 a」；批准依據＝§18 之 2026-08-05「REV2-G1／REV2-G2 完成與輕量複審 APPROVE」列所載「勘誤 a」）**

> **補列之緣由（如實記載）**：REV2 修訂時本節漏列下列 2 條**同檔、直接斷言 `6.6667` 在場**之測試。REV2-G2 之 implementer 依 **#5 之機械連動原則**逕行處置，大總管裁量接受、reviewer 判讀為「**AC-61R(d) 之機械必然、非擴權**」並獨立確認**無 ETC 誤殺面**；該列已載明「正式補列由下一次 Spec 修訂 Task 執行（FW-1）」。本補列為**事後正式化**，**不改變任何已落地之測試內容**（`9a01f61` 逐字）。

| # | 既有測試（改寫**前**逐字實名） | 處置 | 說明與覆蓋面去向 |
|---|---|---|---|
| 9 | `ParametersPage — 折舊列表顯示 derived` ＞ `列表顯示後端回傳的 derived.annualDepreciation 與 derived.perKmUnitPrice`（`ParametersPage.test.tsx`） | **修改（含改名）** | 新名（逐字）：`列表顯示後端回傳的 derived.annualDepreciation；不顯示 derived.perKmUnitPrice（AC-61R 連動）`。**覆蓋面去向**：正向斷言 `getByText(/100000\.00/)`（`derived.annualDepreciation`）**逐字保留**；原對 `6.6667` 之 `getByText(...).toBeInTheDocument()` **反轉為** `queryByText(...).not.toBeInTheDocument()`——**同一 fixture、同一渲染路徑，正向覆蓋面零流失，另新增值層負向鑑別力**（防「僅移表頭、儲存格仍渲染」）。授權來源＝AC-61R(d)(e)② 之機械必然 |
| 10 | `ParametersPage — 版本列表渲染` ＞ `Depreciation: 列表顯示 vehiclePrice / usefulLifeYears / estimatedAnnualKm / derived`（同檔） | **修改（含改名）** | 新名（逐字）：`Depreciation: 列表顯示 vehiclePrice / usefulLifeYears / derived.annualDepreciation；不顯示 derived.perKmUnitPrice（AC-61R 連動）`。**覆蓋面去向**：正向斷言 `500000.00`（車價）與 `100000.00`（每年折舊費用）**逐字保留**；`6.6667` 之斷言同 #9 反轉為負向。原測試名雖列 `estimatedAnnualKm`，**其本體並無 `15000` 之斷言**（改寫前後皆無），故無覆蓋面淨減——名稱同步更正即為勘誤之一部分 |

> **AR-1 記錄（reviewer REV2 輕量複審，Low）**：上列 2 條之負向斷言採**全頁** `screen.queryByText(/6\.6667/)`，未如 AC-61R(e) 之 `within(折舊參數版本清單)` 限縮。**現況無誤殺面成立**（reviewer 獨立確認、spec-writer 於 `ac67acc` 複驗）：該二 `describe` 之 fixture 皆為 `mockGetVersions("etc", [])`，ETC 清單為空 → 畫面不存在 ETC 之「每公里單價（元）」儲存格，全頁查詢與 `within` 等價。**義務（非阻塞）**：**下次因任何理由觸及該二條時，一律 `within` 化**（避免未來有人為 ETC 補 fixture 而使負向斷言在折舊已正確時仍紅／或反向失去鑑別力）。**本次不派工修改**——為修改而修改不符 §3「surgical changes」，且現況斷言正確且會紅於真正的回歸。
...
> **上列三項之結案狀態（`PHASE-007-SPEC-REV3` 回填）**：**#1 已結案**——人類 2026-08-05 裁定③批准 `userstory.md` AD-US-13 第 5 條之字面同步，本次修訂已落地（見 §18 SPEC-REV3 列之「US 修訂前後對照」）。**#2 已履行**——大總管已親跑 E2E（折舊單檔 8/8、全套 38/38）並回填 §12R.5。**#3 已裁定**——#8＝補齊（→ §20.17.7）、#9＝接受現狀零程式變更（見 §20.17.0 之「範圍延伸」註）。

---

#### 20.17.7 G3：折舊「代建立」稽核摘要之對稱補齊（裁定①／Gate 揭露 #8；新增 AC-63）

**依據**：§18 之 2026-08-05「**三項裁定**」列（`ac67acc`）之裁定①，逐字：「**Gate 揭露 #8＝補齊**——折舊「代建立」稽核（`APPLICATION_CREATED_ON_BEHALF`，`admin/routes.ts` 折舊分支）summary 加記 `annualTotalKm`＋測試，與「代修改」對稱；本 Phase 內派 Task（**稽核領域 High，事前批准由本裁定構成**）。」

**實查結論（spec-writer 於基準 commit `ac67acc` 親查，非推測）**

| 項目 | 實查結果 |
|---|---|
| 現況缺口 | `backend/src/admin/routes.ts` :679-695——折舊代建立之 `onCreated` 於 `isOnBehalf` 時寫 `AuditLog`，`summary` **恰三鍵** `{applicationId, type: "DEPRECIATION", applicationYear}`（**無 `annualTotalKm`**） |
| 對稱目標（代修改） | `backend/src/applications/routes.ts` :1092-1129——`summary` 含 `annualTotalKm: {before, after}`；格式化以 `ANNUAL_TOTAL_KM_SUMMARY_SCALE = 1` 之 `toFixed(1)`；`null` **一律如實記為 `null`**（絕不以 0 或空字串代位——「清空」與「填 0」須可區辨） |
| 值之可得性 | **已具備，零新增查詢**：admin 端點沿用**同一份** `parseDepreciationFieldsInput`（`applications/routes.ts` :515），`annualTotalKm` 已隨 `...fields` 進入 `CreateDepreciationDraftInput`（`depreciation-service.ts` :128-145）並於同交易寫入（:188）；`createDepreciationDraft` 之回傳含 `depreciationApplicationInclude`，故 hook 內 `created.depreciation?.annualTotalKm` 為 `Prisma.Decimal \| null`，與 `applicationYear` 之取法同型 |
| 三型代建立分支 | 共三處：travel `admin/routes.ts` :499、maintenance :580、depreciation :679。**裁定①僅及折舊分支**；travel／maintenance **不在範圍**（後者之 summary 本已含其全部可變業務欄位，`toFixed(2)`／`formatUtcDate` 同型慣例） |
| 既有測試檔（實名） | **`backend/test/integration/phase7-on-behalf.test.ts`**（1068 行，`PHASE-007-T11 — 折舊代操作（代建立）＋ 稽核（AC-34/35）`）。既有代建立 describe：`AC-34 代建立 — 201，ownerId=U、createdById=admin，AuditLog 欄位精確`（:369）；既有代修改之封閉鍵集常數 `UPDATED_SUMMARY_KEYS`（:197）與揭露面遞迴斷言 `FORBIDDEN_DISCLOSURE_KEYS`／`assertNoParameterDisclosure`（:161-187）**可直接複用** |
| 既有代建立測試是否會紅 | **否**。:410-413 為**逐鍵「包含式」斷言**（`summary.applicationId`／`type`／`applicationYear`），非集合全等，故新增一鍵**不使其變紅**；:419 之 `{}` 案例僅斷言 `summary.applicationYear` 為 `null` |

**AC-63 折舊「代建立」稽核摘要之 `annualTotalKm`（與「代修改」對稱）**（來源：**裁定①**；AD-US-06／07／08 與 BE-US-03 之代操作稽核語意；補 AC-34 摘要相對 AC-35 之欄位缺口）——

- **(a) 摘要增鍵（本次變更之全部內容）**：`admin/routes.ts` 折舊代建立分支之 `onCreated`，其 `AuditLog.summary` 於既有三鍵之外**恰增一鍵** `annualTotalKm`，值取自**同一交易內**建立之列（`created.depreciation?.annualTotalKm`，不得於交易外另行查詢）。既有三鍵 `applicationId`／`type`（恆 `"DEPRECIATION"`）／`applicationYear` 之**鍵名、取值與型別逐字零變更**。
- **(b) 格式與「代修改」逐字同**：非 `null` 時以**固定 1 位小數**之字串記錄（`toFixed(1)`，比照 `applications/routes.ts` 之 `ANNUAL_TOTAL_KM_SUMMARY_SCALE = 1`，與 §20.6 `DepreciationApplicationDto.annualTotalKm` 之對外契約同精度）；`null`（未輸入／body 未帶該 key）**一律如實記為 `null`**，**絕不**以 `0`、`"0.0"`、`"0"` 或空字串代位——否則「未申報」與「申報 0」於稽核上無從區辨。
- **(c) 形狀非對稱之明文（代建立 vs 代修改）**：代建立為**單值**（`annualTotalKm: string | null`）、代修改為**前後值物件**（`{before, after}`）——建立時不存在「前值」。本 AC **不得**把代建立摘要改成 `{before: null, after: …}`（將與差旅／保養代建立之既有慣例分岔）。「與代修改對稱」之義為**欄位覆蓋面與格式精度對稱**，非結構同形。
- **(d) 摘要欄位集合封閉（§20.10 揭露面不變式續守）**：折舊代建立摘要之鍵集**恰為** `{applicationId, type, applicationYear, annualTotalKm}` **四鍵**；**不得**出現車價／折舊年限／每公里補助單價／預估年里程／每年折舊費用／參數版本 id 等**參數面或推導值**（`FORBIDDEN_DISCLOSURE_KEYS` 遞迴斷言之落點）；亦不得出現密碼／token／session 等敏感字樣（AC-44 掃描語意不變）。
- **(e) 適用範圍嚴格限於折舊分支**：`travel`（:499 起）與 `maintenance`（:580 起）兩個代建立分支之 `summary` **零改動**（欄位集合、取值與格式逐字不變）——裁定①僅及折舊。兩型之既有測試 `backend/test/integration/phase4-on-behalf-audit.test.ts`、`phase6-on-behalf.test.ts` **零改動且須續綠**。
- **(f) 其餘代操作語意零回歸**：C3 邊界（`:userId` ＝ 管理員自己 → **完全不建構 hook**、不寫稽核，PHASE-004 AC-86）、`targetLabel` 恰為 `loginName#applicationId`、**同交易原子性與回滾**（AC-34；hook 拋錯 → 申請與稽核一併回滾）、**恰 1 列稽核**、授權判定順序（403 先於 400／404）**一律不變**。
- **(g) 零對外契約變更**：本 AC **不改變任何 HTTP 回應**——代建立之 `201` status 與 body（`{ application: DepreciationApplicationDto }`）逐字不變；稽核摘要為**內部稽核資料**，非 API 對外揭露面。後端 DTO、前端、DB schema、migration **一律零改動**。

**測試義務（AC-63）**

| # | 測試（預定名稱） | 鑑別力 |
|---|---|---|
| 1 | `AC-63：折舊代建立稽核 summary 含 annualTotalKm（1 位小數定寬），且鍵集恰為四鍵` | 代建立 body 帶 `annualTotalKm`（建議 `"12345.6"`——與代修改既有 fixture 同值，便於對照）→ 201；斷言 `summary.annualTotalKm === "12345.6"` **且**鍵集全等 `["annualTotalKm","applicationId","applicationYear","type"]`（排序後；建議比照 `UPDATED_SUMMARY_KEYS` 新立 `CREATED_SUMMARY_KEYS` 常數）。**漏加鍵必紅**、**夾帶車價／年限必紅**、**以 `toFixed(2)` 或 `toString()` 實作必紅**（`"12345.60"`／`"12345.6"` 之窄窗須以帶尾零之值鑑別——建議另備 `"12345.0"` 之輸入以殺 `toString()` mutant） |
| 2 | `AC-63：未提供 annualTotalKm 之代建立 → summary.annualTotalKm 為 null（不以 0 代位）` | body 不帶該 key（`{ applicationYear: … }`）→ 201；以 `toBeNull()`（**非** falsy 斷言）斷言 `summary.annualTotalKm`；`0`／`"0.0"`／`""` 代位之 mutant **必紅**。**同時鎖住「清空 vs 填 0 可區辨」之稽核語意** |
| 3 | 既有 `POST /admin/users/:userId/applications/depreciation → 201，owner/creator 分離且 AuditLog action/actorId/targetId/targetLabel/summary 精確`（**零改動或僅增列斷言**，二擇一，Handoff 須說明） | 保障既有三鍵、`targetLabel`、恰 1 列與揭露面斷言零回歸 |
| 4 | 既有 C3 邊界 `→ 201，onBehalf=false，且不寫 APPLICATION_CREATED_ON_BEHALF`、同交易回滾 `createDepreciationDraft: onCreated 注入失敗 → 交易整體 rollback…`（**零改動、續綠**） | 保障 (f) 之語意零回歸 |

**Task REV2-G3 之派工要點（詳細列見 §20.12）**

| 項目 | 內容 |
|---|---|
| **Allowed（唯二可寫檔）** | `backend/src/admin/routes.ts`（**僅折舊分支之 `onCreated` summary 物件**）、`backend/test/integration/phase7-on-behalf.test.ts` |
| **Forbidden（明列）** | `backend/src/applications/**`（含 `routes.ts` 之代修改 hook、`depreciation-service.ts`——本 AC 零 service 變更）、`backend/prisma/**`、`backend/src/admin/routes.ts` **之 travel（:499 起）與 maintenance（:580 起）分支**（(e) 之明文約束：該二分支 summary 零 diff）、`backend/test/integration/phase4-on-behalf-audit.test.ts`／`phase6-on-behalf.test.ts`、`frontend/**`、`e2e/**`、`docs/**`、`userstory.md` |
| **必要測試** | 上表 #1／#2（新增）；#3（零改動或僅增列）；#4 續綠。**夾帶防線**沿既有慣例評估：既有「頂層純量夾帶」describe（:813）與 `assertNoParameterDisclosure` 已覆蓋，**不需新增第二套**；若 implementer 判定需增，須於 Handoff 說明既有覆蓋之不足 |
| **Diff 預算** | **~120 行**（src 約 +3~+8 含註解；測試約 +60~+100）。逾 50%（~180 行）須即時回報 |
| **usage 帶** | **~95-130k** |
| **預算依據** | `docs/retrospective/PHASE-007-usage.md` §3 之「**純測試檔 BE ~105-175k**」（下緣數據點 T6b 107k／T9b 113k，皆為**新建整張矩陣**、diff +307／+314）。本 Task 為「既有 hook **加一鍵** ＋ 既有測試檔**加兩條 `it`**」，改動面約為 T6b／T9b 之 **1/4~1/5**，故 diff 下修至 ~120；usage **不等比下修**——`admin/routes.ts`（708 行）與 `phase7-on-behalf.test.ts`（1068 行）之讀檔、既有慣例對照（三型代建立 ＋ 代修改）、以及 TDD 紅→綠兩輪與**整合層需 DB** 之開銷為固定成本，故維持 ~95-130k（與 REV2-G2 之 95-130k 同帶） |
| **Risk** | **High**（稽核領域；CLAUDE.md「認證、授權…一律 High」之既有升級先例）。**事前批准已由裁定①逐字構成**（「稽核領域 High，事前批准由本裁定構成」），**無須另取人類批准** |
| **Lite Packet 適用性（spec-writer 判定）** | **不建議 Lite**——雖僅 2 檔、1 條新 AC，但屬 **High 稽核領域**、需**整合層 DB** 與**四類 mutant 鑑別**（漏鍵／夾帶／精度／`null` 代位），且須逐條論證「travel／maintenance 零改動」。建議走**標準 Packet**，使 reviewer 有完整鑑別力論證可審 |
| **依賴** | 無（與 R1~R13、REV2-G1／G2 皆無依賴；可獨立派工）。前提僅為 R8b 之代修改實作已在場（`ac67acc` 已滿足） |

**自查表（spec-writer 逐項確認）**

| # | 自查項 | 結果 |
|---|---|---|
| 1 | 是否改變裁定①範圍外之產品行為？ | **否**。AC-63 之範圍嚴格等同裁定①逐字內容（折舊代建立 summary 加記 `annualTotalKm`＋測試、與代修改對稱）；(e)(f)(g) 三條為**收斂性**明文約束（travel／maintenance 零改動、代操作語意零回歸、零對外契約變更） |
| 2 | 是否觸及 Gate 揭露 #9 之程式面？ | **否**。#9 已裁定為**接受現狀、零程式變更**；本節不新增任何 AC／Task／測試義務，REV2-G3 之 Forbidden 清單涵蓋 `backend/src/parameters/**`（不在 Allowed 內即不可寫），結構上不可達 |
| 3 | 是否縮減任何既有 AC？ | **否**。AC-34（代建立稽核）之既有義務**逐字保留並擴充一鍵**；AC-35（代修改）逐字不變；§20.10 揭露面不變式由 (d) 明文續守 |
| 4 | 是否改寫任何歷史記錄？ | **否**。§20.17.0~§20.17.6 之原文一律保留，修訂以**新增列／加註／blockquote 勘誤**呈現；§12R.1~§12R.4、§20.12 R1~R13 與 REV2-G1／G2 兩列之既有內容零改寫（REV2-G2 列僅追加檔名勘誤之括號註記，原句逐字保留） |
| 5 | 是否新增 DB／公開 API contract 變更？ | **否**。(g) 明文；Forbidden 清單含 `backend/prisma/**` 與 `backend/src/applications/**` |
| 6 | AC↔測試映射是否完備？ | **是**。AC-63 → §12R.5 新列（2 條預定名，`PENDING`）；受影響檔 → §20.11.2 新列 |
| 7 | 是否有 Blocking Unknown？ | **無**。惟 `docs/PRD.md` 有一處與 AD-US-13 第 5 條同型之字面觸點，已列為下方移交項 #2（**非阻塞**，且本 Task 之 PRD 為 Forbidden） |

**移交大總管之事項（非 Blocking）**

1. **#9 之 RUNBOOK 待辦**：裁定②要求「記 RUNBOOK 待辦（跨 Phase 追蹤既有列）」——屬大總管 `PROJECT_STATE` 白名單，本 Spec 不代行。
2. **`docs/PRD.md` :301 之字面觸點（實查發現，本 Task 未修改——PRD 為 Packet Forbidden）**：該行「模型修訂註記（折舊）」載有「…自新版參數介面退場，**既有歷史版本內容唯讀保留**」，與 `userstory.md` AD-US-13 第 5 條**修訂前**之措辭同型，於裁定 G2（畫面不顯示）後同樣**字面不精確**。**功能上不構成矛盾**（事實來源優先序：最新人類裁定 > `userstory.md` > PRD；且該行為 PHASE-003a 段之**追溯性註記**，其末句已明載「折舊相關驗收以 PHASE-007 為準」）。**建議**：由大總管以 `PHASE-007-PRD-SYNC` Lite Packet 派工，將該句改為「…既有歷史版本內容保留於 API／資料庫，畫面不顯示」（**純字面精確化**，依據＝裁定 G2 與裁定③之同一批准）。
3. **REV2-G3 完成後之回填**：§12R.5 之 AC-63 列須由大總管以實際測試輸出**逐字回填**實名與綠燈證據（沿 §12R 回填義務）。
