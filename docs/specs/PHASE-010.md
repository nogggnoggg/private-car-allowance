...
| AC-10(a)(b)(c)(d) | integration | `backend/test/integration/phase10-audit-structure.test.ts` | `AC-10: 稽核寫入站點白名單（5 檔／13 呼叫）＋ enum 10 值 ＋ 標籤表涵蓋` | T4 | `GREEN`（T4 `c8e8251`） |
| AC-11(a)(b)(c) | integration | 同上 | `AC-11: AuditLog 零 update／delete／upsert 掃描 ＋ mutant 自證` | T4 | `GREEN`（T4 `c8e8251`） |
...
| AC-19(a)~(d) | e2e | `e2e/audit-log.spec.ts` | `AC-19: 稽核頁端到端（建帳號＋作廢兩事件可見）／篩選／分頁／375px` | T8 | `GREEN`（T8 `8b721f2`） |
| AC-20(a)(b) | 全套件 | —（終審實跑對照） | 基準線 後端 3230／前端 346／E2E 50 不下降 | 全 Task | `PENDING`（**終審實跑回填**——本列刻意不由任何 Task 自行轉綠，須以終審之三套件實跑數對照。**各 Task 大總管獨立驗收之實測值（供終審對照，非終審實跑本身）**：後端 **3542**（T9R `f98e7d5`）／前端 **388**（T-MG3-FE `7e66968`）／E2E **55**（T8 `8b721f2`），三者皆 ≥ 起點基準線 3230／346／50） |
