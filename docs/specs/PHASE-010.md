...
| AC-02(a)~(e) | unit | `backend/test/unit/audit-log-query.test.ts` | `AC-02: parseAuditLogQuery 參數矩陣（B-01~B-09）`（**MG-2 後須增 B-29 台北日界雙向格**） | T1 → **T-MG1-BE2** | **`GREEN`（MG-2 重驗 T-MG1-BE2 `c919aac`）** |
...
**狀態計數（BE1/BE2 重驗回填後）**：`GREEN` **32**／**`PENDING（Gate 修訂後待重驗）` 1**／`PENDING` **7**／`N/A（非測試）` **1**＝**41**。
