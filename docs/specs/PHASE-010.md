...
**AC-22 — 技術債收斂批次【建議新增；依 D7 之分流裁定＝(b)】**

> **裁定值明示（`SPEC-REV-010-CLOSE` 補；S-2 更正之防再犯）**：**D7 ＝ (b) 分流**——依 §18 Spec Gate 固化列逐字「其餘 8 條（D1/D2/D4/D6/D7/D9/D10/D12）**照推薦**」，而 §16 **D7 之推薦為 (b)**（納入 D-2／D-3／R-4／R-5；D-5／D-6／D-7 明列不納並移 PHASE-011）。本條 (a)~(d) 之內容即 (b) 之逐項展開，T10 之交付（`c11045a`）亦恰為 (b)。**任何處出現「D7=(a)」皆為記載失準**（§12 曾有一處，已於本次更正）。

(a) **D-2**：`applications/routes.ts` 之 `REPORT_GENERATION_FAILED` 500 轉譯由 `buildErrorBody` 繞道改經 `AppError` 承載，**wire 輸出逐位元組不變**（含鍵序）。
(b) **D-3**：`errorLabel` **兩份**逐字相同之同形實作收斂為單一葉節點模組（`backend/src/platform/error-label.ts`），**2 檔 4 個呼叫端**行為零變更。
　　**〔`SPEC-REV-010-CLOSE` 措辭更正，依 T9/T10 合併複審 AR-5；本次修訂為記載對齊，AC 語意零變更〕** 原文作「**三份**同形實作……**三處**呼叫端」，實查皆失準：
　　· **份數**——實際重複者恰**兩份**（`attachment/attachment-copy.ts`／`attachment/upload-service.ts`，兩者之 `function errorLabel` 逐字相同）。第三份從不存在（T10 以 `git log -S` 覈實）；`platform/health.ts` 之 `errName` 與 `platform/error-handler.ts` 之 `errName` 取 **`err.name`**，本模組取 **`err.constructor.name`**，對「未自行設定 `name` 之 `Error` 子類」回傳相異值（`class Foo extends Error {}` → `err.name === "Error"`／`constructor.name === "Foo"`），**併入即為行為變更**，直接牴觸本條同句之「行為零變更」硬約束，故**刻意不併**（屬同一「零內容標籤」慣例家族，非同一實作）。
　　· **呼叫端數**——實查為 **4 個**（`attachment-copy.ts` :121／:321、`upload-service.ts` :319／:332），分布於 **2 檔**。「三處」為承 (b) 原文之連帶失準。
　　· **溯源（W-1）**：`KNOWN_ISSUES.md` §3 D-3 之「三份同形實作」係 PHASE-009-T18 檔頭所稱之「**慣例同型**」在登記為 KNOWN_ISSUES 條目時被收斂成「**同形實作**」——語意由「同一族的寫法慣例」滑移為「同一份程式碼的複本」，本 Spec 之 AC-22(b) 再承接該失準字面。經 **T10 實查 ＋ W-1 ＋ T9/T10 合併複審**三重覈實，**大總管裁定「兩份收斂＝AC-22(b) 足額關閉」，複審 CONFIRM**。
　　· **反退化守門之缺格（據實登記，不在本 Phase 補）**：本條**無專屬測試斷言**（收斂後若有人再寫回第二份私有 `errorLabel`，測試不會紅）——複審 AR 已登記為 **FW-D**，移交 PHASE-011（`KNOWN_ISSUES.md` §5）。
　　· **程式面之連帶失準（本 Task 不觸程式，登記待辦）**：`backend/src/platform/error-label.ts` :42-43 之函式註解仍承本條原文作「**三處呼叫端**」——已登記為 `KNOWN_ISSUES.md` §4 **R-6**（記載型更正，零斷言影響）。
...
| AC-10(a)(b)(c)(d) | integration | `backend/test/integration/phase10-audit-structure.test.ts` | `PHASE-010-T4 — AC-10: 稽核寫入站點白名單掃描 ＋ enum 對照封閉`（7 it：白名單 5 檔/13 呼叫逐檔次數、FW-1 註解陷阱、FW-3/6 射程負控制、enum 10 值、標籤表涵蓋＋值非空互異＋刪值 mutant、第三份複本三方一致、AC-10(d) 記載實查——SF-1 終審後逐字回填） | T4 | `GREEN`（T4 `c8e8251`） |
| AC-11(a)(b)(c) | integration | 同上 | `PHASE-010-T4 — AC-11: 稽核不可變性結構守門`（4 it：全域零命中、.count 合法讀取負控制、三型規避探針鑑別力自證、AC-11(c) 純測試記載——SF-1 終審後逐字回填） | T4 | `GREEN`（T4 `c8e8251`） |
...
| AC-19(a)~(d) | e2e | `e2e/audit-log.spec.ts` | `稽核檢視端到端 Gate — PHASE-010-T8（AC-19）` › 5 test：`AC-19(a): 管理員真實作廢 → 首頁入口進稽核頁 → 建立帳號／申請作廢兩事件可見且四要素正確`／`AC-18(e)／MG-3 回歸（負向）: 稽核頁零內部識別碼（cuid）呈現`／`AC-19(c): 375px——稽核列表／異動明細表／長作廢原因零水平溢位`／`AC-19(b): 依 action 篩選確實收斂（值層中文化）＋ 起訖日以台北日曆日收斂`／`AC-19(b): 分頁至第 2 頁且兩頁列不重複` | T8 | `GREEN`（T8 `8b721f2`） |
| AC-20(a)(b) | 全套件 | —（終審實跑對照） | 基準線 後端 3230／前端 346／E2E 50 不下降 | 全 Task | **`GREEN`（終審 2026-08-10 同一 HEAD `18b237b` 親跑：後端 3542／前端 388／E2E 55，零 skip；起點 3230/346/50）** |
| AC-21(a)(c)(d) | integration | `backend/test/integration/phase10-error-handler-leak.test.ts` | `AC-21(a)(c)(d): 未預期例外兜底 — 日誌洩漏面與 wire 逐位元組`（該 describe 下 **7 條 `it`**，逐字）：`AC-21(a): 兜底日誌仍存在且記錄 error.name 與固定分類標籤`／`AC-21(a): 兜底日誌不再帶 errMessage 欄位（不記 error.message 原文）`／`AC-21(a): requestId 仍為關聯手段（日誌與回應同一值）`／`AC-21(c): 兜底日誌之字串值零路徑分隔字面命中（'/' 與 '\' 皆零）`／`AC-21(c): 全日誌輸出零哨兵字面命中（注入之絕對路徑片段皆未落地）`／`AC-21(d): 500 回應 body 逐位元組等於 INTERNAL_ERROR ＋ 固定文案 ＋ requestId`／`AC-21(d): 500 回應 body 零哨兵字面命中（wire 面不洩漏例外訊息）` | T9 | **`GREEN`（T9 `3bae7fd`）** |
| AC-21(b) | integration | 同上 | `AC-21(b): 站點白名單掃描 — 錯誤訊息之輸出面`（該 describe 下 **19 條 `it`**，逐字）：`L1（AC 字面）：直接送入 log.*／console.* 或回應 details 之檔案集合恰等於白名單`／`L2（保守超集）：全域錯誤訊息取值之檔案集合恰等於白名單`／`L1 鑑別力①：單行 log 呼叫內之 error.message 必被偵測`／`L1 鑑別力②：跨行 log 呼叫內之 err.message 必被偵測`／`L1 鑑別力③：巢狀呼叫（sanitizeForLog(err.message)）與模板字面必被偵測`／`L1 鑑別力④：回應 details 內之 err.message 必被偵測`／`L1 鑑別力⑤（反向）：領域物件之 .message（blocker／issue／detail）不得誤報`／`鑑別力⑥：型別斷言後取值 (err as Error).message 必被偵測（L1／L2 皆是）`／`鑑別力⑦：optional chaining err?.message 必被偵測（L1／L2 皆是）`／`鑑別力⑧：bracket 取值 err["message"]／err?.["message"] 必被偵測（L1／L2 皆是）`／`鑑別力⑨：TS 窄化後之他名變數（wireError／fastifyError）必被偵測（L1／L2 皆是）`／`鑑別力⑩：catch 他名繫結（catch (reason)）之 .message 必被偵測（L1／L2 皆是）`／`鑑別力⑪：解構賦值 const { message } = err 必被偵測（L2；L1 不及——變數間接）`／`已知不可及⑫（記載型反向對照）：log.error({ err }) 之 pino 序列化型掃不到 — FW-A 移交 PHASE-011`／`已知不可及（記載型）：參數位解構 function f({ message }) 掃不到 — 無從判定是否為錯誤物件`／`L1 已知盲點（變數間接型）由 L2 承接 — 反向對照明示`／`L2 鑑別力：註解內之 err.message 不得誤報（stripComments 生效）`／`L2 鑑別力：新增任一站點（新檔）即必紅 — 以 fixture 內容自證`／`真實檔暫改自證（重放複審 M5）：error-handler 分支 3 植入 wireError.message 即轉紅，原檔位元組不變` | T9 → **T9R** | **`GREEN`（M-1 修復 T9R `f98e7d5`）**——複審 M-1（掃描對七型主流 TS 寫法漏報）已封閉：T9R 於**同一檔**新增 **9 條 `it`**（鑑別力⑥~⑪、已知不可及⑫、參數位解構、M5 重放）＋ **2 處更名**（describe 由「`— err.message 之輸出面`」改為「`— 錯誤訊息之輸出面`」、L2 該 `it` 同步改字面），全檔 **17 → 26 條 `it`**；`backend/src` **零 diff**（blob hash 全等），故上一列 (a)(c)(d) 之 `GREEN` 結論不受擾動 |
| AC-22(a) | integration | `backend/test/integration/phase9-void-report.test.ts`（**`SPEC-REV-010-CLOSE`／FW-E 落點更正**：原載「既有 contract 檔擴充」，實查 T10 之兩格皆落於本檔——該檔非 contract 檔） | ①`PHASE-009-T12a §D-0 — 結構性斷言（AC-21(f) ＋ SPEC-REV-9T12 兩下游約束）` › `AC-22(a) 結構: applications/routes.ts 之 REPORT_GENERATION_FAILED 經 AppError 承載，零 buildErrorBody 繞道`；②`PHASE-009-T12a §D — 作廢版 PDF 之產生與保存（AC-21(a)(c)(e)）` › `§快速替身（renderPdf 覆寫為合成位元組；renderReportHtml 仍為真實實作）` › `AC-22(a): 作廢版 PDF 產生失敗之 500 wire body 逐位元組全等（頂層恰一鍵 error；error 恰四鍵 code/message/requestId/details；details 恰一鍵 stage；鍵序釘住）` | T10 | **`GREEN`（T10 `c11045a`）**（D7=**(b)** 生效解鎖 2026-08-09——`SPEC-REV-010-CLOSE`／S-2 更正：原載 `D7=(a)` 與 §18 Gate 固化列「照推薦」＝(b) 矛盾） |
| AC-22(b)(c)(d) | —（非測試） | — | **非測試落點，逐項據實登記**：**(b)** `errorLabel` 兩份收斂為 `platform/error-label.ts` 已落地（T10 `c11045a`），惟**無專屬斷言**——「再寫回第二份私有 `errorLabel`」之反退化守門格**缺**（複審 AR，登記為 **FW-D** 移交 PHASE-011）；**(c)** R-4／R-5 為**記載型更正**（`report-service.ts` 檔頭矛盾句／`applications/routes.ts` 檔頭 17 端點總覽），依 §2 逐字即「**零斷言影響**」，其載體為原始碼註解與複審覈實；**(d)** D-5／D-6／D-7 **明列不納**，為純記載決策（續留 `KNOWN_ISSUES.md` §3 並改排 PHASE-011）。**本列不計入測試覆蓋計數**（沿 AC-14(c) 之既有處置形狀）。 | T10 | `N/A（非測試）` |

**合計 22 條 AC ／ 44 列映射**（含 **2 列** `N/A（非測試）`＝AC-14(c)／AC-22(b)(c)(d)，故**測試列為 42 列**；另 **1 列**為 §5 邊界列 B-17／B-18）。條件 AC（AC-14／AC-21／AC-22）已隨 Spec Gate 裁定於 2026-08-09 解鎖。**AC 條數維持 22**——`SPEC-REV-010-LITE-3` 之 **AC-16(d)**、`SPEC-REV-010-GATE` 之 **AC-18(d)**／**AC-06(b-0)~(b-3)**、`SPEC-REV-010-GATE-2` 之 **AC-18(e)** 皆為既有 AC 之**子項**，不新增 AC 編號。
**★ 狀態計數（`SPEC-REV-010-CLOSE` 後＝現況，44 列）★**：`GREEN` **41**／`PENDING` **1**（**僅 AC-20**——終審實跑回填）／`PENDING（Gate 修訂後待重驗）` **0**／`N/A（非測試）` **2**（AC-14(c)／AC-22(b)(c)(d)）＝**44**（測試列 42 ＋ `N/A` 2）。
> 計數推導（供機械核對；自 `SPEC-REV-010-GATE-2` 之 42 列起算）：①**42 列期間之逐列回填**（大總管於 T4／T7R／T8／T-MG3-FE 各結案時回填，計數行未同步）使 `GREEN` 由 33 → **38**、`PENDING` 由 7 → 3（AC-20／AC-21／AC-22）、待重驗 1 → 0（AC-15 由 `T-MG3-FE` `7e66968` 轉綠）；②本次 **AC-21 一列拆為兩列**（(a)(c)(d)／(b)——兩者分屬不同 `describe`、不同 Task 輪次：(a)(c)(d) 由 T9 一次到位，(b) 經 T9R 修復），兩列皆 `GREEN` → `GREEN` 38＋2＝**40**、`PENDING` 3−1＝2；③本次 **AC-22 一列拆為兩列**（(a) 有兩格測試 → `GREEN`；(b)(c)(d) 依 §2 逐字皆為非測試載體 → `N/A`），→ `GREEN` 40＋1＝**41**、`PENDING` 2−1＝**1**、`N/A` 1＋1＝**2**；④總列數 42＋2＝**44**。**與 Packet 之差異（據實記錄）**：Packet 指示「AC-22 列轉 `GREEN`」；實查 AC-22 之四子項中**僅 (a) 有測試斷言**（(b) 反退化守門格缺＝複審 AR／FW-D、(c) 依 AC 逐字為「零斷言影響」、(d) 為純記載決策），整列標 `GREEN` 將構成「聲稱未存在之測試證據」，故改採**拆列 ＋ (b)(c)(d) 標 `N/A（非測試）`**（沿 `SPEC-REV-010-LITE-2` 對 AC-14(c) 之既有處置形狀），AC 語意零變更。
〔**以下為歷史計數段，保留供追溯**（`SPEC-REV-010-GATE-2` 當時之 42 列）：`GREEN` **33**／`PENDING` **7**（T4×2／T8／T9／T10／AC-20／**AC-18(e)**）／**`PENDING（Gate 修訂後待重驗）` 1**（AC-15(a)(b)(c)——僅 (c) 一格受衝擊）／`N/A（非測試）` **1**＝**42**。前一版計數為 41 列＝`GREEN` 34／`PENDING` 6／`N/A` 1（大總管 2026-08-10 SF-B 去重更正：原 AC-18(d) 預定列與實名列重複——刪預定列留實名列）。本次兩處：①**新增 AC-18(e) 一列**（`PENDING`／T-MG3-FE）②**AC-15 一列由 `GREEN` 轉 `PENDING（Gate 修訂後待重驗）`**（`GREEN` 34−1＝33）。**其餘 40 列狀態逐列零變更**——實查佐證：`AuditChangesList.test.tsx` 內**零**斷言依賴 `applicationId`／`revisionOf` 兩列之呈現（`REQUIRED_SUMMARY_FIELDS` :232-242 之守門為「英文鍵名**不得**出現」之負向式，過濾後仍為真——**惟其對該兩鍵之鑑別力歸零，故 AC-18(e-4) 強制改以標籤表為斷言對象**）；`e2e/audit-log.spec.ts` 尚未實作（AC-19 `PENDING`），零 E2E 衝擊。〕
...
## 13. Architecture / Data Flow 需同步項清單（**Task 期間不修改該二檔**；結案 DOC-SYNC 批次處理——**已於 `PHASE-010-DOCSYNC` 執行，逐項核銷見「處置」欄**）

| # | 檔案／節 | 待同步內容 | 處置（`PHASE-010-DOCSYNC`，2026-08-10） |
|---|---|---|---|
| 1 | `ARCHITECTURE.md` §4.8 稽核 | 補「稽核檢視端點」之落點與投影語意（`changes[]` 扁平化為單一事實來源） | ✅ **已落地**——§4.8 新增「稽核檢視（PHASE-010 落地）」段（端點／授權／投影／`flattenAuditSummary` 單一事實來源／呈現層過濾之邊界） |
| 2 | `ARCHITECTURE.md` §3 模組表 :75 `audit` 列 | 現載「稽核事件寫入與查詢」——**查詢面本 Phase 才落地**，須更新落點檔名 | ✅ **已落地**——`audit` 列補四檔落點與各自職責（`audit.ts` 寫入助手／`audit-query.ts` 解析與 `where` 純函式／`audit-summary.ts` 攤平純函式／`routes.ts` 薄層編排 ＋ `auditPlugin`），並明載寫入面本 Phase 零變更 |
| **9**〔DOCSYNC 追加〕 | `ARCHITECTURE.md` §3 `platform` 列 | `platform/error-label.ts` 葉節點（T10 新增）入模組圖 | ✅ **已落地**——`platform` 列補五檔落點，`error-label.ts` 明載「無專案內部相依故可被任何層級 import 而不成環」與其 4 個呼叫端 |
| **10**〔DOCSYNC 追加〕 | `ARCHITECTURE.md` §4.9 結構化錯誤處理 | error-handler 兜底日誌之取捨（T9）＋ `buildErrorBody` 組裝點唯一（T10） | ✅ **已落地**——§4.9 新增三條：組裝點唯一與繞道歸零／兜底不記例外原文（根因、`requestId` 為唯一關聯手段、wire 零變更、**不得回退**之紀律、兩層掃描與已知不可及型別）／零內容錯誤標籤之單一來源與「刻意不與 `err.name` 慣例合併」 |
| 3 | `DATA_FLOW.md` | 新增 §「稽核檢視（唯讀）」流程（本 Spec §9.1） | ✅ **已落地**——新增 **§2.9 稽核檢視（唯讀；PHASE-010 落地）** |
| 4 | `DATA_FLOW.md` :518 稽核紀錄列 | 補「對外投影不含內部識別值」與「管理員 only」 | ✅ **已落地**——§3 稽核紀錄列補三點（管理員 only／`actorId`／`targetId` 不外露／`targetLabel` 與 `changes[]` 之 wire 完整而畫面過濾） |
| 5 | 【依 D5】`ARCHITECTURE.md` 使用者刪除守門 | 六條 RESTRICT FK 與 `userHasHistory` 之對應關係 | ✅ **已落地**——**新增 §4.13「使用者刪除守門與 FK 對應」**（八條指向 `User` 之 FK 逐條對照表：六條 RESTRICT ↔ `USER_RESTRICT_FK_GUARDS`、兩條刻意不納（`Session.userId` CASCADE／`AuditLog.targetId` SET NULL）＋ P2003 兜底 ＋ 欄位級機械相符斷言 ＋ 使用者可見代價） |
| 6 | 【依 D11】`ARCHITECTURE.md` §4.8 | 帳號類五事件之交易語意（fire-and-forget vs 同交易）之明文記載 | ✅ **已落地**——§4.8 新增兩種寫入語意之對照與 D11(a) 之明示接受（移交 PHASE-011） |
| 7 | `KNOWN_ISSUES.md` §5 PHASE-010 五項 | 逐項結清或改列 PHASE-011（依 D7 分流） | ✅ **已落地**——①~④ 結清、⑤ 依 D7=(b) 拆分（D-2／D-3 結清；D-5／D-6／D-7 改排 PHASE-011）；併 §3 D-1 結清、§4 R-4／R-5 結清 ＋ 新增 R-6 |
| 8 | `CHANGELOG.md` | PHASE-010 條目 | ⏳ **未執行——不在本 Task 之 Files Allowed**（`CHANGELOG.md` 屬大總管白名單，見 `CLAUDE.md` 治理節）。本列續留，由大總管於終審／合併批次補齊 |
...
| **T9** | 【條件，依 D6】error-handler 兜底洩漏面收斂 | BE | AC-21 | `backend/src/platform/error-handler.ts`、`backend/test/integration/phase10-error-handler-leak.test.ts`（新）、`backend/test/integration/error-handler.test.ts`（既有，零弱化確認）、**`backend/src/admin/routes.ts`**（`SPEC-REV-010-CLOSE` 追記——見下方註） | Medium | — | 小型接線／合約批次／**~110-170k** | — | 修復前反向探針必紅之物證；白名單掃描綠且新增站點必紅；`500` wire body 逐位元組不變；既有 `error-handler.test.ts` 全綠零改動 |
...
> **【`SPEC-REV-010-CLOSE` 追記】T9 之第四檔 `backend/src/admin/routes.ts`（P2003 兜底 `warn`）**：本 Phase **期中複審 #1（T5 即審 A 部）之 AR／FW-8**——「`DELETE /admin/users/:id` 之 P2003 兜底分支**完全靜默**，守門若漏補一條 RESTRICT 外鍵，運維端只見 `409` 而看不到縱深被觸發」——原記載之**分派對象為「終審」**；實際落地於 **T9**（`3bae7fd`，`admin/routes.ts` +9 行，單一 `request.log.warn({ fkField }, "USER_DELETE_P2003_FALLBACK: …")`，`fkField` 取自 Prisma `meta.field_name` 屬 schema metadata、**零 PII**、**不記 `err.message`**，與 AC-21(a) 同一理由）。**本列據實更正為「T9 落地」**。批准鏈逐筆：①**期中複審 #1 之 FW-8 建議**（觀測性候選）②**大總管 T9 Packet 之擴列授權**③**T9/T10 合併複審之附條件 CONFIRM**（裁定③「FW-8 無斷言接受」——`admin/routes.ts` 為 AC-14(d) 測試檔之 Files Forbidden 所致之結構限制，且該 `warn` 可獨立回退）④**關閉確認之「終審前完成」條件**（S-1 之處置 (a)＝SPEC-REV 追記，本列即其落地）。**射程限定**：本檔之變更**恰為該一行 `warn` ＋ 其上方註解**，`admin/routes.ts` 之任何行為面（`409` 文案、判定順序、守門呼叫）**逐字不變**；AC-21 之四段斷言**不涵蓋本行**（無專屬斷言為已裁定接受之事實，非漏測）。**與 Packet 之差異（據實記錄）**：Packet 稱同時更正「FW-8 分派記載由『終審』更正為『T9 落地』」——實查該分派之字面落點在 `PROJECT_STATE.md` :25（大總管白名單、本 Task Files Forbidden），**Spec 四檔內原無該分派字面**，故本 Phase Spec 側之更正即以本註記承載；`PROJECT_STATE.md` 之字面更正歸大總管。**另須與 PHASE-008 之同名 `FW-8` 區辨**（本 Spec :546／:1001 所引之「raw SQL baseline fixture」為 PHASE-008-T1 即審之 FW-8，與本註記無關）。
...
| T8 | 1 | 1 | E2E only | ✅ |
| T9 | 1 | **4**（`SPEC-REV-010-CLOSE`：原 3，追記 `admin/routes.ts`） | BE only | ✅ |
...
> **★ 裁定結果（Spec Gate 2026-08-09，人類 leonchih）＝ (b) 分流 ★**（依 §18 固化列逐字「其餘 8 條（D1/D2/D4/D6/D7/D9/D10/D12）**照推薦**」）。落地即 T10（`c11045a`）＝AC-22(a)(b)(c) 三項納入 ＋ (d) 之 D-5／D-6／D-7 明列不納並移 PHASE-011。〔`SPEC-REV-010-CLOSE` 補記，S-2 之防再犯——本裁定值原僅能由「照推薦」＋本節推薦欄兩段推導，未有單一處明載，致 §12 一度誤標為 `D7=(a)`。〕
...
> **【`SPEC-REV-010-CLOSE` 補記】本節與 §17.2 各條之條件式前綴（「【若 Dx 裁定 …】」）皆已於 2026-08-09 Spec Gate 解除，逐條對照如下——條文本身**逐字不動**（保留原措辭以維持與 Gate 當時之對應關係），惟其條件**均已成真、全部條文現皆生效**：**D5=(c)**（#2 生效）／**D11=(a)**（#3 生效）／**D3=(c)**（#7 生效）／**D7=(b)**（#9 生效，D-5／D-6／D-7 已於 `KNOWN_ISSUES.md` 改排 PHASE-011）／**D8=(a)**（§14 之 migration 回滾面為零）。
...
| 2026-08-10 | `SPEC-REV-010-CLOSE`（結案記載批次；Task ID `PHASE-010-DOCSYNC`） | **記載面修訂批次（零 AC 語意變更、零新 AC、零 Task Graph 結構變更、零程式影響）——終審凍結前之記載債收攏**，六項：①**S-1 追記（§15 T9 檔案欄）**——追加 `backend/src/admin/routes.ts`（P2003 兜底單行 `warn`；§15.2 T9 檔數 3→**4**），批准鏈逐筆載於 §15 表下新註（期中複審 #1 之 FW-8 建議→大總管 T9 Packet 擴列授權→T9/T10 合併複審附條件 CONFIRM「FW-8 無斷言接受」→關閉確認之「終審前完成」條件）；併記 **FW-8 分派由「終審」更正為「T9 落地」**，及其與 PHASE-008-T1 同名 `FW-8`（raw SQL fixture，:546／:1001）之區辨。②**S-2 更正（D7 裁定標記）**——**Spec 內恰一處**（§12 AC-22 列之 `D7=(a)`）更正為 **(b)**；另於 §2 AC-22 標題段與 §16 D7 推薦段各補一則**裁定值明示**（依 §18 Gate 固化列逐字「其餘 8 條……照推薦」＋ §16 D7 推薦＝(b)），使該值日後可單處查得而非兩段推導。③**AR-5 措辭更正（§2 AC-22(b)）**——「三份同形實作／三處呼叫端」更正為「**兩份**（`attachment-copy.ts`／`upload-service.ts`）／**2 檔 4 個呼叫端**（:121／:321／:319／:332）」，附 **W-1 溯源註記**（PHASE-009-T18 檔頭之「慣例同型」被登記為 KNOWN_ISSUES 條目時收斂成「同形實作」，本 Spec 再承接該失準；`health.ts`／`error-handler.ts` 之 `errName` 取 `err.name` 而本模組取 `err.constructor.name`，併入即行為變更故刻意不併）＋**反退化守門缺格**（FW-D 移交 PHASE-011）＋**程式面連帶失準**（`error-label.ts` :42-43 之「三處呼叫端」→ 登記 `KNOWN_ISSUES.md` §4 **R-6**）。④**§12 回填**——**AC-21 一列拆為兩列**（(a)(c)(d)＝T9 `3bae7fd` 7 格／(b)＝T9R `f98e7d5` 19 格，皆以實際 `describe`／`it` 名**逐字**回填並轉 `GREEN`；併載 T9R 之 **9 新格 ＋ 2 更名**、全檔 17→**26** `it`、`backend/src` 零 diff）；**AC-22 一列拆為兩列**（(a) 轉 `GREEN`（T10 `c11045a`）且落點由「既有 contract 檔擴充」更正為 **`phase9-void-report.test.ts`**（FW-E）並補兩格逐字測試名；(b)(c)(d) 標 **`N/A（非測試）`**）；**AC-20 維持 `PENDING`** 並註明終審實跑回填與各 Task 驗收實測值（後端 3542／前端 388／E2E 55）；**狀態計數行同步**為 **44 列＝`GREEN` 41／`PENDING` 1／`N/A` 2**（測試列 42），附機械核對之推導。⑤**§13 同步清單逐項核銷 ＋ 新增 Packet 追加之第 9／10 項**——第 1~7、9、10 項標 ✅ 已落地（`ARCHITECTURE.md` §3 `audit`／`platform` 兩列、§4.8（稽核檢視 ＋ 兩種寫入交易語意 ＋ D8(a) 結案）、§4.9（組裝點唯一 ＋ 兜底不記原文 ＋ 零內容標籤單一來源）、**新增 §4.13**（使用者刪除守門與八條 FK 對照）；`DATA_FLOW.md` **新增 §2.9** ＋ §3 稽核紀錄列；`KNOWN_ISSUES.md` §3／§3.1／§4／§5），**第 8 項 `CHANGELOG.md` 標 ⏳ 未執行**（不在本 Task 之 Files Allowed，屬大總管白名單）。⑥本列。**與 Packet 之差異（據實記錄，需大總管知悉）**：**(i)** Packet 指示「AC-22 列轉 `GREEN`」——實查其四子項中**僅 (a) 有測試斷言**（(b) 反退化守門格缺、(c) 依 AC 逐字為「零斷言影響」、(d) 為純記載決策），整列標 `GREEN` 將構成「聲稱未實際存在之測試證據」，故改採**拆列 ＋ (b)(c)(d) 標 `N/A（非測試）`**（沿 `SPEC-REV-010-LITE-2` 對 AC-14(c) 之既有形狀）；**(ii)** Packet 指示之「FW-8 分派記載由『終審』更正為『T9 落地』」，實查該分派字面在 `PROJECT_STATE.md` :25（大總管白名單、本 Task Files Forbidden），Spec 四檔內原無該字面，故以 §15 新註承載，`PROJECT_STATE.md` 之字面更正歸大總管；**(iii)** Packet 稱「S-2 之 D7 標記於 Spec 內有多處（§12 AC-22 列**等**）」，實查 **Spec 內恰一處**（另一處在 `PROJECT_STATE.md` :53，Files Forbidden）。 | T9/T10 合併複審（S-1／S-2／S-3／AR-5／FW-A／FW-C／FW-D／FW-E）＋T9R 關閉確認之兩項終審凍結前置條件（`PROJECT_STATE.md` 對應結案列）／大總管各裁定（W-1 兩份足額關閉、FW-8 無斷言接受、DOC-SYNC 歸屬）／人類 leonchih 2026-08-09 Spec Gate 固化列（D7 照推薦＝(b)）／大總管派工 `PHASE-010-DOCSYNC` 2026-08-10 |
