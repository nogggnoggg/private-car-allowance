...
**狀態計數（SF-B 去重後，41 列）**：`GREEN` **34**／`PENDING` **6**（T4×2／T8／T9／T10／AC-20）／`N/A` **1**／待重驗 **0**＝40 測試列＋1 N/A（大總管 2026-08-10 更正：原 AC-18(d) 預定列與實名列重複——刪預定列留實名列；合計行以實體機械計數 41 列為準）／`PENDING` **7**／`N/A（非測試）` **1**＝**41**。
