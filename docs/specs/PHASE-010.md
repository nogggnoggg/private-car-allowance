...
| AC-15(a)(b)(c) | frontend | `frontend/test/AuditLogPage.test.tsx` ＋ `frontend/test/HomePage.test.tsx`（**兩檔並列**） | ①`AuditLogPage` › `AC-15: 路由與列表欄位` › `AC-15(a)：非管理員進入 /admin/audit-logs 不渲染任何稽核內容`；`AC-15(c)：列表逐列顯示四要素（操作時間／操作者／操作類型／受影響資料）`；②（＝AC-15(b) 入口正負向）`HomePage — 稽核紀錄導覽連結` › `管理員登入 → 首頁顯示「稽核紀錄」連結，指向 /admin/audit-logs`；`一般使用者（role=USER）登入 → 首頁不顯示「稽核紀錄」連結` | T7 → **T-MG3-FE**（僅 AC-15(c) 一格） | **`PENDING（Gate 修訂後待重驗）`**（原 `GREEN` T7 `15a1a9a`；**MG-3 修訂後必紅之具體斷言**：`AuditLogPage.test.tsx` :374 `expect(screen.getByText(/user1#app-9/)).toBeInTheDocument()`——`#app-9` 依 AC-18(e-1) 不再呈現。該格須由 T-MG3-FE 重錨定為「只見 `user1`」＋「`#app-9` 字面零命中」之正負向，**不得刪格**；AC-15(a)／(b) 兩格與 `HomePage.test.tsx` **零影響**） |
