...
| AC-22 | integration | `phase6-maintenance-attachment.test.ts` | `AC-22 草稿可缺 > a draft with zero attachments saves successfully`；`… > completing with zero attachments returns 400 with MAINTENANCE_ATTACHMENT_REQUIRED`；`… > exactly 1 attachment is enough` | T6 | **PARTIAL**（T6 `ada74d1` 草稿半段 GREEN：`a draft with zero attachments saves successfully and its blockers include MAINTENANCE_ATTACHMENT_REQUIRED`＋`exactly 1 attachment is enough…`；完成端點 400 段待 T7——T6 即審 SF-3 裁定） |
