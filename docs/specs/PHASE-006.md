...
| AC-19 | unit + integration | `maintenance-calculation.test.ts`；`phase6-maintenance-complete.test.ts`（T7：超容量 400 零快照＋邊界正例）；`phase6-snapshot-immutability.test.ts`（T8：DRAFT DTO 與 preview 雙處暴露 AMOUNT_OUT_OF_RANGE） | T2, T7, T8 | GREEN（T2 `fc48e20`＋T7 `245d5f8`＋T8 `062c3b7`；檔案歸屬依 T8 即審 SF-2 更正，無雙重宣稱） |
