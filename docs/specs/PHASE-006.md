...
| AC-39 | unit(structural) | `backend/test/integration/phase6-contract.test.ts` | `AC-39 不新增 ErrorCode > errors.ts ErrorCode union equals the known baseline (mutant: adding BOGUS must fail)`；`AC-39 > PHASE-006 source files reference only baseline ErrorCode literals` | T13 | GREEN（T13：`phase6-contract.test.ts` 聯集 13 碼全等（BOGUS mutant 紅）＋本 Phase 9 src 檔字面值掃描） |
| AC-40 | integration | `phase6-contract.test.ts` | `AC-40 錯誤合約 > every PHASE-006 endpoint's 401/403/400/404/409 body has the exact closed top-level key set`（it.each 逐路徑）；`AC-40 日誌安全 > logStream contains no cookie value / password= / bearer pattern / SQL keyword / Prisma class name / absolute path`；`… > errors are still logged (never silent)` | T13 | GREEN（T13：八端點×狀態碼統一形狀（頂層鍵封閉）＋logStream 六類掃描＋錯誤不靜默；AR-1 路徑正則盲點檔頭記載） |
| AC-41 | E2E | `e2e/maintenance-allowance.spec.ts` | `保養費用分攤 — PHASE-006 Gate E2E > 情境1~6：播種差旅→建草稿→補齊四欄→上傳證明→預覽四值→完成→詳情快照→修改得 403`（六 it，播種冪等） | T14 | GREEN（T14 `90fad67`：七情境八步驟（API 播種冪等兩輪實證、上傳→儲存→完成順序、手算 1000/250/25%/1000、快照四值＋無刪除入口、修改 403）；大總管親跑 30/30） |
