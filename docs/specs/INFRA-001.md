...
- 狀態：**COMPLETED**（D1~D8 批准實作完畢；INFRA-001 主回合 PR #7 與 infra-001b 補充回合 PR #9 均經人類批准合併；26 AC 全數關閉——AC-23 熱路徑達標、冷路徑經人類 2026-08-03 裁定收為 Accepted Risk；終局測試基準線 946/0/0）
...
| AC-06 | unit | `test/unit/infra001-global-setup-guard.test.ts` | `NODE_ENV=production 時 globalSetup 護欄拒絕執行` | T1→R2 | **更正（Review M-1，2026-08-03）**：原「已完成（T1，指向 infra001-db-isolation.test.ts）」為**不實宣稱**——該測試從不存在（reviewer 以 grep 與 `git show 3120029` 雙重查證），僅 T3 曾以臨時腳本手動驗證護欄行為。修復：R2（`e74ff11`）補上正式 unit 測試 2 條，reviewer 以突變測試實證鑑別力（刪護欄→2 條全紅）。見 §17 R2 交付列 |
...
| AC-23 | evidence | — | 全套 wall time 前後對照（同機同負載） | T3→R1 | **未達標（歷程如下），待人類最終裁定**——T3 首測 ~69%（見 §17）→ 人類裁定派 R1 → R1 交付（共用連線+FK 拓撲 DELETE）增幅降至其量測之 ~40% → **reviewer 對齊重測推翻 R1 歸因**：兩邊同 47 檔之對齊量測增幅 **+25.3%**（12.08s vs 9.64s）,且 per-file 邊際成本 ≈ 0——殘餘幾乎 100% 為 globalSetup 一次性供裝 ~2.7s（佔基線 28.5%）;R1 的「isolate:true 架構性成本 47ms×53」歸因**不成立**（該數字實為供裝時間之誤歸因）。reviewer 提出第四選項（globalSetup 按需重建:比對 `_prisma_migrations` 吻合即不 drop 不 spawn,實測 no-op deploy 468ms→預期增幅 ~2%）,但需人類重裁 D3「run 前 drop」語意——**留待人類裁定:接受 ~25% or 批准第四選項**（→已批准,見 §17）。**INFRA-001b-T1 重測（2026-08-03,reviewer 獨立對齊量測,冷路徑數字經其二輪自我更正）**:**熱路徑（暖 DB,本機常態）+5.2~5.4%**（n=3+2,vs 同時段 off 基線;最壞配對 +16.4% 仍達標）——**達標,本機開發迴圈之批准標的達成**;**冷路徑（全新 DB,首跑）+35.7%**（n=3,12.99s vs 9.57s;供裝典型 2.7~3.4s,n=8;初報「+20.2% 壓線」為 n=1 離群值 1620ms 所致,已更正）——**明確超過 ≤20% 門檻**,惟**與修訂前同量級（歷史對齊量測 +25.3%）,非本回合造成之回歸**——本優化對冷路徑「不改善」而非「弄壞」。**CI 每 job 全新 postgres container 恆走冷路徑 ⇒ CI 恆處超標側且零收益**（CI maxForks=2 絕對成本較低）。AC-22（≤30s）冷熱均大幅達標（最慢 3380ms）。**冷路徑之 AC-23 已由人類裁定收為 Accepted Risk（2026-08-03,leonchih,「批准」,連同 PR #9 合併批准）**——冷路徑僅發生於全新 DB 首跑與 CI,絕對成本 ~3s 供裝,與修訂前同量級。**AC-23 至此終局:熱路徑達標、冷路徑 Accepted Risk,26 條 AC 全數關閉,本 Spec 具備轉 COMPLETED 條件** |
...
| 2026-08-03 | 合併後裁定三項（人類批准，infra-001b 回合） | ①**AC-23 第四選項批准**：D3 第 5 點「run 前 drop」修訂為「按需重建」（AC-09 修訂列即新語意；比對 `_prisma_migrations` 吻合即沿用），目標增幅 ~25%→~2%，於 branch `infra-001b` 實作，AC-22/23 屆時重測。②**NFR-3 追認選項 A**：連線策略定稿為「每檔一條共用至檔尾（實測峰值 25/100）」，理由：退回逐次斷線付出真實效能代價、消除的是不存在的風險（餘裕 4 倍且原文「不增加」無基準不可證）。NFR-3 之「待人類於 Gate 確認」註記據此解除。③新追蹤項（D4-Decimal-number ×4、4xx status 對照表）排入 CHORE-002，於本回合後、PHASE-005 前執行。 | 使用者裁定（2026-08-03，leonchih，「按照你的建議執行」）|
| 2026-08-03 | R2 交付 + 輕量複審 | **R2（`e74ff11`）**：M-1 補 AC-06 護欄 unit 測試 ×2（`infra001-global-setup-guard.test.ts`）；S-3 清空 0 表路徑改 throw + `WorkerSchemaSqlExecutor` 結構性注入介面（unit 測試 ×3）；S-4 AC-09 改即時查 `information_schema.tables` 獨立見證。兩模式全套 **927/0/0**。**輕量複審：技術面全過（0 Must Fix）**，reviewer 以突變測試實證兩組新測試鑑別力（刪護欄/還原 return → 4/5 轉紅）。遺留 3 件 Should Fix：S-A（guard 測試檔註解不實 + 缺 DATABASE_URL 護具→R3 Lite）、S-B（本列與 AC-06 檔案欄更正→已由本次修訂完成）、S-C（**NFR-3 語意由「不增加連線峰值」調整為「受控（實測 25/100）」屬規範性放寬，超出既有人類裁定範圍，待人類於 Gate 確認**——R1 前無峰值基準故「不增加」不可證，reviewer 判實質風險 Low-Medium）。 | R2 依 reviewer REQUEST_CHANGES；S-B/S-C 記載由大總管執行（白名單） |
