...
| AC-11 | unit | `backend/test/unit/report-html.test.ts` | `AC-11: 三型列印版必含項目逐字在場（共通 7 欄位列 ＋ 類型大標題 + 差旅 / 保養 / 折舊各自清單）`；`AC-11: 報表標題欄位列不再出現、類型大標題仍在場（裁-2；還原欄位列之 mutant 必紅）`；`AC-11: 產生時間與申請建立時間以台北時區中文格式呈現（裁-1；移除格式化之 mutant 必紅、格式化不讀時鐘）`；`AC-11: 差旅油種以中文名稱呈現且 LEGACY 之 null 仍為「—」（裁-3；顯示列舉代碼之 mutant 必紅）` | T5 ＋ **MOCK-R** | PENDING（**後三條為 Mock Gate 裁定之預定名，待 MOCK-R 落地後實名回填**） |
...
| AC-30 | frontend | `frontend/test/ReportSection.test.tsx`；三型頁既有測試檔 | `AC-30: 已完成申請顯示報表區塊，產生成功後逐字顯示編號與產生時間`；`AC-30: 草稿不渲染報表區塊（負向斷言）`；`AC-30: 三型詳情頁皆掛載報表區塊`；`AC-30: 產生時間以台北時區中文格式呈現且格式化結果由 DTO 值推導（裁-1；自組時間值之 mutant 必紅）` | T12 ＋ T13 ＋ **MOCK-R** | PENDING（**末條為 Mock Gate 裁定之預定名，待 MOCK-R 落地後實名回填**） |
