...
> **【修訂（2026-08-07；Mock Gate 三裁定之呈現面落地；批准依據＝`PROJECT_STATE.md` 2026-08-07「★ Mock Gate 通過（人類 leonchih 2026-08-07）＋三裁定（AskUserQuestion 複述確認後落檔）★」列，commit `5fd2061`；走查腳本 `docs/verification/WALKTHROUGH-PHASE-008-MOCK.md`）】** 上表**逐字保留**，其**呈現面**依人類逐案裁定修訂三點：
>
> **⒜【裁-2】「報表標題」欄位列移除**：共通項之「報表標題」**不再以欄位列（標籤＋值）呈現**，該資訊改**僅由版面大標題 `<h1>{類型}報表` 承載**（Gate 現場：欄位列「報表標題：差旅報表」與緊接之大標題「差旅報表」資訊重複）。**AC-11 語意不變**——列印版仍**必含類型標題**（缺大標題必紅）；共通必含項之計數表述由「8 個欄位列」改為「**7 個欄位列 ＋ 1 個類型大標題**」，**資訊項數與「缺任一項必紅」之守門強度均不變**。
>
> **⒝【裁-1】時間欄位之在地化**：共通項之**「產生時間」與「申請建立時間」**一律以**台北時區（UTC+08:00）之中文在地化格式**呈現（Gate 現場：原樣輸出 `2026-08-06T16:32:33.910Z`，與同頁「計算時間」之中文格式 `2026/8/6 上午9:23` 並排不一致；**使用者確認之範圍釐清＝兩欄一併改，同頁時間格式全一致**）。**目標呈現＝與前端既有「計算時間」同一長相**（`2026/8/6 上午9:23` 之形狀，含 `上午`／`下午` 中文時段標示）。**「尚未產生」之佔位字串不變**（`generatedAt` 為 `null` 時不進入格式化）。前端報表區塊之「產生時間」同批同步——見 **AC-30 之修訂引註**。
> **【BE／FE 秒差之容忍（2026-08-07 補記；終審 AR-3）】** 落地後兩處格式**不逐字相同**：**列印版／PDF（後端）**為決定性自組字串、**不含秒**（`2026/8/6 上午9:23`）；**前端報表區塊**沿用既有慣例 `new Date(...).toLocaleString("zh-TW")`（`ReportSection.tsx` :141），**含秒**。裁-1 之意旨為「**台北時區＋中文在地化、與同頁計算時間一致**」——同頁之「計算時間」本即以同一 `toLocaleString("zh-TW")` 呈現，故前端此形狀**正是**裁定所要求之一致；秒之有無屬**裁定容忍範圍內**，**不視為 AC-11／AC-30 之違反**。若日後要求兩處逐字一致，屬**新的呈現面裁定**，須人類批准後另批修訂。
>
> **⒞【裁-3】油種以中文名稱呈現**：列印版之**「油種」**欄位不得顯示列舉代碼（`GASOLINE_92` 等），一律顯示中文名稱——`92 無鉛汽油`／`95 無鉛汽油`／`98 無鉛汽油`／`柴油`（與前端既有下拉選項用字**逐字一致**）。**LEGACY 之「—」呈現不變**（`dash()` 語意優先於翻譯：值為 `null` 時恆為 `—`，不得譯出任何文字）。
> **勘誤（如實記載，未擅自擴大範圍）**：裁-3 之裁定原文與走查腳本步驟 6 記為「**保養**報表的油種」，惟實查版型——**「油種」欄位僅存在於差旅列印版**（`backend/src/reports/report-html.ts` :196 `renderTravelBody`；上表「差旅」列本即列有該欄），**保養列印版（同檔 `renderMaintenanceBody`）與 `MaintenanceReportBody`（`backend/src/reports/report-data.ts` :121-132）皆無油種欄位**。故本引註將 ⒞ 落於**差旅**列印版之油種欄位（裁定實質＝「油種不得顯示英文代碼」，不因型別歸屬而改變）。**若人類原意另含「保養報表應新增油種欄位」，則屬新增揭露欄位＝擴大 Scope 與揭露面之變更，本 Spec 未納入，須回人類批准後另批修訂。**
>
> **契約不變式（明文確認，防 implementer 誤解）**：①**`renderReportHtml` 仍為純函式、仍不讀時鐘**——在地化係**格式化傳入之 `ReportData` 值**（`common.generatedAt`／`common.createdAt`），**不得**以 `Date.now()`／無參數 `new Date()` 取現在時刻；同輸入兩次呼叫仍 `toEqual`（**AC-15 第一句不受影響**）。②**AC-15 之「列印端點 HTML 與 PDF 所用 HTML 字串全等」不受影響**——兩者同函式、同輸入，格式化發生於同一路徑。③**`ReportData` 之欄位型別與值不變**（`generatedAt`／`createdAt` 仍為 ISO 8601 字串），在地化**發生於版型（呈現）層**——故 **AC-27 鍵集封閉與 AC-17／AC-18／AC-19 之資料層斷言一律不受影響**。④**格式化必須是決定性的**：不得依賴執行主機時區（`toLocaleString` 未帶 `timeZone` 者受 `TZ` 環境變數左右），**亦不得依賴 ICU locale 資料之在場**（Node 之 small-icu 建置會使 `zh-TW` 靜默回退為 `en-US`，屬「開發機綠、容器紅」之型態，與 T14 之字型分歧同類風險）——**建議沿 `report-number.ts` 既有作法**（固定 `TAIPEI_OFFSET_MS = 8h` ＋ 一律以 `getUTC*` 讀出後自行組字），零 `Intl` 依賴；若採 `Intl`，須另加「ICU 在場」之測試守門。
>
> **實作落點建議（內部技術細節；implementer Packet 可直接引用）**：⒤**油種中文映射之單一來源**——於後端新增**唯一一處**窮舉映射（建議 `backend/src/reports/report-labels.ts`，形如 `Record<FuelType, string>`，比照 `report-number.ts` 之 `REPORT_NUMBER_PREFIX` 以**型別窮舉**取得「新增油種必編譯失敗」之守門），版型層單一引用；**不得**於版型函式內就地散寫 `if`／`switch`。⒤⒤**與前端既有映射之關係（如實記載）**——前端現有**三份**相同映射（`frontend/src/components/MyFuelConsumptionSection.tsx` :33-36、`frontend/src/pages/AdminFuelConsumptionPage.tsx` :59-62、`frontend/src/pages/ParametersPage.tsx` :71-74），本次後端新增者為**後端側之第一份且唯一一份**；**FE／BE 跨層合併為單檔不在本次範圍**（前端三份重複屬**既存**狀態，本 Phase 未新增亦未處理，記為後續整理候選）——此即 T5 AR「若須翻譯則單一共享來源、勿再開第四份」於本次之落地解讀：**後端只允許一份**。⒤⒤⒤**時間格式化**同理以**一處共用純函式**承載（列印版兩欄共用）；前端報表區塊之格式化沿前端既有慣例（三型詳情頁「計算時間」之 `toLocaleString("zh-TW")`）以維持同頁一致。
...
> **【修訂（2026-08-07；取證手段變更，驗收語意零變更）；批准依據＝§18 `PHASE-008-SPEC-REV-T14` 列＋`PROJECT_STATE.md` 2026-08-06「T14 第一輪：BLOCKED」裁定列（commit `64314f0`，方案 (a) 幾何變體）】** 上句所述之量測手段——「**於列印媒體模擬下**量測 `top`／`bottom-1` 之 `floor(y / 頁面內容高度)`」——**經 T14 探針實證對本 AC 無鑑別力**（`emulateMedia({ media: "print" })` 於一般 Playwright 分頁不觸發 Blink 分頁／分片，有無 `break-inside: avoid` 之 `getBoundingClientRect()` 逐位元組相同），故**取證手段改為 §11.4 【方法論修訂】引註所述之「真實後端 PDF 之零依賴幾何量測」**（`page.pdf({ preferCSSPageSize: true })`，與 `pdf-renderer.ts` 同管線；`node:zlib` 解 `FlateDecode`，以 `.segment` 邊框之**封閉子路徑** bbox ＋ `.image-block` 影像 XObject 之 `cm` 置放矩形為**字型無關**之幾何訊號）。**驗收語意逐字不變**：每個標記為不可分割之區塊**不得被頁面邊界切開**（頁索引一致；於幾何量測中之等價證據＝全文件**封閉**邊框子路徑總數恆等於段落數——切割使起訖點不再重合而計數短少），且**任兩區塊之矩形互不重疊**（同頁內兩兩比對）；**「移除 `break-inside: avoid` 之 mutant 必紅」之義務一字不減**（實測鑑別力 6 → 15）。**量測範圍限制**：`.reconciliation` 無邊框亦無影像，不在幾何量測範圍——如實記載見 §11.4 該列。
...
> **【修訂（2026-08-07；Mock Gate 裁-1；批准依據＝`PROJECT_STATE.md` 2026-08-07 Mock Gate 三裁定列，commit `5fd2061`）】** 上句「**逐字顯示報表編號與產生時間**」之「逐字」定義於此收口（**此即 T12~T13 複審 FW 預留之「AC-30 逐字定義依 Gate 裁定回填」點**）：
> ①**報表編號**——維持**嚴格逐字**：值取自產生端點回傳之 `ReportDto.reportNumber`，前端**不自組、不轉換、不截斷、不重排**（**自組編號之 mutant 必紅**）。
> ②**產生時間**——由「原樣逐字」改為「**值取自 `ReportDto.generatedAt`、以台北時區中文在地化格式呈現**」（與同頁「計算時間」一致；理由見 AC-11 修訂引註 ⒝ 之 Gate 現場）。**mutant 牙齒逐條保留**：格式化結果**仍須由 DTO 之值推導**，以「DTO 給定值 → 期望顯示字串」之對照斷言釘住；**前端自組時間值之 mutant 必紅**（改用 `Date.now()`／忽略 DTO 值／寫死字串三種變異皆須紅）。
> ③**其餘一律不變**：未產生時之「產生正式報表」按鈕、`DRAFT` 整區不渲染之負向斷言、成功後兩入口之出現，以及 AC-31／AC-32／AC-33 全數不受影響。
...
| **Success（POST 觸發）** | 於本次工作階段**點擊「產生正式報表」成功** | 報表編號、產生時間、檢視列印版、下載 PDF | `201 { report: ReportDto }`（首次）／`200`（冪等再次） |
| **Success（GET 觸發）** | **進入頁面時該申請已產生報表**（`mount` 之查詢直接回傳） | 同上；**且產生按鈕不在場、零 `POST`** | `200 { report: ReportDto }` |
...
| storage 前綴 ＋ nginx 直出守門 | `backend/test/unit/storage.test.ts`（**既有檔，只增不改**）＋ `backend/test/unit/report-storage.test.ts` | AC-26；`rpt` 前綴之路徑穿越矩陣重跑；跨實例互拒；既有 `att` 測試逐條零改動（diff 檢查） |
...
> **【AC-09(c) 取證方式（2026-08-07 記錄型修訂；終審 SF-2；大總管裁定採 (a)；依據＝T7b 裁定①已在案 ＋ 與 `SPEC-REV-T14` 同類先例）】** 上表「PDF 渲染器」列所述之 **AC-09(c) 中文可見度量測**（`document.fonts.check()` ＋ 字寬量測雙層斷言）**未落地為 vitest 常駐測試**，**實查 `phase8-pdf-render.test.ts` 零命中**。**修訂後之取證方式為兩層，皆已實際執行且可重跑**：①**容器內 probe**（T7b／T7bR）——於**正式容器**內執行三層法：字型清單列舉、`document.fonts.check()`、**含 tofu（豆腐方框）對照之像素簽名**比對（渲染同一中文字串於「有字型」與「強制缺字型」兩情境，比對像素簽名必不同——使「方框亦可通過」之偽陽性封閉）；量測結果見 T7b／T7bR Handoff。②**整合 Gate 目視**——判準為**繁體字形正確**（非僅「無方框」），列入整合 Gate 揭露目視清單。**probe 步驟已固化於 `docs/verification/WALKTHROUGH-PHASE-008-INTEGRATION.md`，任何人可重跑。**
>
> **無 vitest 常駐守門之理由（方法論，非省略）**：本 AC 之風險面是**容器映像之字型安裝**，**不是**程式邏輯。開發機（Windows／macOS）本就有系統中文字型，於開發機執行之 vitest **無論容器是否漏裝字型都會綠**——即該測試對目標回歸**先天無鑑別力**，屬「恆真式守門」，與 §11.0「mutant 必紅」之紀律相牴觸。真正有鑑別力之執行環境是容器本身，故取證改由**容器 probe ＋ Gate 目視**承擔，並以**可重跑之走查腳本**保存重現能力。**AC-09(c) 之驗收語意零變更**（中文字型在場、字形正確）；**§12 該列狀態標為 `GREEN（文件化取證，非自動化測試）`，不得被讀為「有自動化測試通過」**。**已知限制**：容器映像若日後移除字型安裝步驟，**不會**有自動化測試變紅——防線為 Dockerfile 變更之 review 與整合 Gate；此限制已如實記載，屬本 Phase 明示接受之殘留風險。
...
>
> **【鑑別力歸屬之澄清（2026-08-07 補記；T14R 關閉輪 FW）】** AC-14 之兩項幾何斷言**鑑別力不對等，須明記以免誤讀**：①**`.image-block` 影像置放矩形之計數**（每段恰一張）對 `break-inside: avoid` **天生不敏感**——影像為不可分割之單一 XObject，無論是否被保護，其 `Do` 置放次數恆等於圖片數，故該斷言**不承擔 mutant 必紅之義務**（其作用為「圖片確實在場且同頁不重疊」之輔助錨點）。②**AC-14 之 mutant 必紅義務，僅由 `.segment` 邊框之封閉子路徑計數承擔**（切割使起訖點不再重合 → 計數短少；實測 6 → 15）。**後續維護者不得以「影像矩形數仍正確」推論版面未被切開**，亦不得在弱化 `.segment` 封閉子路徑斷言後宣稱 AC-14 仍有守門。
>
> **【互指同步（2026-08-07；`PHASE-008-SPEC-REV-T14B`）】** 上段原文末句「限制已記於 §17.1」之指涉**已同批同步**：§17.1 #1 之原敘述（「非於 PDF 位元組層解析驗證——後者需新增 PDF 解析相依」）**已失效並經同日引註更新**，續存之限制改為⒜幾何量測涵蓋範圍（`.reconciliation` 除外）與⒝`ToUnicode` 文字擷取僅限「不存在」方向兩點——以 **§17.1 #1 之 2026-08-07 引註**為準。**§2 AC-14 之取證手段**亦於同批加註引註指向本節（**驗收語意與 mutant 必紅義務逐字不變**）。
>
> **【PDF 字型內嵌敘述之更正（2026-08-09；`SPEC-REV-008`；記載型勘誤，零斷言／零 AC／零決策變更。更正出處＝`PHASE-009-T18` AC-40(a) 與 `PHASE-008-T8R2` **SF-7** 實測）】** 上方【方法論修訂】②末段原記「**免疫** dev（Windows、CJK 字型缺裝 → 逐字 `/Subtype /Type3` 內嵌）與正式容器（已裝字型 → `Type0`／CID）之字型內嵌管線分歧」——**該技術敘述失準，據實更正如下**：⒜**實測（本機真實 Chromium）之持久化 PDF 文字係以「子集字型（subset font）之 glyph 編碼」繪製**——PDF 內嵌 `/FontFile` ＋ `/ToUnicode` CMap **皆在場**、文字操作子為一般 `Tj`／`TJ`（SF-7 實測 5 個），**並非** `Type3` 之逐字向量化描繪；文字**原則上可還原**（經 `/ToUnicode` 反查 glyph index → Unicode），僅「對原始位元組直接做子字串比對」不可命中（glyph 索引非 ASCII 碼點）。⒝隨之，「正式容器為 `Type0`／CID、與 dev **不同構**」一句**同屬當時之推定而非實測結論**——已取得之量測僅涵蓋本機環境；正確之如實記載為「**無法排除**兩環境之字型內嵌管線有差異」。⒞**初版 `ToUnicode` 錨點法失敗之根因，應記為「本機回退鏈下之子集字型 CMap 覆蓋不完整」**（物件級 dump 實證：`SEGORIGIN1` 之 `S` 字元 glyph code 於對應字型 CMap 內無條目），**與是否 `Type3` 無關**。**本更正之必要性**：若誤信「文字已向量化」，後續 Phase 會錯誤地認定 PDF 無文字層而放棄一切文字面斷言。**一律不變**：AC-13(b)／AC-14／AC-34 之字面與驗收語意、AC-14 之 mutant 必紅義務與其歸屬（僅 `.segment` 封閉子路徑承擔）、改採**字型無關幾何訊號**之裁定與其理由（幾何運算子在任何字型下逐位元組相同，故對上述任何字型管線差異一律免疫——更正後理由**更強而非更弱**）、以及上方③與 §17.1 #1 ⒝之限制「`ToUnicode` 文字擷取僅安全用於『不存在』方向」（子集 CMap 覆蓋不完整只會造成**偽陰性**，故該單向限制**不因本更正而放寬**）。**權威出處三處一致**：`backend/test/integration/phase8-report-generate.test.ts` :586-605（T8R2 SF-7 實測記載）、`e2e/report-print-layout.spec.ts` :50-59（`PHASE-009-T18`／`T19` 已更正之同型敘述）、本節本段。
...
|---|---|---|---|---|---|
| AC-01(a)~(e) | integration | `phase8-migration-safety.test.ts` | `AC-01(a): Report 表之欄位集合、可空性、型別與四個唯一約束逐欄符合 §8.1`；`AC-01(b): 既有 15 張表逐欄零改寫（含突變自證：ADD COLUMN 必紅）`；`AC-01(c): 乾淨與既有 DB 各一輪 migrate deploy 成功且冪等`；`AC-01(d): Application 欄位集合前後全等（report 為虛擬關聯欄，零 DDL）`；`AC-01(a): the new table's column set, nullability and numeric precision match Spec §8.1 逐欄`；`AC-01(a): the new table's PK, FK (onDelete: Restrict), four unique constraints and one general index exist`；`AC-01(a): a fully-populated row writes and reads back byte-for-byte (every column is required — no draft/null state exists for Report)`；`AC-01(a): FK onDelete: Restrict — deleting an Application that has a Report is rejected by the DB`；`AC-01(a): the four unique constraints each independently reject a collision (applicationId / reportNumber / storageKey / prefix+period+sequence)`；`AC-01(b): every pre-existing column, across all nine representative tables, is byte-for-byte unchanged after migration`；`AC-01(b) 突變自證: the zero-rewrite comparator is discriminating — a single rewritten value, and a single ADDed column, each make it red`；`AC-01(b): the migration SQL performs zero ALTER on any pre-existing table, zero DROP/INSERT/UPDATE/ALTER TYPE`；`AC-01(c): prisma migrate deploy succeeds end-to-end against a brand-new empty DB, and is idempotent on re-run`；`AC-01(c): prisma migrate deploy succeeds incrementally against an existing (pre-Phase) DB`；`AC-01(c): the new Report table exists and is empty (零回填) after migrating an existing DB`；`AC-01(c): re-running migrate deploy against the already-upgraded existing DB is idempotent (no-op, still succeeds)`；`AC-01(d): \`Application.report Report?\` is a zero-DDL virtual relation — the Application column set is unchanged from PHASE-007`；`AC-01(e) enum union baseline — all seven enums exactly match the pre-T1 set (no silent additions)` | T1 | GREEN |
| AC-01(f) | integration | `infra001-isolation-self-check.test.ts`；`phase6-migration-safety.test.ts`；`phase7-migration-safety.test.ts` | `AC-01(f) 表數 15→16 — Report is the 16th business table`（新增之逐字測試）；既有測試之斷言值 15→16（**測試名零改動**，故無新實名）——`AC-08: worker schema 含全部 16 張業務表…`（`infra001-isolation-self-check.test.ts`）、`Done When: MaintenanceApplication 仍在 16 張業務表中`（`phase6-migration-safety.test.ts`）、`phase7-migration-safety.test.ts` 之更新位於既有測試**斷言值內**（非測試名，無可回填之實名） | T1 | GREEN |
| AC-01(g) | integration | `phase8-migration-safety.test.ts` | `AC-01(g): RATIO_DISPLAY_SCALE 等於 DepreciationApplication.snapshotRatio 之 numeric_scale（常數側與 DDL 側各一 mutant 必紅）`；`AC-01(g): DepreciationApplication.snapshotRatio numeric_scale equals RATIO_DISPLAY_SCALE in both depreciation-calculation.ts and depreciation-service.ts`；`AC-01(g): MaintenanceApplication.snapshotRatio numeric_scale equals the display scale in maintenance-calculation.ts and maintenance-service.ts`；`AC-01(g) 雙側 mutant 自證: a constant changed to 4, or a DDL changed to scale 4, each independently make the comparator red`；`AC-01(g) 讀取器鑑別力自證: readNamedConstant / readMaintenanceRatioToFixedScale actually parse the live source files (not a hardcoded stub)` | T1 | GREEN |
| AC-01(h) | integration | `infra002-schema-drift.test.ts` | `schema.prisma 與 prisma/migrations 逐一套用後之狀態完全一致（prisma migrate diff --exit-code = 0）；影子 schema 用畢即清`；併同落地之破壞性 DML 常設守門（T1 即審 AR-1 承接）：`prisma/migrations 下沒有任何一個 migration.sql（白名單外）含 DELETE FROM / TRUNCATE`；`mutant 自證: findDestructiveStatements 對 DELETE FROM / TRUNCATE 有鑑別力，一般 DDL 不誤判`；`allowlist 自證: 白名單命中之目錄即使含破壞性語句亦被跳過（以暫存目錄操作，不觸碰真實 migrations 目錄）` | T1b | GREEN |
| AC-02 | unit | `backend/test/unit/report-number.test.ts` | `REPORT_NUMBER_PREFIX / getReportNumberPrefix — AC-02` › `TRAVEL → TRV`／`MAINTENANCE → MNT`／`DEPRECIATION → DEP`／`映射恰為三型窮舉（無多餘、無缺漏鍵）`／`三前綴值互異（鑑別「對調」mutant 的必要前提——若 TRV/MNT 互換，上方個別斷言即紅）` | T2 | GREEN |
| AC-03 | unit | `report-number.test.ts` | `AC-03(a): 編號格式為 {PREFIX}-{YYYYMM}-{NNNN} 且逐段值精確`；`AC-03(b): 月份依 Asia/Taipei 換算 — 四點邊界（UTC mutant 必紅）`；`AC-03(c): sequence 補零與 10000 之五位延伸`；`getReportNumberPeriod — AC-03(b) Asia/Taipei 四點邊界` › `2026-08-31T15:59:59Z → 202608（台北 08/31 23:59:59，仍屬 8 月）`／`2026-08-31T16:00:00Z → 202609（台北 09/01 00:00:00，跨入新月）`／`2026-12-31T16:00:00Z → 202701（台北跨年 01/01 00:00:00）`／`2026-01-01T00:00:00Z → 202601（台北 01/01 08:00:00，仍屬 1 月）`／`YYYYMM 不受主機時區影響（TZ 覆寫下四點邊界不變）`；`buildReportNumber / generateReportNumber — AC-03(a)(c) 格式與序號` › `格式逐字為 {PREFIX}-{YYYYMM}-{NNNN}，例：TRV-202608-0001`／`逐段值精確等值（非僅正則寬鬆匹配）`／`sequence=10000 → 自然延伸為 5 位「10000」，不截斷、不歸零、不拒絕`／`generateReportNumber 整合三型 + 月份 + 序號之完整管線`；`編號字元集不變式 — AC-03(d)` › `三型 × 5 個跨年月份 × 7 個序號（含五位延伸）之全數組合皆符合 [A-Z0-9-]+ 且逐段可反解` | T2 | GREEN |
| AC-04 | integration | `phase8-report-generate.test.ts` | `AC-04(a): 同月同型連續 5 份序號 0001~0005 且互異`；`AC-04(b): 不同型別同月份互不干涉`；`AC-04(c): 12 筆併發產生之編號集合恰為 0001~0012（無跳號、無重複）`（**方案 (d) 結構下重驗；併發輪以合成 PDF 替身，替身不繞過配號/put/讀回校驗/INSERT；顧問鎖排隊下 12 筆全數成功**）；`AC-04(c) 守門: pg_advisory_xact_lock 於冪等再查與配號之前呼叫（移除或後移之 mutant 必紅）`；`AC-04(c) 守門: 交易隔離等級為 ReadCommitted（改回 Serializable 之 mutant 必紅）`；`AC-04：同月同型多份皆唯一（含併發）` › `(a) 同月同型連續 5 份 → 序號互異且恰為 base+1..base+5`／`(b) 不同型別同月份互不干涉（TRV 與 MNT 序號各自獨立累計）`／`(d) 序號重試 mutant 必紅（正向對照）：DB 唯一約束確實擋下手動配置之重複序號插入…`；`T8R4：交易內呼叫 pg_advisory_xact_lock（顧問鎖移除 mutant 必紅）`；`T8R4：顧問鎖呼叫為交易首語句——早於冪等再查（tx.report.findUnique）與配號（buildReportNumber／tx.report.aggregate），且位於 $transaction(...) 回呼內部（鎖後移 mutant 必紅；鎖搬出交易 mutant 必紅）`；`T8R4／SF-4：交易隔離等級恆為 ReadCommitted（改回 Serializable 之 mutant 必紅——T8R3 三層獨立複現已證明 Serializable 下顧問鎖排隊機制不成立）`；`T8R4：同組（相同 numberPrefix／numberPeriod）併發 2 筆、各 500ms → 顧問鎖排隊使總耗時貼近序列化下限（~1000ms，正向對照——證明鎖確實在同組生效，ReadCommitted 下亦然）`；`T8R4：不同組（不同 numberPrefix，同一 numberPeriod）併發 2 筆、各 500ms → 顧問鎖不阻擋，總耗時貼近單筆耗時（若鎖鍵誤用共用常數而非逐組雜湊，本測試必紅）` | T8 ＋ **T8R／T8R2／T8R3／T8R4** | GREEN |
| AC-05 | integration | `phase8-report-generate.test.ts` | `AC-05(a): 同一申請重複 POST 不新增列與檔，首次 201 其後 200 且編號/時間/檔名/雜湊全不變`；`AC-05：同一申請不重產編號` › `(a) 重複 POST → 首次 201、其後 200，零新增列／檔，reportNumber/generatedAt/fileName/contentHash 全不變`／`(b)(精神；下載端點屬 T9) 第二次 POST 完全零渲染——renderPdf 於本次呼叫前後呼叫次數不變`（下載側之零渲染由 AC-08 列之 `phase8-report-download.test.ts` 承擔） | T8 | GREEN |
| AC-06 | integration | `phase8-report-generate.test.ts` | `AC-06：首次產生成功之完整保存（七項同時成立）`（describe 層載體）；併同結構性守門：`AR-1：INSERT 明寫 generatedAt（不仰賴 DB @default(now()) 之隱式時間）` | T8 | GREEN |
| AC-07 | integration | `phase8-report-generate.test.ts` | `AC-07: 四種失敗注入（RENDER/STORE/VERIFY/PERSIST）各自零殘留 — 零 Report 列、零 storage 檔、500 REPORT_GENERATION_FAILED、申請快照逐欄不變`（**STORE 情境須斷言交易內 put 之補償刪檔確有執行，非僅「無殘留」之恆真式**）　→　**實名**：`AC-07：產生失敗零殘留、不得標示成功` › `①HTML 渲染拋錯 → 500 REPORT_GENERATION_FAILED { stage: RENDER }，零殘留`／`①'（T4-FW4 大總管裁定釘住）快照缺欄（COMPLETED 但必要快照欄為 null）觸發 report-data.ts 之 required() 拋錯 → 同一 RENDER 階段收斂，非 buildReportData 之特例`／`②PDF 渲染器拋錯／逾時 → 500 REPORT_GENERATION_FAILED { stage: RENDER }，零殘留`／`③storage.put 拋錯 → 500 REPORT_GENERATION_FAILED { stage: STORE }，零殘留`／`④寫檔後讀回校驗不符（位元組數不符）→ 500 REPORT_GENERATION_FAILED { stage: VERIFY }，零殘留（已寫入之檔已被清理）`／`④'（雜湊不符變體，T8R 更正——corrupted buffer 改由當次實際 pdfBytes 長度構造）：讀回位元組數相同但內容不同（雜湊不符）→ 500 REPORT_GENERATION_FAILED { stage: VERIFY }，零殘留；雜湊臂 mutant（移除 SHA-256 比對）必紅` | T8 ＋ **T8R** | GREEN |
| AC-08 | integration | `phase8-report-download.test.ts` | `AC-08：兩次下載位元組全等 ＋ 渲染器零呼叫` › `兩次下載之回應位元組逐位元全等、等於保存值；Content-Type: application/pdf；renderPdf/renderReportHtml 於下載期間零額外呼叫` | T9 | GREEN |
| AC-09(a)(b) | integration | `phase8-pdf-render.test.ts` | `AC-09(a): 標準 fixture 之產生端到端 <= 10000 ms（量測值記入 Handoff）`；`AC-09(a) 效能守門`（describe 層載體；量測值記入 Handoff）；`AC-09(b) 逾時守門`（describe 層載體）；併同 PDF 校驗與版面幾何：`PDF 校驗（§9.1 步驟 5：%PDF- 開頭／%%EOF 結尾／非零長度）`、`SF-1：PDF 輸出版面幾何為 A4（preferCSSPageSize，非預設 US Letter）` › `輸出 PDF 之 /MediaBox 為 A4 幾何（約 594~596 × 841~843 pt），而非 US Letter（612×792）` | T7 ＋ T8 | GREEN |
| AC-09(c) | **文件化取證**（無 vitest 常駐守門） | —（`phase8-pdf-render.test.ts` **無**對應測試；實查零命中） | **取證＝①T7b／T7bR 之容器內 probe**（Handoff 三層法：字型清單、`document.fonts.check`、**含 tofu 對照之像素簽名**）**＋②整合 Gate 目視**（判準含**繁體字形正確**，非僅「無方框」）；probe 步驟已固化於 `docs/verification/WALKTHROUGH-PHASE-008-INTEGRATION.md`（**可重跑**）。**無自動化常駐守門之理由與方法論**見 §11.2 之【AC-09(c) 取證方式】引註 | T7b | **GREEN（文件化取證，非自動化測試）** |
| AC-10 | integration | `phase8-report-generate.test.ts` | `AC-10: 同一申請 2 併發產生恰一份 Report、兩回應編號相同、storage 恰一個 key（零孤兒）`（**方案 (d) 結構下重驗；敗方之交易內 put 於回滾後補償**）　→　**實名**：`AC-10：同一申請併發產生` › `2 個併發 POST → 恰一份 Report、兩回應皆成功且編號相同、storage 恰一個 key（零孤兒）；T8R4 更新：顧問鎖使敗方於 (4)b 冪等再查即收斂，結構上不再有機會抵達 (4)f put（零浪費，非『先浪費後補償』）——以 put/delete spy 正向斷言鑑別新機制` | T8 ＋ **T8R** | GREEN |
| AC-11 | unit | `backend/test/unit/report-html.test.ts` | `AC-11: 三型列印版必含項目逐字在場（共通 7 欄位列 ＋ 類型大標題 + 差旅 / 保養 / 折舊各自清單）`；`AC-11: 報表標題欄位列不再出現、類型大標題仍在場（裁-2；還原欄位列之 mutant 必紅）`；`AC-11: 產生時間與申請建立時間以台北時區中文格式呈現（裁-1；移除格式化之 mutant 必紅、格式化不讀時鐘）`；**實名**：`AC-11 — 三型列印版必含逐字標題與值` › `差旅：共通 7 欄位列 ＋ 1 類型大標題〔裁-2〕＋ 差旅專屬全數在場`／`保養：共通 7 欄位列 ＋ 1 類型大標題〔裁-2〕＋ 保養專屬全數在場`／`折舊（CURRENT）：共通 7 欄位列 ＋ 1 類型大標題〔裁-2〕＋ 折舊專屬五值全數在場`／`折舊禁列（AC-18(b) 之落地防線）：車價／折舊年限／預估年度行駛公里數／折舊參數版本 id 之字面字串零出現`／`折舊 CURRENT 對帳列（AC-18(c)）：以三來源值呈現算式`／`差旅取整前總額對帳列（SF-2；AC-11「取整前總額之對帳列」）：.reconciliation 區塊內完整算式子字串逐字`；**裁-1**：`formatTaipeiDateTime（裁-1）— DTO 給定 ISO 值 → 期望顯示字串` › `與前端「計算時間」同一長相之範例：2026-08-06T01:23:00.000Z（+8h＝台北 9:23）→ 「2026/8/6 上午9:23」`／`上午側`／`下午側`／`跨日邊界`／`正午邊界`／`分鐘補零` 六則；**裁-3**：`getFuelTypeLabel（裁-3）— 四值逐字對照` › `GASOLINE_92 → 「92 無鉛汽油」`／`GASOLINE_95 → 「95 無鉛汽油」`／`GASOLINE_98 → 「98 無鉛汽油」`／`DIESEL → 「柴油」`；LEGACY 之「—」：`MF-1 — LEGACY 六欄（差旅新三欄／折舊 CURRENT 專屬三欄）恆常渲染，值以「—」呈現`；零附件：`B-09 — 零附件之已完成申請：證明區顯示說明文字而非空白或錯誤`；尚未產生之列印版：`§G AC-11／§3.2：尚未產生報表之已完成申請 — 列印版仍可預覽，零寫入`（`phase8-report-print.test.ts`） | T5 ＋ **MOCK-R** | GREEN |
| AC-12 | unit | `report-html.test.ts` | `AC-12 — 行程段 sortOrder 升冪呈現且每段截圖恰為該段附件` › `3 段（輸入非 sortOrder 順序）：資訊區塊依 1,2,3 升冪呈現`／`3 段：每個 segment-images 區塊僅含該段自身之 dataUri（段—圖錯置 mutant 之鑑別基礎）`／`整體字串序：SEG1 早於 SEG2 早於 SEG3（升冪之總體不變式）`；資料側對應：`AC-27/SF-2: 差旅段—圖對應 — 逐段恰為該段之 LINKED 附件，非 LINKED 者不出現，ReportImage 鍵集封閉`（`phase8-report-data.test.ts`）；附件排序 tiebreaker：`同段 3 張附件 linkedAt 完全相同、寫入序與 id 遞增序刻意相反：輸出恆依 id 遞增序（附件排序含 id tiebreaker）`（`phase8-report-print.test.ts`；**僅差旅覆蓋——保養／折舊之 tiebreaker 為終審 AR-2／X8b 守門缺口，FW→PHASE-009**） | T5 | GREEN |
| AC-13(a) | integration | `phase8-report-images.test.ts` | `AC-13(a): 圖片以 data URI 內嵌、長邊上限 1600 且不放大小圖（高熵 fixture 使邊界 mutant 必紅）；來源遺失以佔位取代且整份報表仍成功`；`AC-13(a): 取小者 — 索引色 PNG 之重編碼膨脹時改用原圖位元組（像素尺寸保留；修復前必紅）`；**實名**：`AC-13(a): 長邊超過 REPORT_IMAGE_MAX_PX（4000px 原圖）等比降尺寸至長邊 1600，嵌入位元組 ≤ 原圖`；`AC-13(a): 長邊未超過上限（800px 原圖）不放大，維持原尺寸與原位元組`；`AC-13(a): 長邊恰為上限（1600px）不視為超過，原樣嵌入`；`AC-13(a) 取小者: 索引色地圖型 PNG（2400x2400）重編碼膨脹時改用原圖位元組，像素尺寸保留`；`AC-13(a) 取小者一律化(T6R2 AR-12 釘住): 索引色 PNG＋orientation 6（需轉正且重編碼必膨脹）改用原圖，位元組相等且像素尺寸保留`；`AC-13(a) EXIF 轉正: orientation 6（轉正後長邊未超過上限）resize 前轉正，寬高互換`；`AC-13(a) EXIF 轉正: orientation 6（轉正後長邊超過上限）resize 前轉正且降尺寸，長邊 1600 且方向正確`；`B-10: 來源檔遺失 → 該圖 missing:true、dataUri:null，整份不中止（其餘圖片仍正常嵌入）`；`B-10: sharp 解碼失敗（位元組損毀，非有效圖片）亦視為缺檔佔位，不拋出`；`日誌零 key：缺檔錯誤日誌不含 storageKey 值，亦不含 att/ 子字串`；`B-11：5 張證明（上限）同時嵌入，皆成功且互不影響`；`mimeType：data URI 前綴逐字對應 img.mimeType（jpeg/png/webp 三型）` | T6 ＋ **T6R／T6R2** | GREEN |
| AC-13(b) | e2e | `e2e/report-print-layout.spec.ts` | `AC-13(b): 列印媒體下每張圖寬度與右緣不超出內容區、文件無水平溢位（移除 max-width:100% 之 mutant 必紅）` | T14 ＋ **T14b** | GREEN |
| AC-14 | e2e | `e2e/report-print-layout.spec.ts` | `AC-14: 多頁 fixture 下每個不可分割區塊之頁索引一致且任兩區塊矩形不重疊（移除 break-inside: avoid 之 mutant 必紅）` | T14 ＋ **T14b** | GREEN |
| AC-15 | unit ＋ integration | `report-html.test.ts`；`phase8-report-print.test.ts` | `AC-15: 同輸入兩次呼叫 renderReportHtml 結果全等（純函式、不讀時鐘）`；`AC-15: 列印端點回傳之 HTML 與產生 PDF 所用之 HTML 字串全等（已產生報表之申請，兩者同以含編號/產生時間之 report 欄位為輸入；PDF 走另一份模板之 mutant 必紅）`　→　**實名**：`AC-15（純函式側）— 同一 ReportData 輸入之兩次呼叫結果 toEqual` › `兩個各自獨立建構、內容相同之輸入物件，結果亦 toEqual（非物件參考比較之巧合）`；`§F AC-15：列印端點與產生時傳入 renderPdf 之 HTML 字串全等`（`phase8-report-print.test.ts`）› `已產生報表之申請：兩條路徑各自獨立計算，wire 層字串全等（非重用同一次呼叫結果）`；純函式守門：`SF-4／§7.4：零 Date.now(/new Date(/Math.random(（純函式零時鐘零隨機；鑑別 M14）` | T5 ＋ T10 | GREEN |
| AC-16 | unit ＋ integration | `report-html.test.ts`；`phase8-report-print.test.ts` | `AC-16(a): 列印版零 <script、零 on* 屬性、零外部資源參照`；`AC-16(b): 三種 XSS payload × 五個插值點皆跳脫且於 Chromium 載入後零副作用（移除跳脫之 mutant 必紅）`；**實名**：`AC-16(a) — 列印版為自足文件` › `零 <script`／`零 on*= 事件屬性`／`零 <link 標籤`／`零 http(s):// 或 // 開頭之 src／href`／`零 url( 外部資源（CSS）`／`零 @import`／`零 <img src= 使用非 data: 前綴`；`AC-16(b) — 3 payload × 5 插值點逐格跳脫`；`SF-3 — dataUri 屬性上下文跳脫（M11 鑑別：屬性逃逸 payload 不得產生真實屬性逃逸）`；`escapeHtml — 唯一跳脫函式` › `跳脫 & < > \`／`& 優先跳脫，避免雙重跳脫錯位：輸入 '&amp;' → '&amp;amp;'`／`<script>alert(1)</script> → 逐字元跳脫，零原始標籤`；`§H AC-16(c)：列印端點四個安全標頭逐字在場`（`phase8-report-print.test.ts`）› `已產生報表之申請：Content-Type／X-Content-Type-Options／Cache-Control／Content-Security-Policy 四項逐字相符`／`尚未產生報表之申請：四項安全標頭仍逐字相符（狀態守門通過後之列印版一律套用）` | T5 ＋ T10 | GREEN |
| AC-17 | integration | `phase8-report-data.test.ts` | `AC-17(a): LEGACY 差旅呈現舊模型單價、新三欄以「—」呈現且不顯示 null/NaN`；`AC-17(b): LEGACY 折舊呈現每公里單價原值、新三欄以「—」`；`AC-17(a): LEGACY 差旅 — 呈現快照油資/ETC 單價原值，新三欄為 null，零 NaN`；`AC-17(a): CURRENT 差旅 — 完整呈現五值（單價 + 油種/每公升油價/車輛油耗）`；`AC-17(b): LEGACY 折舊 — 呈現折舊每公里單價原值 + 年度公務里程，新三欄為 null`；`AC-17(b): CURRENT 折舊 — 呈現 AC-18 之五值，perKmUnitPrice 為 null`；`AC-17(c): buildReportData 全程零呼叫五個計算引擎；正控組證明 spy 有牙齒`；`AC-17(c): report-data.ts 原始碼零 .times(/.div(/.plus(/.minus(`；版型側同一紀律：`AC-17(c)：零 .times(/.div(/.plus(/.minus(（版型檔零算術）`、`差旅 LEGACY：油種／每公升油價／車輛油耗三欄標籤恆在場，值恰為「—」（非省略、非 null／NaN／undefined）`、`折舊 LEGACY：每年折舊費用／年度總里程／公務比例三欄標籤恆在場，值恰為「—」（AC-17(b) 同一紀律）`（`report-html.test.ts`） | T4 | GREEN |
| AC-18 | integration | `phase8-report-data.test.ts` | `AC-18(a): 折舊呈現五值（每年折舊費用/年度公務里程/年度總里程/公務比例/金額）`；`AC-18(b): 車價、折舊年限、預估年里程、參數版本 id 於本人與管理員兩身分皆不出現（鍵集封閉 + 字面負向斷言）`；`AC-18(a): CURRENT 折舊五值精度逐字 — 2dp/1dp/4dp%/2dp/整數`；`AC-18(b): 折舊資料組裝結果不含車價／年限／預估年里程／參數版本id — 擁有人本人檢視路徑（CURRENT）`；`…— 管理員檢視路徑（CURRENT；同一函式無 actor 分支）`；`…— 擁有人本人檢視路徑（LEGACY）`；`…— 管理員檢視路徑（LEGACY；同一函式無 actor 分支）`（四則逐字同前綴）；`AC-18(c): 對帳列之三來源值皆在場，4dp rawAmount 字面不出現`；版型側落地防線：`折舊禁列（AC-18(b) 之落地防線）：車價／折舊年限／預估年度行駛公里數／折舊參數版本 id 之字面字串零出現`、`折舊 CURRENT 對帳列（MF-2①）：.reconciliation 區塊內完整算式子字串逐字（M4 真算術／M21 清空皆須紅）`（`report-html.test.ts`） | T4 | GREEN |
| AC-19 | integration | `phase8-report-data.test.ts` | `AC-19: 保養呈現四值 + 實際費用 + 全部證明（≥1），逐字取自快照，零重算`；併同快照缺欄防線：`SF-3: 快照缺欄（status=COMPLETED 但折舊快照未寫入）— required() 顯式拋錯，訊息僅含欄名與 applicationId` | T4 | GREEN |
| AC-20 | unit | `backend/test/unit/report-filename.test.ts` | `REPORT_TYPE_LABEL / getReportTypeLabel — AC-20 型別 zh-TW 標籤` › `TRAVEL → 差旅`／`MAINTENANCE → 保養`／`DEPRECIATION → 折舊`／`映射恰為三型窮舉（無多餘、無缺漏鍵）`／`三標籤值互異（鑑別「對調」mutant 的必要前提）`；`buildReportFileName — AC-20 三型組成，四段值逐字斷言` › `TRAVEL：差旅_王小明_2026-03-05_TRV-202608-0001.pdf`／`MAINTENANCE：保養_陳大文_2026-07-20_MNT-202608-0002.pdf`／`DEPRECIATION：折舊_林小美_2026_DEP-202608-0003.pdf（年度段僅 YYYY）`／`同一使用者同型多份報表：因編號段互異而檔名互異，其餘三段相同`；`null 防禦：以 Application.primaryDate 為 fallback（三型各一）`；`SF-3（期中複審）：日期／年度段跨時區不變式（getUTC*，不受主機時區影響）` | T3 | GREEN |
| AC-21 | unit | `report-filename.test.ts` | `AC-21: 不安全字元 gate table（12 列）輸出安全且非空`；`sanitizeDisplayName / buildReportFileName — AC-21 不安全字元 gate table（12 列）` › `gate-1 ../../etc/passwd`～`gate-10 空字串 → 使用者`（含 `gate-8a CON（Windows 保留裝置名）→ 整體檔名恆不等於 CON 或 CON.pdf`、`gate-9 長度 300 字 → 姓名段截斷至 30、整體 ≤120`）；`200 例不變式 sweep — 恆不含 / \\ CR LF \\x00、恆含報表編號（確定性 LCG，非 Math.random）` › `恆不含 / \\ \\r \\n \\x00（任意輸入）`／`編號與 .pdf 恆保留；優先截姓名段、其次截日期段，最終仍 ≤120`；`SF-1（期中複審）：姓名段截斷不得產生孤兒代理字元（surrogate pair 邊界）`；`buildReportFileName — 整體 120 上限截斷（防禦性：異常長報表編號）` | T3 | GREEN |
| AC-22 | integration | `phase8-report-download.test.ts` | `PHASE-008-T9 — buildReportContentDisposition（純函式，AC-22）` › `正向：ASCII fallback 僅 [A-Za-z0-9.-]，RFC 5987 完整檔名百分比編碼`／`注入封閉：fileName 含 \\r\\n 與標頭分隔字元 → 輸出零裸 CR/LF、原文不透傳（percent-encoded）`／`encodeRfc5987ValueChars：encodeURIComponent 未跳脫之 '()* 另補跳脫`；端到端：`AC-22：Content-Disposition 雙形式與標頭注入封閉（端到端）` › `正常姓名：回應同時含 ASCII fallback 與 RFC 5987 形式，且皆與保存值一致`／`含 \\r\\n 之姓名 fixture：下載成功（200），回應標頭數量與內容未被注入（wire-level 斷言）`／`T3R/期中 FW-2 同型：含非 BMP 字元（emoji，代理對）之姓名 fixture → 下載成功（200，不 500），百分比編碼可還原、無孤兒代理` | T9 | GREEN |
| AC-23 | integration | `phase8-report-download.test.ts` | `AC-23：檔名於產生時凍結，事後改名不影響已產生報表之下載檔名` › `產生後修改 displayName 再下載 → Content-Disposition 之檔名逐字不變` | T9 | GREEN |
| AC-24 | integration | `phase8-report-print.test.ts`；`phase8-report-download.test.ts` | `AC-24: 授權矩陣 20 格逐格（狀態碼 + 回應鍵集）`；**20 格實名**：`§A AC-24：授權矩陣 — 產生端點（POST /applications/:id/report，5 格）`（`phase8-report-print.test.ts`；格1~格5 逐格）；`§B AC-24：授權矩陣 — 列印端點（GET /applications/:id/report/print，5 格）`（格1~格5 逐格）；`AC-24：授權矩陣 — 查詢端點`（`phase8-report-download.test.ts`；未登入 401／需強制改密 403／本人 200／他人 403／管理員 200）；`AC-24：授權矩陣 — 下載端點`（同檔五格）；零洩漏補強：`§C AC-24 回補（T9 AR-2/AR-3）：查詢／下載端點之 401/403 鍵集與零洩漏掃描` | T9 ＋ T10 | GREEN |
| AC-25 | integration | `phase8-report-print.test.ts` | `AC-25: 草稿之產生與列印兩端點 409 CONFLICT 且 details.status 在場`；`AC-25: 草稿之下載端點 404 NOT_FOUND（尚未產生、不帶 details.status），與已完成未產生之回應一致`；`AC-25: 草稿之查詢端點 200 {report:null}，與已完成未產生之回應一致`；**實名（兩檔）**：`phase8-report-print.test.ts` › `§D AC-25：草稿申請 — 產生與列印兩端點 409 CONFLICT + details.status`（`產生端點：草稿申請 → 409 CONFLICT，details.status = DRAFT`／`列印端點：草稿申請 → 409 CONFLICT，details.status = DRAFT`）、`§E AC-25 側信道（mutant I 必殺）：他人草稿 vs 他人已完成未產生 → 403 逐字相同`；`phase8-report-download.test.ts` › `AC-25（SPEC-REV-T9）：DRAFT 與已完成未產生之回應逐字一致`（`下載端點：DRAFT vs 已完成未產生 → 404「尚未產生正式報表」逐字相同，皆不帶 details`／`查詢端點：DRAFT vs 已完成未產生 → 200 { report: null } 逐字相同`）、`D10／B-27：尚未產生報表`、`T9R SF-1：授權順序（他人 × 未產生報表）` | T9 ＋ **T10** | GREEN |
| AC-26 | unit ＋ integration | `backend/test/unit/report-storage.test.ts`；`backend/test/unit/storage.test.ts`（既有，零改動） | `AC-26: rpt 前綴之路徑穿越矩陣全數拒絕`；`AC-26: 附件實例拒 rpt 金鑰、報表實例拒 att 金鑰（跨實例互拒）`；**實名**：`AR-3: TRAVERSAL_MATRIX_RPT covers the full 21-row matrix (18 original + 3 anchor/boundary rows)`；`LocalVolumeStorage — rpt prefix (key validation, no fs)`；`LocalVolumeStorage — rpt prefix (functional, real tmpdir)`；`AC-26: 附件實例拒 rpt 金鑰、報表實例拒 att 金鑰（跨實例互拒）` › `an att-only instance rejects rpt/ keys`／`a rpt-only instance rejects att/ keys`／`a default (no options) instance behaves exactly like an explicit att-only instance: rejects rpt/ keys`；`LocalVolumeStorage — prefix regex-escaping (AR-1: escapeRegExp correctness)`；`LocalVolumeStorage — default prefixes parameter stays ["att"]`（既有 `storage.test.ts` 12 則零改動）；**nginx 直出守門（NGINX-GUARD-LITE）**：`AC-26: nginx.conf 結構性斷言 — PDF/附件 volume 不得由 nginx location 靜態直出` › `AC-26: nginx.conf 現況零 location 之 root/alias 指向 storage/report volume 路徑`／`AC-26: nginx.conf 現況全文字面零命中 volume 路徑（全檔字面兜底之正向對照）`／`鑑別力自證（N1｜MF-1）：server 層裸 root 指向 volume 路徑（不在任何 location 內）之繞過必被偵測`／`鑑別力自證（N2｜MF-1）：帶正則／前綴修飾子（如 ~ ^/files/）之 location alias 繞過必被偵測`／`鑑別力自證（N3｜MF-1）：location 內巢狀大括號（如 if 區塊）截斷 body 之繞過必被偵測`；server 接線：`T8a-FW①：server.ts 對兩個 LocalVolumeStorage 實例皆明寫 { prefixes: [...] }`（`phase8-report-generate.test.ts`） | T8a ＋ **NGINX-GUARD-LITE** | GREEN |
| AC-27 | integration | `phase8-report-data.test.ts`；`phase8-report-download.test.ts` | `AC-27: ReportDto 鍵集恰為六鍵（toEqual 全等，多一鍵必紅）`；`AC-27: ReportData 三型 body 與 common 之鍵集封閉（toEqual 全等，多一鍵必紅）`；`AC-27/SF-2: 差旅段—圖對應 — 逐段恰為該段之 LINKED 附件，非 LINKED 者不出現，ReportImage 鍵集封閉`；DTO 側：`AC-27：ReportDto 鍵集恰為六鍵（查詢端點側，toEqual 全等）` › `查詢端點回應之 report 鍵集恰為 { reportNumber, generatedAt, fileName, byteSize, downloadUrl, printUrl }`（`phase8-report-download.test.ts`） | T4 ＋ T9 | GREEN |
| AC-28 | integration | `phase8-contract.test.ts` | `AC-28: ErrorCode 聯集全等（基線 + REPORT_GENERATION_FAILED；BOGUS mutant 必紅）`；`AC-28 結構性 — ErrorCode 聯集全等 + PHASE-008 reports/ src 僅用基線字面值` › `errors.ts ErrorCode union equals the known baseline + REPORT_GENERATION_FAILED`／`PHASE-008 reports/ 原始碼僅使用基線 ErrorCode 字面值（含 buildErrorBody 繞道），且 REPORT_GENERATION_FAILED 確實被使用`；`PHASE_008_SRC_FILES 結構清單自證（T4 即審 FW-5）` › `固定九檔清單與 backend/src/reports/ 目錄之實際 .ts 檔案清單逐一相符（新增/刪除 src 檔而未同步更新本清單即紅）`；`details.stage 四值封閉（結構）` › `ReportFailureStage 型別聯集恰為 RENDER/STORE/VERIFY/PERSIST 四值（多一/少一值必紅）`；四端點錯誤表逐格：`§D-1 — 401 UNAUTHORIZED（B-30…）`／`§D-2 — 403 PASSWORD_CHANGE_REQUIRED（B-29…）`／`§D-3 — 404 NOT_FOUND（不存在之申請 id；四端點，M11/M12 穿透封閉）`／`§D-4 — 403 FORBIDDEN（他人資料，四端點；回應零業務值）`／`§D-5 — 409 CONFLICT（B-01：草稿產生/列印）＋ 下載/查詢不適用（§7.5 修訂後對齊）`／`§D-6 — 500 REPORT_GENERATION_FAILED（正例，details.stage='RENDER'，不外洩堆疊/路徑/storage key）`／`§D-6b — details.stage='PERSIST'（確定性注入，取代先前僅機率性 MF-3 之覆蓋）`／`§D-7 — 下載端點兩種 404 訊息逐字（B-27「尚未產生正式報表」／B-28「報表檔案遺失」）` | T11 | GREEN |
| AC-29 | integration | `phase8-contract.test.ts` | **七類逐條實名**：`1／7：報表 storage key 前綴 rpt/ 不入日誌（含本次實際配得之 storageKey）`；`2／7：附件 storage key 前綴 att/ 不入日誌（含本次實際使用之標記 storageKey）`；`3／7：report volume 絕對路徑不入日誌`；`4／7：attachment volume 絕對路徑不入日誌`；`5／7：session cookie／token 不入日誌`；`6／7：密碼不入日誌`；`7／7：折舊車價／折舊年限不入日誌（全身分一致，AC-18(b)/AC-29）；PDF 位元組／base64 片段亦零出現（本套件從未將位元組交給 logger）`；反向探針：`反向探針：直接寫入七類敏感字串至同一 logStream，證明掃描機制非恆真（若日誌真的洩漏，上方 7 則測試必能偵測）`；可追查性：`requestId 在場可供追查（渲染失敗之日誌行含可追查識別 requestId 與 stage；一般錯誤路徑之日誌行亦含 requestId）` | T11 | GREEN |
| AC-30 | frontend | `frontend/test/ReportSection.test.tsx`；三型頁既有測試檔 | `AC-30: 已完成申請顯示報表區塊，產生成功後逐字顯示編號與產生時間`；`AC-30: 草稿不渲染報表區塊（負向斷言）`；`AC-30: 三型詳情頁皆掛載報表區塊`；**實名**：`AC-30: 草稿不渲染報表區塊（負向斷言：「產生正式報表」文字不存在），且零 fetch`；`AC-30: 已完成但尚未產生報表時顯示 Empty 態文案與「產生正式報表」按鈕`；`AC-30: 點擊產生後成功，逐字顯示報表編號與產生時間，並出現「檢視列印版」與「下載 PDF」兩入口`；`Success（經 GET 進入，非點擊產生）：mount 時 GET 直接回傳已產生報表，逐字顯示編號與產生時間、兩入口在場、產生鈕不在場，且零 POST`；三型頁掛載：`AC-30: 三型詳情頁皆掛載報表區塊`（**三檔各一**：`TravelApplicationPage.test.tsx`／`MaintenanceApplicationPage.test.tsx`／`DepreciationApplicationPage.test.tsx`，同名逐字） | T12 ＋ T13 ＋ **MOCK-R** | GREEN |
| AC-31 | frontend | `ReportSection.test.tsx` | `AC-31: 產生端點回 500 REPORT_GENERATION_FAILED → 顯示錯誤與重試，零編號零「已產生」零入口；重試會重新呼叫產生端點` | T12 | GREEN |
| AC-32 | frontend | `ReportSection.test.tsx` | `AC-32: 檢視列印版以 target=_blank + rel=noopener 開啟 /api/.../report/print；下載導向 /api/.../report/pdf 且點擊時零 POST`（**測試名保留 `rel=noopener` 字樣；實作實際輸出為 `rel="noreferrer noopener"`——為 AC-32 之嚴格加強（noreferrer 併同封閉 referrer 外洩），非弱化；終審 AR 記錄，測試斷言涵蓋兩值**） | T13 | GREEN |
| AC-33 | frontend | `ReportSection.test.tsx` | `AC-33: 五態` › `Loading：產生中按鈕停用並顯示「產生中…」`／`AC-30: 已完成但尚未產生報表時顯示 Empty 態文案與「產生正式報表」按鈕`（Empty）／`Error：讀取報表資訊失敗顯示錯誤與重試（與 AC-31 產生失敗分開）`／**Success 兩條觸發來源分列**：`AC-30: 點擊產生後成功，逐字顯示報表編號與產生時間，並出現「檢視列印版」與「下載 PDF」兩入口`（**POST 觸發**）與 `Success（經 GET 進入，非點擊產生）：mount 時 GET 直接回傳已產生報表，逐字顯示編號與產生時間、兩入口在場、產生鈕不在場，且零 POST`（**GET 觸發**）／`Permission denied：403 FORBIDDEN → 顯示無權限訊息，且零後續請求（無重試鈕）` | T13 | GREEN |
| AC-34 | e2e | `e2e/report-print-layout.spec.ts` | `AC-34: 列印版於 375px 可閱讀且含桌面列印提示；該提示於列印媒體與 PDF 文字中皆不出現` | T14 ＋ **T14b** | GREEN |
| AC-35 | e2e | `e2e/report-pdf.spec.ts` | `AC-35: 三型端到端（完成→產生→顯示編號→列印版→下載→重下一致）`；`AC-35: 草稿無報表入口（負向）` | T15 | GREEN |
| AC-36 | e2e | `e2e/report-pdf.spec.ts` | `AC-36: 列印版與三型詳情頁報表區塊於 375px 無水平溢位`（終審 AR-1：列印版 375px 段**無內容錨點**，僅量測溢位——內容可讀之獨立錨定由 AC-34 承擔，實害趨近零） | T15 | GREEN |
| AC-37 | 全 Task ＋ 終審 | —（無單一測試載體；以三套件實跑數為證） | 每個 Task 之 Done When 含「既有測試零弱化」；**終審三套件實跑 2948／254／44 全綠**（Phase 開工時之預估基準線為 2573/240/38；終審獨立實跑對照，`PROJECT_STATE.md` 終審記錄列） | 全 Task | GREEN |
| **AC-38（條件）** | integration | `phase8-report-keyword.test.ts` | **實名（12 則）**：`AC-38：三型皆可由完整報表編號查得` › `TRAVEL：以完整 reportNumber 查得恰該筆`／`MAINTENANCE：…`／`DEPRECIATION：…`；`AC-38：部分編號與大小寫不敏感比對` › `部分編號（期間段，如 'TRV-2045'）查得該筆`／`序號段（如 '0012'）查得該筆（MNT）`／`大小寫變體（小寫 'trv-'）仍查得該筆`；`AC-38：LIKE 萬用字元注入負向` › `keyword 'TRV-______-%' 不得命中任何申請（跳脫使其為字面比對，而非萬用樣式）`／`keyword 含字面 '%' 不命中任何 reportNumber（reportNumber 字元集不含 '%'）`；`AC-38：未產生報表之申請不受影響` › `既有 keyword 行為（出差目的比對）不受影響`／`以編號樣式查詢不誤傷未產生報表之申請（不會被錯誤地帶入結果）`；`AC-38：授權範圍維持` › `以他人之完整 reportNumber 查詢，owner 查無此筆（ownerId 範圍未被 OR 條件繞過）`／`該他人以自己身份查詢同一編號，查得恰該筆（證明資料確實存在，只是 owner 查不到）` | **T17**（D13 已於 Spec Gate 批准納入） | GREEN |
...
> **【結案 DOC-SYNC 之清單決議（2026-08-07；`PHASE-008-DOC-SYNC`）】**
> ⒜**不新增「AC-14 版面驗證方法論」同步條目**——**決議依據為實查而非推定**：`ARCHITECTURE.md` 與 `DATA_FLOW.md` 全文對「列印／PDF」之敘述皆為**流程與職責面**（`ARCHITECTURE.md` §1 技術棧 PDF 列、§3 `reports` 模組列、§4.7；`DATA_FLOW.md` §2.4 :258-265 之產生／預覽／下載流程，其中 :261「多頁不重疊」係**流程層之使用者可見結果描述**），**兩檔皆無「如何驗證版面」之方法論敘述面**，故無可同步之落點；該方法論已由 **§11.4【方法論修訂】＋【鑑別力歸屬之澄清】兩段完整承載**，另立條目只會產生第二處事實來源。
> ⒝**裁-3 勘誤（保養 vs 差旅之油種欄位歸屬）維持「待人類覆核」，本批次不得結案**——見 §2 AC-11 修訂引註 ⒞ 之勘誤段與 §18 `SPEC-REV-MOCK` 列；**DOC-SYNC 無權代為認定人類原意**，該項續掛於結案待決清單，直至人類明示（若確認「差旅即為原意」則勘誤關閉；若原意含保養新增油種欄位，則屬 Scope 變更須另批修訂）。
...
| **T14** | 列印版面機械驗證（列印媒體量測）＋ 桌面列印提示　**【方法論已修訂（2026-08-07；`SPEC-REV-T14`／`T14B`，批准依據＝§18 同名列）：括號內「列印媒體量測」之適用範圍限縮至 AC-13(b) 與 AC-34 之 DOM 面；AC-14 改以真實後端 PDF 之零依賴幾何量測承接（`node:zlib` ＋ `.segment` 邊框封閉子路徑／`.image-block` 影像 XObject，字型無關）——見 §11.4【方法論修訂】引註。技術落地於 `T14b`（`9e0447d`）。右欄 Done When 各項義務逐字不變，含兩個樣式 mutant 各必紅】** | E2E ＋ FE | AC-13(b), AC-14, AC-34 | `e2e/report-print-layout.spec.ts`、`backend/src/reports/report-html.ts`（僅樣式修正範圍） | Medium | T5, T10 | ~700 行／~185-265k | 圖片右緣與文件寬度量測綠；區塊頁索引一致 ＋ 兩兩不重疊；兩個樣式 mutant（移除 `max-width`／`break-inside`）各必紅；提示於列印媒體不可見 |
...
1. **「多頁不重疊」之驗證域**：於 DOM ＋ 列印媒體模擬驗證（同一 Chromium 排版引擎），**非**於 PDF 位元組層解析驗證——後者需新增 PDF 解析相依。PDF 層以完整性（magic bytes／長度／雜湊）與 Gate 目視覆蓋。（§11.4 方法論說明）
   > **【修訂（2026-08-07；`PHASE-008-SPEC-REV-T14B`；批准依據同 §18 `PHASE-008-SPEC-REV-T14` 列）】** 本項上述敘述**已失效，不再是本 Phase 之限制**：①「於 DOM ＋ 列印媒體模擬驗證」對 AC-14 **經實證無鑑別力**（見 §11.4 【方法論修訂】引註）；②「PDF 位元組層解析**需新增 PDF 解析相依**」之前提**不成立**——T14b 已落地**零新 npm 依賴**之幾何解析（僅 `node:zlib`），AC-14 **即是**於 PDF 位元組層解析驗證。**修訂後之實際限制（本項續存者僅此二點）**：⒜**幾何量測之涵蓋範圍**——僅 `.segment` 邊框與 `.image-block` 影像兩類訊號，`.reconciliation`（無邊框、無影像）不在其內（單行小區塊；FE-US-23④ 之核心風險已由前二類覆蓋）；⒝**PDF 文字層**之 `ToUnicode` 擷取僅安全用於「**不存在**」方向之斷言（AC-34），不得反向用於「存在」之斷言。PDF 之完整性（magic bytes／長度／雜湊）與 Gate 目視覆蓋**一律不變**。
...
| 2026-08-07 | `PHASE-008-SPEC-REV-T14B` | **前列（`SPEC-REV-T14`）之內部一致性收口——同一裁定之連動，非新決策**：該列已明記三處「未處理之落差」＋一項對齊，經大總管授權於本批收口，修訂四處落點——⒜**§2 AC-14（:186）**：原句**逐字保留**，其下新增【修訂】引註——上句所述之取證手段（「於列印媒體模擬下量測 `top`／`bottom-1` 之 `floor(y / 頁面內容高度)`」）**經 T14 探針實證對本 AC 無鑑別力**，故取證手段改指向 §11.4 【方法論修訂】引註之**真實後端 PDF 零依賴幾何量測**；引註內**逐字重申驗收語意零變更**（不可分割區塊不得被頁面邊界切開＋任兩區塊互不重疊，並記明幾何量測中之等價證據＝封閉邊框子路徑總數恆等於段落數）與**「移除 `break-inside: avoid` 之 mutant 必紅」一字不減**（實測 6 → 15）。⒝**§17.1 #1**：原文**逐字保留**，其下新增【修訂】引註——明記該項兩點敘述已失效（①DOM ＋ 列印媒體模擬對 AC-14 無鑑別力；②「PDF 位元組層解析**需新增 PDF 解析相依**」之前提**不成立**，T14b 為**零新 npm 依賴**之幾何解析，AC-14 **即是**於 PDF 位元組層驗證），並改列**續存之實際限制兩點**（⒜幾何量測涵蓋 `.segment`／`.image-block` 二類、`.reconciliation` 除外；⒝`ToUnicode` 文字擷取僅限「不存在」方向）；PDF 完整性與 Gate 目視覆蓋不變。**§11.4 原段末「限制已記於 §17.1」之互指**於同節 【方法論修訂】引註後補一段【互指同步】說明（**原段文字零改寫**，沿後段引註覆蓋前段慣例）。⒞**§15 T14 列**：**標題欄**補記【方法論已修訂】——「列印媒體量測」之適用範圍限縮至 AC-13(b)／AC-34 之 DOM 面、AC-14 改由真實 PDF 幾何量測承接、技術落地於 **T14b**（`9e0447d`）；**右欄 Done When 原文逐字不變**（含兩個樣式 mutant 各必紅），依 §15 慣例**不另立 T14b 列**。⒟**§12 AC-13(b)／AC-14／AC-34 三列 Task 欄**：`T14` → **`T14 ＋ T14b`**（沿 `T6 ＋ T6R`／`T8 ＋ T8R…` 先例，與 `PROJECT_STATE.md` 之「T14/T14b：DONE」結案列對齊）；**測試名與 `GREEN` 狀態不動**。**一律不變**：三條 AC 之**字面與驗收語意**、全部 mutant 必紅義務、§16 全部決策、§11.4／§17.1／§15 之既有原文（一律以後段引註覆蓋前段，零改寫）、其他章節。**性質判定**：**測試策略層之記錄型收口**（治理分級「可修改但必須記錄」），**未擴大 Scope、未新增依賴、未縮減任何 AC 之守門強度、未改變任何已批准 US 原意與使用者可見行為**；**未修改 `userstory.md`／`docs/PRD.md`／任何程式或測試**。Spec 狀態維持 `ACTIVE`。**本批後 §11.4／§2 AC-14／§17.1 #1／§15 T14 四處敘述已一致**；`SPEC-REV-T14` 列所列之三處落差**全數結清**。 | 同 §18 `PHASE-008-SPEC-REV-T14` 列（`PROJECT_STATE.md` 2026-08-06「T14 第一輪：BLOCKED」裁定列，commit `64314f0`；T14b 落地實證 commit `9e0447d`）；大總管 2026-08-07 授權收口（`SPEC-REV-T14B` Packet） |
| 2026-08-07 | `PHASE-008-SPEC-REV-MOCK` | **Mock Gate 三裁定之 Spec 本體落地（使用者可見輸出格式之修訂；已由人類逐案裁定並複述確認）**：依 `PROJECT_STATE.md` 2026-08-07「★ Mock Gate 通過（人類 leonchih 2026-08-07）＋三裁定（AskUserQuestion 複述確認後落檔）★」列（commit `5fd2061`；走查腳本 `docs/verification/WALKTHROUGH-PHASE-008-MOCK.md`，版型目視**通過**）修訂四處落點——①**§2 AC-11**：必含項目表**逐字保留**，其下新增【修訂】引註，載明三裁定之呈現面義務——⒜**裁-2**「報表標題」**欄位列移除**、該資訊改由版面**大標題 `<h1>{類型}報表`** 單一承載（**AC-11「必含類型標題」語意不變**，計數表述由「8 個欄位列」改為「**7 個欄位列 ＋ 1 個類型大標題**」，缺任一項必紅之守門強度不變）；⒝**裁-1**「產生時間」與「申請建立時間」**兩欄一併**改台北時區中文在地化格式（**使用者確認之範圍釐清**；目標長相＝前端既有「計算時間」之 `2026/8/6 上午9:23`；`null` 時之「尚未產生」佔位不變）；⒞**裁-3**「油種」改中文名稱（`92 無鉛汽油`／`95 無鉛汽油`／`98 無鉛汽油`／`柴油`，與前端下拉選項用字逐字一致；**LEGACY 之 `—` 不變**）。引註併載**契約不變式四點**（`renderReportHtml` 仍純函式、仍不讀時鐘；**AC-15 wire 全等不受影響**；`ReportData` 之 ISO 值與型別不變、在地化發生於呈現層故 **AC-27／AC-17／AC-18／AC-19 一律不受影響**；**格式化須決定性**——不得受 `TZ` 環境變數或 ICU locale 在場與否影響，建議沿 `report-number.ts` 之固定 `TAIPEI_OFFSET_MS` ＋ `getUTC*` 自組字串，零 `Intl` 依賴），以及**實作落點建議**（油種中文映射於**後端唯一一處**窮舉 `Record<FuelType, string>`、比照 `REPORT_NUMBER_PREFIX` 取型別窮舉守門；**如實記載**前端既有三份相同映射之位置與「FE／BE 跨層合併不在本次範圍」；時間格式化以一處共用純函式承載）。②**§2 AC-30**：原句**逐字保留**，其下新增【修訂】引註**收口「逐字」之定義**（**T12~T13 複審 FW 預留之回填點**）——報表編號維持**嚴格逐字**（不自組不轉換，自組之 mutant 必紅）；產生時間改為「**值取自 DTO、以台北時區中文格式呈現**」並**明記 mutant 牙齒保留**（格式化結果須由 DTO 值推導；`Date.now()`／忽略 DTO 值／寫死字串三種變異皆須紅）；AC-31～AC-33 不受影響。③**§12 AC-11 列**：預定名由一條擴為**四條**（既有一條之「共通 8 項」連動為「共通 7 欄位列 ＋ 類型大標題」；新增裁-2／裁-1／裁-3 各一條），Task 欄 `T5` → **`T5 ＋ MOCK-R`**，狀態註記「待 MOCK-R 落地後實名回填」。④**§12 AC-30 列**：新增裁-1 之預定名一條，Task 欄 → **`T12 ＋ T13 ＋ MOCK-R`**，同註記。**一律不變**：AC-11 必含項目表原文與其他各型清單、AC-30 原句、其餘 AC 語意、§16 全部決策、§7.3／§7.4 契約、任何程式與測試。**性質判定**：屬**使用者可見輸出格式**之修訂，**已由人類 2026-08-07 於 Mock Gate 逐案裁定並經 AskUserQuestion 複述確認**，spec-writer 未自行定案；**AC 條數不變（38）、無任何 AC 之守門強度被弱化、未擴大 Scope、未改變任何已批准 US 原意**；**未修改 `userstory.md`／`docs/PRD.md`／任何程式或測試**。Spec 狀態維持 `ACTIVE`。**併記勘誤（交大總管／人類覆核）**：裁-3 之裁定原文與走查腳本步驟 6 記為「**保養**報表的油種」，實查「油種」欄位**僅存在於差旅列印版**（`report-html.ts` :196），保養版型與 `MaintenanceReportBody` 皆無此欄；本 Spec 依實作落點將該裁定落於**差旅**油種欄位，**未擅自為保養報表新增油種欄位**（若人類原意含此，屬新增揭露欄位之 Scope 變更，須另批人類批准）。 | 人類 leonchih 2026-08-07 Mock Gate 三裁定（AskUserQuestion 複述確認；固化於 `PROJECT_STATE.md` commit `5fd2061` 之 Mock Gate 通過列）；走查腳本 `docs/verification/WALKTHROUGH-PHASE-008-MOCK.md`（`51efe5a`） |
| 2026-08-07 | `PHASE-008-DOC-SYNC`（A＋B 批） | **結案文件同步（Spec 側；A 段 §12 全表實名回填、B 段本文四項）**：依終審 `REQUEST_CHANGES` 之 DOC-SYNC 義務彙整清單（`PROJECT_STATE.md` 2026-08-07 終審記錄列，commit `ec8f000`）——**A-1 §12 全 43 列實名回填並轉 `GREEN`**：實名**一律以 `grep` 逐檔核對後回填，未採信任何清單抄錄**（方法：自 `report-*.test.ts`／`phase8-*.test.ts`／`ReportSection.test.tsx`／三型頁測試／`e2e/report-*.spec.ts` 機械抽取全部 `describe`／`it` 標題後逐列比對）；巢狀者以 `describe 標題 › it 標題` 表示，全部字串**逐字可 grep**。**A-2 AC-38**：`BLOCKED（待裁定）` → **`GREEN`**，Task 欄改 `T17`（D13 已於 Spec Gate 批准），回填 5 個 describe ＋ 12 則 it 實名。**A-3**：§4 五態表之 **Success 列拆為 POST 觸發／GET 觸發兩列**（對應兩條實測）；**AC-32 列註記** `rel="noreferrer noopener"` 與 AC 字面 `rel=noopener` 之差異係**嚴格加強非弱化**；**AC-11／AC-30 秒差**於 AC-11 修訂引註下新增【BE／FE 秒差之容忍】段（BE 無秒／FE 含秒，屬裁-1 容忍範圍，**不視為違反**）。**B-4 SF-2 記錄型修訂（大總管裁定採 (a)）**：§11.2 新增【AC-09(c) 取證方式】引註——**實查 `phase8-pdf-render.test.ts` 對 AC-09(c) 零命中**；取證改記為①**容器 probe**（T7b／T7bR 三層法，含 **tofu 對照像素簽名**）＋②**整合 Gate 目視**（判準含**繁體字形正確**，非僅無方框），probe 步驟固化於 `WALKTHROUGH-PHASE-008-INTEGRATION.md` **可重跑**；併記**無 vitest 常駐守門之方法論理由**（開發機有系統字型 → 該測試對「容器漏裝字型」之回歸**先天無鑑別力**，屬恆真式守門，與 §11.0 mutant 紀律牴觸）與**殘留風險**（Dockerfile 移除字型不會有測試變紅，防線為 review ＋ Gate）；§12 該列狀態明標 **`GREEN（文件化取證，非自動化測試）`**，禁止被讀為自動化通過。**B-5**：§11.4 新增【鑑別力歸屬之澄清】——`.image-block` 影像矩形計數對 `break-inside: avoid` **天生不敏感、不承擔 mutant 必紅義務**；**AC-14 之 mutant 必紅僅由 `.segment` 封閉邊框子路徑計數承擔**（防後續維護者誤推）。**B-6**：§13 末新增【結案 DOC-SYNC 之清單決議】——**不新增 AC-14 方法論同步條目**（**實查** `ARCHITECTURE.md` §1／§3／§4.7 與 `DATA_FLOW.md` §2.4 :258-265 皆僅有流程／職責敘述、**無驗證方法論敘述面**，另立條目將產生第二事實來源）。**B-7**：**裁-3 勘誤維持「待人類覆核」明示不得結案**（DOC-SYNC 無權代為認定人類原意）。**一律不變**：所有 AC 字面與語意、§16 全部決策、既有測試名（僅新增實名，未改寫任何既有字串）、任何程式與測試。**性質**：文件同步與測試實名回填（記錄型），**未改變任何行為、未擴大 Scope**。Spec 狀態維持 `ACTIVE`。**本批次未處理（BLOCKED 分批，已依 Packet 授權回報）**：**C 段** `ARCHITECTURE.md`／`DATA_FLOW.md` §13 十一項之落地（兩檔本 Phase 現況**零 diff**）——需獨立批次；**D 段**程式註解（SF-3）另派 implementer，不在 spec-writer 範圍。 | 終審 reviewer `REQUEST_CHANGES`（SF-1／SF-2／SF-3；固化於 `PROJECT_STATE.md` commit `ec8f000` 終審記錄列）；大總管 2026-08-07 `PHASE-008-DOC-SYNC` Packet（SF-2 裁定採 (a)）；實名以本批次 `grep` 逐檔核對為據 |
| 2026-08-07 | `裁-3 勘誤人類覆核結案`（大總管記錄） | **裁-3 勘誤之人類覆核結果**：使用者 leonchih 2026-08-07 裁定**保養報表不新增油種欄位**（原話意旨「保養跟油種有什麼關係」）——SPEC-REV-MOCK 與 §13 決議 ⒝ 所留「若人類原意含保養報表新增油種欄須另批」之待覆核事項**正式結案**：油種中文化僅落差旅列印版欄位（既有唯一油種欄），**零範圍擴大**。本列為裁定記錄（大總管白名單之修訂紀錄區），未改動任何 AC 與本文。 | 人類 leonchih 2026-08-07（對話裁定；固化於 `PROJECT_STATE.md`「使用者三裁定」列①、commit `a9e82d5`） |
| 2026-08-09 | `SPEC-REV-008` | **§11.4 PDF 字型內嵌敘述之記載型更正（跨 Phase 移交項⑦；零語意變更）**：§11.4【方法論修訂】②末段原記之「dev（Windows、CJK 字型缺裝）→ 逐字 `/Subtype /Type3` 內嵌；正式容器（已裝字型）→ `Type0`／CID；初版 `ToUnicode` 錨點法因該**管線分歧**而失敗」**與 T8R2 SF-7 實測不符**，於該節末新增【PDF 字型內嵌敘述之更正】引註據實更正三點——⒜實測（本機真實 Chromium）為**子集字型之 glyph 編碼**，`/FontFile` ＋ `/ToUnicode` **皆在場**、文字操作子為一般 `Tj`／`TJ`（實測 5 個），**非** `Type3` 向量化，文字**原則上可還原**；⒝「正式容器為 `Type0`／CID 且與 dev 不同構」屬**當時推定而非實測**，如實記載應為「**無法排除**兩環境有差異」；⒜⒝之外⒞初版錨點法失敗根因應記為「**本機回退鏈下子集字型之 CMap 覆蓋不完整**」（`SEGORIGIN1` 之 `S` 於對應字型 CMap 無條目），**與是否 `Type3` 無關**。更正必要性：誤信「已向量化」將使後續 Phase 錯誤認定 PDF 無文字層而放棄一切文字面斷言。**一律不變**：所有 AC 字面與驗收語意、AC-14 mutant 必紅義務及其歸屬、改採**字型無關幾何訊號**之裁定與理由（更正後理由**更強**）、§11.4 ③與 §17.1 #1 ⒝「`ToUnicode` 僅安全用於『不存在』方向」之限制（子集 CMap 覆蓋不全只產生**偽陰性**，故**不放寬**）、§16 全部決策、§12 全表、其他章節；**未修改任何程式、測試、`userstory.md`、`docs/PRD.md`**。**性質判定**：COMPLETED Spec 之**記載型勘誤**（治理分級「可修改但必須記錄」層之測試策略／技術細節面），非 AC／US／公開契約／使用者可見行為之變更；**Spec 狀態維持 `COMPLETED`**。**歷史列不改寫**：本節 `PHASE-008-SPEC-REV-T14` 列（2026-08-07）內含之同型敘述（「dev Windows 缺 CJK 字型 → 逐字 `Type3` 內嵌…與正式容器 `Type0`／CID 不同構」）**依本 Spec 既有慣例（一律以後段引註覆蓋前段、歷史紀錄零改寫）不予改寫**，以本列與 §11.4 更正引註為準。 | `PHASE-009-T18` AC-40(a)（commit `5dfd8ed`）；`PHASE-008-T8R2` SF-7 實測記載＝`backend/test/integration/phase8-report-generate.test.ts` :586-605；已更正之同型敘述＝`e2e/report-print-layout.spec.ts` :50-59（`PHASE-009-T18`／`T19-DOCFIX`，commit `381d778`）；大總管 2026-08-09 `SPEC-REV-008` Packet（PHASE-009 開工記錄移交跨 Phase ⑦） |
